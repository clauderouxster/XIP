    ...
    for r in elements:
        if tl(r):
            ...
            if r[0:2]=="X_":
                ...
                ps=r.find('=')
                subname=r[:ps]
                nr='x_test_string(lret,'+subname+'.size(),'+subname+')'
                if r not in keepvariables:
                    keepvariables.append(r)
                rule.append(nr)                
            else:
            ...
            if "symbole" in ruletree:
                ...
                if lx==1:
                    ...
                    if sb in stringtocompare.keys():
                        ...
                        ivarstring="i"+varstring
                    else:
                        varstring="varstring"+str(idstringtocompare)
                        ivarstring="ivarstring"+str(idstringtocompare)
                        idstringtocompare+=1
                        stringtocompare[sb]=varstring
                    result.append('x_test_string(lret,'+ivarstring+','+varstring+')')                    
...
def compiling(filename):
    ...
    for e in f:
        ...
        if l[0]=='>':
            ...
            parsenot.append(ct)
        #we detect punctuations (single character)
        if l[0]=='&':
            l=l[1:]
            ps=l.find(":")
            ct=l[:ps].strip()
    ...
    for stcp in stringtocompare:
        foutput.write("static const int i"+stringtocompare[stcp]+'=strlen('+stringtocompare[stcp]+');\n')
    ...
    for u in usefullstring:
        sy=parsenotrule(u,classname,"5",tokenrule)
        foutput.write(sy)
    for u in usetokens:
