...
"%UIMA_JAVA_CALL%" -cp "%UIMA_CLASSPATH%" "-Dcom.xerox.xrce.uimaxip.core.grammar.grmfile=%GRAMMAR_FILE%" "-Duima.home=%UIMA_HOME%" "-Duima.datapath=%UIMA_DATAPATH%" -DVNS_HOST=%VNS_HOST% -DVNS_PORT=%VNS_PORT% "-Djava.util.logging.config.file=%UIMA_LOGGER_CONFIG_FILE%" %UIMA_JVM_OPTS% org.apache.uima.tools.docanalyzer.DocumentAnalyzer
C: