...
//----------------------------------------------------------------------
static bool arreteExecution;
extern "C" {
	Exporting void SetXipExecution(bool v) {
		arreteExecution = v;
	}

	Exporting bool xipExecutionEnd() {
		return arreteExecution;
	}
}
//----------------------------------------------------------------------
...
Label* GlobalParseur::AjouteLabel(VECTA<Label*>& v, char* e, typeRegle t,int c,int& nbs,DecompteRegle* d) {
	...
	if (e[0]!='#') {
		if (rechercheLabel(e)!=NULL) {
			...
			sprintf_s(tampon,500,"%s: %s",e,CHAINES[257]);
			erreur(tampon);
		}
		ArbreLabel[e]=l;
	}
...
//    if (strcmp(repertoire,rep)) {
        strcpy_s(repertoire,1024,rep);
		int lngrep = strlen(repertoire) - 1;
		if (lngrep >= 0 && repertoire[lngrep] != CHSEP[0])
            strcat_s(repertoire,1024,CHSEP);
        strcpy_s(Nom_Fichier,1024,repertoire);
        strcat_s(Nom_Fichier,1024,nom_fichier);
        strcpy_s(Nom_Desamb,1024,repertoire);    
//    }
  
    if (laLangue!=PERSONNEL)
        strcpy_s(commentaire,1024,repertoire);
    ...
    appliqueGrammaireDefini=appliqueGrammaire;
     ...
     if (laLangue==FRANCAIS)
         strcat_s(commentaire,1024,"chaines.fic");
     else
         if (laLangue==ANGLAIS)
             strcat_s(commentaire,1024,"strings.file");
...
void GlobalParseur::Initialisation(char* rep) {
    ...
    if (access(Nom_Fichier,00)) {
        NOM=CHAINES[262];
        sprintf_s(commentaire,1024,"%s: %s",CHAINES[259],Nom_Fichier);
        erreur(commentaire);
    }
    ...
    for (i=0;i<Procedures.dernier;i++) {
        if (Procedures[i]->X.type==C_PROCEDUREDECLAREE) {            
            sprintf_s(mes,100,"%s:%s",CHAINES[255],Procedures[i]->X.nom);
            erreur(mes);
        }
    }
	...
	MAJNumeroRegles();	
	if (GUESSED_TAG[0]==0)
		strcpy_s(GUESSED_TAG,256,"+guessed+Noun");
...
void GlobalParseur::DupliqueGrmFiles(char* nom) {
    ...
    if (grm.fail()) {
        sprintf_s(bufread,1000,"Cannot open: %s",nom);
...
void GlobalParseur::ChainagePlusieurs(istream& fichier,
    ...
    while (k!=';') {    
        ...
        if (entrelacement==2) { //Cas particulier pour la lecture des modules...
            strcpy_s(nomFichier,2048,tampon);
            Trim(tampon);
        }
        else
            strcat_s(nomFichier,2048,tampon);
...
void GlobalParseur::LitNomFichiers(void) {
	...
	if (fichier.fail() ) {
		strcpy_s(tampon,500,CHAINES[97]);
		strcat_s(tampon,500,Nom_Fichier);
		erreur(tampon);
	}
	LIGNES=0; 
	...
	strcpy_s(UtiliseLimite,300,Nom_Fichier+rec+1);
	Trim(UtiliseLimite);
	...
	while (1) {
		...
		if (!strcmp(tampon,"version")) {
			...
			strcpy_s(version_grammaire,1024,tampon);
			continue;
		}
		...
		if (!strcmp(tampon,"guesserfeature")) {
			...
			Trim(tampon);
			//Lecture de la version, c'est une simple chaine
			strcpy_s(guesser_feature,100,tampon);
			continue;
		}
		...
		if (!strcmp(tampon, "onguessed")) {
			LitLigne(fichier, tampon);
			Trim(tampon);
			strcpy_s(GUESSED_TAG,256, tampon);
			continue;
		}
...
void GlobalParseur::LitReg(char* nom_regle) {
	...
	if (fichier.fail() || strlen(nom_regle)<=1 || nom_regle[strlen(nom_regle) - 1] == SEP) {
        ...
        strcpy_s(nom_fichier,1024,nom_regle+i+1);
        k=Llit(fichier,tampon);
...
void GlobalParseur::LitVoc(char* nom_lexique,char lire_couche) {
    ...
    if (fichier.fail() ) {
        strcpy_s(tampon,500,CHAINES[97]);
        strcat_s(tampon,500,nom_lexique);
        erreur(tampon);
    }
    else {
...
char GlobalParseur::ChoixLectureRegle(istream& fichier,char* tampon) {
    ...
    char ch[200];
    strcpy_s(ch,200,CHAINES[217]);
    strcat_s(ch,200,tampon);
    erreur(ch);
...
char GlobalParseur::ConsPL(istream& fichier) {
    ...
    if ((trgauche->valeur)->Zero() ) {
        sprintf_s(tampon,500,"%s %s",CHAINES[137],TC(this,trgauche));
        erreur(tampon);
    }
    ...
    if ((trdroit->valeur)->Zero()) {
        sprintf_s(tampon,500,"%s %s",CHAINES[137],TC(this,trdroit));
        erreur(tampon);
    }
...
char GlobalParseur::AppelCons(istream& fichier,char k, char* tampon) {
    ...
    if (!strcmp(tampon,CHAINES[181]) && k==':') {
        k=litBis(fichier,tampon);
        if (k!=':')
            strcpy_s(TAG_NOUVEAU,256,tampon); 
		if (!TestOptions(NON_NORMALISATION_TAG))
...
void EtatArbre::print(ostream& os,int profondeur) {
    ...
    sprintf_s(chh,20,"%d:%d>",etat,nombre);
    strcat_s(ch,20,chh);
...
void ArcArbre::print(ostream& os,int id,int profondeur) {
    ...
    strcat_s(ch,200,X->nom);
    ch[taille+5]=0;
    sprintf_s(chh,200,"%d:%d",num,clefsuivante);
    strcat_s(ch,200,chh);
    if (argument!=-1) {
        sprintf_s(chh,200,"#%d",argument);
        strcat_s(ch,200,chh);
    }
    if (type==2)
        strcat_s(ch,200,"^");
...
void GlobalParseur::LitProcedurePython(istream& fichier) {
    ...
    */
    ...
    if (pt!=NULL)
        strcpy_s(tampon,1000,pt+1);
    else
        strcpy_s(tampon,1000,Nom_Fichier);
    //On retire le point
    pt=strchr(tampon,'.');
    if (pt!=NULL)
        *pt=0;
    sprintf_s(espace_module,XIPBUFFERSIZE,"%s%d",tampon,num_module_python);
    ...
    */
	...
	if (type==DEDUCTIONGENERATION) {
		if (c->X.type!=C_GENERATION && c->X.nom[0]!='?') {
			sprintf_s(suite,100,"%s: %s",CHAINES[392],c->X.nom);
			erreur(suite);
		}
	}
...
void GlobalParseur::LitLesDeductions(istream& fichier,typeDeduction typededuc) {
    ...
    char ch[20];
	uchar kbrut = LectCrypte(fichier);
	k = minlettre(kbrut);
    VECTA<Noeud*> vect;
    ...
    while (etat!=100) {            
        ...
        case 0: {
			...
			LectureBruteBlancs(fichier, kbrut);
			k = minlettre(kbrut);
        ...
        case 2:            
            ...
            //sinon on lit la suite
			tampon[0] = kbrut;
			k = litbrut(fichier, tampon + 1);
			{
				string s = conversion_utf8_to_latin(s_to_lower(tampon));

				if (nomsoriginaux.find(s) == nomsoriginaux.end())
					nomsoriginaux[s] = tampon;

				strcpy_s(tampon, XIPBUFFERSIZE, STR(s));
			}
...
char GlobalParseur::AffichageFonction(istream& fichier,char cacher) {
    ...
    char k,tampon[XIPBUFFERSIZE],fin=0;
    k = lit(fichier,tampon);
   ...
   //Dans le cas d'un fichier de vocabulaire, il ne peut y avoir rien d'autre...
    ...
    while (tampon[0]!=0) {
        if (!TestNomChamp(tampon,k)) {
            char err[100];
            sprintf_s(err,100,"%s:%s",CHAINES[217],tampon);
            erreur(err);
        }
	...
	while (i>=min) {
		...
		if (VlabelVecteur[i]->dr->couche_relative>=basecouche) {
			...
			if (i>numero || VlabelVecteur[i]->dr->couche_relative>basecouche) {
				sprintf_s(ch,100,"%d>",VlabelVecteur[i]->dr->couche_relative+1);
...
int GlobalParseur::ChargerDependances(string nomfichier) {
	...
	while (!donnees->eof()) {
		...
		if (c==NULL) {
			erreur_survenue=142;
			strcpy_s(ajoute_info_erreur,4096,tampon);
			affiche_derniere_erreur(erreur_survenue);
			Detruit();
			TasDeListes->Raz();
			NoeudsTemporaires.raz();
		...
		while (k!=')') {
			...
			if (k=='[') {
				...
				if (L==NULL) {
					...
					vlex.X=c->Categ();
					strcpy_s(vlex.lemme,2048,L->N.fils->lemme);
					strcpy_s(vlex.surface,2048,L->N.fils->surface);
...
char GlobalParseur::chargementprobabilites(istream& fichier) {
	...
	while (!fichier.eof()) {
		...
		if (c1!='=' || c2!=':' || c3!=':' || c4!='*' || sep!=';') {
			strcpy_s(tampon,500,CHAINES[400]);
			erreur(tampon);
		}
...
char GlobalParseur::chargeprobabilites(string nom) {
	ifstream fichier((char*)nom.c_str(),modeOuverture);
	strcpy_s(Nom_Fichier,1024,(char*)nom.c_str());
	NOM=Nom_Fichier;
	char tampon[XIPBUFFERSIZE];
	if (fichier.fail()) {
		strcpy_s(tampon,500,CHAINES[399]);
		strcat_s(tampon,500,Nom_Fichier);
		erreur(tampon);
	}
