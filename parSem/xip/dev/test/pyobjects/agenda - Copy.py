...
#reunion du 1 janvier 2008 au 30 décembre 2008
...
def questionnement(datedepart,datearrivee,lieu,sujet,phrase,avecdate,doubledate,avecjour,avecheure):
    ...
    #namespace = outlook.GetNamespace("MAPI")
    #app=namespace.GetDefaultFolder(9).Items
    print "QUESTION:",datedepart,datearrivee
    rdv= outlook.CreateItem(olAppointmentItem)    
    rdv.Subject=sujet
    rdv.Start=datedepart.strftime("%d/%m/%Y %H:%M")
    rdv.End=datearrivee.strftime("%d/%m/%Y %H:%M")
...
def init():
    return xips.loadgrm(r"C:\eclipse\adresse\src\adresse.grm")
...
def calculdate(u,d):
    ...
    if "SLASHE:+" in f:
        ...
        if "ANNEE:+" in f:
            ...
            fils=lefils
            while fils!=None:
                if fils.pos[0]=="NUM" and "BASE:4" in fils.features[0]:                    
        ...
        if "JOUR:+" in f:
            ...
            if "PASSE:+" in f:
                ...
                multi=-1
            if "DECALAGE:+" in f:
                jourdiff=deltaduree(lefils.lemma[0])
            else:                
                if "ENHUIT:+" in f:
                    increm=7
                if "ENQUINZE:+" in f:
...
def extraitheure(u,n,dfeat):
    ...
    if "UNH:+" in n.features[0]:
        hm=n.surface.split("h")
...
def analyse(grm,app,s,quest):

    #try:
        res=objects.XipResult()
        objects.parse(grm,s,res)
        print res
        ...
        if not enregistrer and not question:
            ladate=aujourdui
            heuredebut=[aujourdui.hour,aujourdui.minute]
            avecheure=True

        doubledate=len(lesdates)
        #Cas particulier. Nous avons deux dates, mais pas de date d'origine ni de fin
        if doubledate==2:
            lesdates.sort()            
            if datedebut==None and datefin==None:
                datedebut=lesdates[0]
                datefin=lesdates[1]
        else:
        ...
        if not question:
            ...
            #c'est une question
            print "DATES 3",datedebut,datefin,avecdate,doubledate,avecjour,avecheure
            app.majcalendar()
...
class App:
    ...
    def __init__(self, master):
        ...
        self.hi_there = Tkinter.Button(self.frame, text="Enregistrer", default=Tkinter.ACTIVE, command=self.analysis)
        self.hi_there.pack(side=Tkinter.LEFT)

        self.quest = Tkinter.Button(self.frame, text="Question", fg="red", command=self.question)
        self.quest.pack(side=Tkinter.LEFT)

        self.quest = Tkinter.Button(self.frame, text="Recharger", fg="green", command=self.recharger)
        self.quest.pack(side=Tkinter.LEFT)


        self.button = Tkinter.Button(self.frame, text="Quitter", fg="red", command=self.quitter)
        ...
        self.text.bind("<KeyRelease-r>", self.intercepte)
        self.text.bind("<KeyRelease-s>", self.intercepte)
        self.text.bind("<KeyRelease-t>", self.intercepte)
        self.text.bind("<KeyRelease-e>", self.intercepte)

    def intercepte(self,*args):
        s=""
        listes=["enregistrer", "enregistre", "enregistrent", "enregistres", "Enregistrer", "Enregistre", "Enregistrent", "Enregistres"]
        try:
            ...
            for ls in listes:
                ...
                if p!=-1:
                    print s[:p]
                    analyse(self.grm,self,s[:p],False)                       
                    id=self.text.search(ls, 1.0, Tkinter.END)        
                    self.text.delete(id,Tkinter.END)
                    return
        except:
            pass
    ...
    def afficher(self,datedebut,datefin,lieu,sujet,force):
        ...
        chaine+="Sujet:\t"+sujet+"\n"
        chaine+="------------------------------\n"
        print "Dans:",str(datedebut)
        chaine=unicode(chaine).encode("Latin-1")
        chaine+="Début:\t"
        chaine+=datedebut.strftime("%d/%m/%Y %H:%M")+"\n"
        chaine+="Fin:\t"+datefin.strftime("%d/%m/%Y %H:%M")+"\n"
        chaine+="------------------------------\n"
        chaine+="Lieu:\t"+unicode(lieu).encode("Latin-1")+"\n"
        d=Dialog.Dialog(self.maitre,title="Entrée du calendrier",text=chaine,strings=["Suivant","  Ok  "],bitmap="",default=0)
        return d.num
    ...
    def acceptation(self,datedebut,datefin,lieu,sujet,corps):
        ...
        chaine+="------------------------------\n"
        chaine+="Lieu:\t"+lieu+"\n"
        chaine+="Mots clefs:"+corps[corps.find('#')+2:]+"\n"
    ...
    def majcalendar(self):
        ...
        unan=datetime.datetime.now()+datetime.timedelta(365)
    ...
    def question(self):
        s=""
        si=self.text.get(1.0,Tkinter.END)
        try:
            s=str (si)
        except:
            s=unicode(si).encode("Latin-1")
        analyse(self.grm,self,s,True)
...
appagenda=App(root)
#analyse(0,appagenda,"Visite chez le médecin, Jeudi à 9h30",False)

#analyse("Meeting de 30mn, mardi à 14h00",False)

#analyse("rendez-vous la semaine du 25",False)
#analyse("Dinner en mai",True)
