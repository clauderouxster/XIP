...
def addelements(graphes,dp,posarg):
    ...
    root=buildelement(dp[1])
    arg=buildelement(dp[2])
...
def addarguments(graphes,dp):
    #first we use the first element as our main key...!
    arg=[buildelement(dp[1]),buildelement(dp[2])]
    depname=dp[0]
    ...
    for g in graphestoappend:
        ...
        if g[0]==2:
            ...
            if test:
                ...
                graphes["KEYS"][grammarcodes.extractjustroot(dp[1])]=True
                graphes["KEYS"][grammarcodes.extractjustroot(dp[2])]=True
        else:
            if test!=None:
