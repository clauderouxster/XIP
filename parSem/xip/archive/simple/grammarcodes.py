...
def translate(wrd,dico,root,language):        
    if "root" in wrd:
        ...
        #We have two cases: either the lower case exist as a root, or it is the original form
        lookotherpos=False
        if rt in root:
            lookotherpos=True
        elif rti in root:
        ...
        if lookotherpos:
            ...
            if language=="francais" or language=="english":
                #We try adjective into a noun
                if wrd["pos"]=="Adj":
                    nn=rt+"ta"
                    if nn in dico:
                        return dico[nn].split("=")[1]
                    
                if wrd["pos"]=="Adj" or wrd["pos"]=="Noun":
