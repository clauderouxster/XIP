...
##     self.mekaBox.pack(side=LEFT, fill=BOTH, expand=1)
     ...
     self.traduire.buildidx(self.dicoenglish,self.dicofrench)
...
###Parsing the sentence
   ...
   def recursivecasegeneration(self,pronouns,i,tags):
        if i>=len(pronouns):
             nbmodif=0                                           
             genres=self.gen_text
             nbmodifbis=1           
             while nbmodifbis>0:
                 try:                
                    #sx=self.aff(genres)
             ...
             if nbmodif>self.maxprojections:
             ...
             self.listcase=self.lstant
             if i==0:
                 #we can restrict the list of antecedent
                 ph=p-1
                 self.listcase={"ANT_NOM":True}
                 while ph>0:
                     wrd=self.gen_text[ph][1]
                     end=ending.ending(wrd)
                     if end["pos"]=="Pron":
                         break
                     if "case" in end:
                         self.listcase["ANT_"+end["case"].upper()]=True
                     ph-=1
                 self.listcase=self.listcase.keys()
                      
             for lant in self.listcase:
                 ...
                 tags.insert(0,lant)                 
                 nbmodif=self.recursivecasegeneration(pronouns,i+1,tags)
                 tags.pop(0)
   ...
   def updateprojected(self,toreplace,l,newcase,gentext):
        #We rebuild our word together with the ending from the target sentence
        root=toreplace["root"]
        gfind=toreplace['surf']
  
        if "case" in toreplace and newcase!=toreplace["case"] and toreplace["pos"]!="Prep" and newcase!=None:
        ...
        if l!=None:            
            if root in self.ontologyheads:
                ...
                if toreplace["pos"]=="Verb":
                    ...
                    foreign=[gentext[l["p"]][0],gfind]                             
                    return foreign
        else:
            return [gfind,gfind]

   def updatesisters(self,l,newcase,gentext,genres):
        if newcase==None or newcase==l["case"]:
           return
      ...
      for g in self.generationgraphes:
        ...
        if semantics.multiplematch(gr,g,True):
            ...
            for lstvarias in variablelist.values(): 
                ...
                #We create a copy of gentext
                genres=[]
                inumword=0
                for e in gentext:
                    genres.append(e[:])
                    genres[-1].append(inumword)
                    inumword+=1

                lstequ=g['ALIGN']

                #We have many cases:
                #1st case: elements that should be destroyed
                #An element that should be destroyed has the following specificity, it hsa None as value in lstequ
                firstelement=True
                lstvarkeys=lstvarias.keys()
                lstvarkeys.sort()
                toswap=[]
                previous=None
                previoustoreplace=None
                for vk in lstvarkeys:
                    modif=True
                    nbmodif+=1
                    ...
                    indexvar=int(vk[1:])
                    l=ndx[matchingnode][1]
                    positionelement=l["p"]
                    #Swapped elements
                    toreplace=lstequ[indexvar]                    
                    #We delete this element                    
                    if toreplace==None:
                        genres[positionelement]=[genres[positionelement][0],None]
                    else:
                        if previoustoreplace!=None and previoustoreplace>toreplace["i"]:
                            #Then it is a swap,we keep it
                            toswap.insert(0,[previous,genres[previous["p"]],toreplace,l])
                            continue
                        
                        previoustoreplace=toreplace["i"]
                        previous=l
                        newcase=None
                        
                        if firstelement and "case" in l:
                            newcase=l["case"]
                        else:
                            if "case" in toreplace:                            
                                newcase=toreplace["case"]
                        newelement=self.updateprojected(toreplace,l,newcase,gentext)
                        newelement.append(inumword)
                        inumword+=1
                        genres[positionelement]=newelement
                        self.updatesisters(l, newcase, gentext, genres)
                        firstelement=False
                        
                toadd=[]
                for sw in toswap:
                    [previous,wheretoinsert,toreplace,l]=sw
                    newcase=None
                    if toreplace["pos"]=="Adj":                        
                        if "case" in previous:
                            newcase=previous["case"]
                    newelement=self.updateprojected(toreplace, l, newcase, gentext)
                    #The we move it
                    if l["pos"] in ["Noun","Verb"] and "up" in l:
                        parent=l["up"]
                        idx=parent["node"][0]["p"]
                        for e in parent["node"]:
                            genres.pop(idx)
                    toadd.append([wheretoinsert,newelement])
                ...
                for e in lstequ:
                    ...
                    if e<0:
                        ...
                        if positionelement!=None:
                            testposition=positionelement
                            newelement=[toreplace["surf"],toreplace["surf"]]                            
                            if toreplace["pos"]=="Prep":#then we must add Dat
                                if "case" in l and l["case"]=="Gen" and "case" in toreplace:
                                    precelement=gentext[positionelement-1]
                                    wrd=ending.ending(precelement[1])
                                    if "case" in wrd:
                                        suff=self.addprepcase(toreplace, wrd["case"])
                                        newelement=[suff,suff]
                                self.updatesisters(l, "Dat", gentext, genres)      
                            elif toreplace["pos"]=="Adj":
                                if "case" in l:
                                    newelement=self.updateprojected(toreplace, None, l["case"], None)                                    
                                positionelement=l["p"]
                                testposition=positionelement-1
                            elif toreplace["pos"]=="Verb":
                                #We must test if it is a relative verb
                                newelement=self.updateprojected(toreplace, l, None, gentext)
                            
                            element=newelement[:]
                            element.append(testposition)
                            if element in genres:
                                error=True
                                break                                    
                            toadd.append([genres[positionelement],newelement])
                ...
                for t in toadd:
                    [wheretoinsert,newelement]=t
                    position=genres.index(wheretoinsert)
                    genres.insert(position,newelement)
   ...
   def getlocalgraph(self):
      ...
      self.align(gr,lexicals,sentright)    
      self.globalsentence=lexicals
      self.lastgraphe=gr
      if sentright=="":
   ...
   def align(self,newgr,lexicals,res):
      #We align the two sentences
      ...
      consumed=[]
      resconsumed=[]
      #First we search all common lemmas
      i=0
      nbalign=0
      #The we try to align along POS+CASE
      for w in lexicals:
          if nbalign==len(wordsres):
              break                    
          if w["pos"]==wordsres[nbalign]["pos"]:
              if "case" in w:
                  if w["case"]==wordsres[nbalign]["case"]:
                      aligns[i]=wordsres[nbalign]
                      consumed.append(i)
                      resconsumed.append(aligns[i]["i"])
                      nbalign+=1
              elif "tense" in w:
                  if w["tense"]==wordsres[nbalign]["tense"]:
                      aligns[i]=wordsres[nbalign]
                      consumed.append(i)
                      resconsumed.append(aligns[i]["i"])
                      nbalign+=1
      ...
      if nbalign<nbwords:
          for w in lexicals:
              if i in consumed:
                  i+=1
                  continue
              if w["rootbasis"] in lemmas:
                   add=lemmas[w["rootbasis"]]
              else:
                  lstvariations=self.traduire.generateOntologicalVariations(w)
                  add=None
                  if len(lstvariations)>1:
                      testwith="_"+"_".join(lstvariations)+"_"
                      for l in lemmas:
                          if "_"+l+"_" in testwith:
                              add=lemmas[l]
                              break
              ...
              i+=1
      if nbalign<nbwords:
          #we still have some words unmatched, we add them into the alignement with a negative key
          for w in wordsres:
              if w["i"] in resconsumed:
                  continue
              aligns[w["i"]*-1]=w
      if len(consumed)<len(lexicals):
          i=0
          for w in lexicals:
   ...
   def initgenerationgraphes(self):
       ...
       for spair in self.generationgrapheslist:
          ...
          try:    
              sgr=grammar.parse(sent,sentence,wlexicals,self.dicoparta,self.roots,newgr,False,None,grammarcodes.currentlanguage,None)
          except grammar.Eraiag,e:
              res="Exception: "+e.error+" --> "+e.my_message
              self.text.delete(1.0,END)
              self.text.insert(INSERT,res,"a")
          ...
          self.align(newgr,wlexicals,res)
         ...
         if self.lastgraphes!=None and "QUESTION" in self.lastgraphe:
            ...
            for g in self.lastgraphe:
                if g in ["KEY","ARGUMENTS"]:
                    if g=="ARGUMENTS":
                        for kg in self.lastgraphe[g]:
                            res+=kg+" = "+str(self.lastgraphe[g][kg])+"\n"
                    continue
...
##     self.rootFrenchBox.delete(0,END)
     ...
     for i in msorted:
         ...
         if lems!="":
            #i is a lingveta word 
            #lems is the translation of that word combined with all other meanings (in French too)
            self.dicoparta[i]=unicode(rac+"="+lems,"utf8")
            #rac is the root of the lingveta word
