...
def parse(ph,sentence,lexicals,dico,root,graphes,debug,generated,language,ontologies):
    #deux cas, avec un 
   phrs=ph.split(".")
   commonroots={}
   res=""
   for i in phrs:
       ...
       if s!="":
           if s[-1]!=".":
               s+="."
           sent=[]
           res+=parsing(s,sent,lexicals,dico,root,graphes,commonroots,debug,generated,language,ontologies)
           sentence.extend(sent)
           res+=endl
   return res

def parsing(ph,sentence,lexicals,dico,root,graphes,commonroots,debug,generated,language,ontologies):
    if debug:
        print ph
    lph=ph.split()
    categories={}
    ...
    for m in lph:
        ...
        try:
            ontologyinput="_".join(ontologies[str(x['root'])])+"$"
            x["root"]="_"+x["root"]+"_"+ontologyinput
        except:
            pass
    ...
    if generated!=None:
        dependencystring=[]
        stgen=translation.generestring(idxsentence,dps,dico,root,localgraphe,mxnb[0],dependencystring,language,ontologies)
        stgen=stgen.rstrip()
