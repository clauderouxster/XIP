...
def browse(tree,res):
    if "%NAME" in tree:
        ...
        if name in ["nb","pos"]:
            res[name]=value
        if name in ["surface","lemme"]:
            res[name]=unicode(value[:-1],"latin1").encode("utf8")
    if "%NODES" in tree:
        lst=tree["%NODES"]
        for l in lst:
            browse(l,res)
...
def splitword(w):
    sw=parsenoden.parse(w)
    res={}
    browse(sw,res)
    if "att" in res:
        lst=[res["lemme"]]
        lst.extend(res["att"])
        res["lemme"]="_".join(lst)
    if res["surface"][0].isupper():
        res["lemme"]+="_PROPER"
    if res["pos"]=="VERB" and "_PL" in res["lemme"]:
        res["lemme"]=res["lemme"].replace("_PL","") 
    return [res["pos"],res["lemme"],res["nb"],res["surface"]]


def COMP(feats,n,w1,w2):
    return ["COMP",n,splitword(w1),splitword(w2)]
def COORD(feats,n,w1,*w2):
    lstw=[]
    for w in w2:
        lstw.append(splitword(w))
    return ["COORD",n,splitword(w1),lstw]
def JUXT(feats,n,w1,w2):
    return ["JUXT",n,splitword(w1),splitword(w2)]
def APPOS(feats,n,w1,w2):
    return ["APPOS",n,splitword(w1),splitword(w2)]
def SUJV(feats,n,w1,w2):
    return ["SUJ",n,splitword(w1),splitword(w2)]
def AUXV(feats,n,w1,w2):
    return ["AUX",n,splitword(w1),splitword(w2)]
def CODV(feats,n,w1,w2):
    if "PROPQUE" in feats:
        return ["CODVPROPQUE",n,splitword(w1),splitword(w2)]
    else:
        return ["CODV",n,splitword(w1),splitword(w2)]
def MODV(feats,n,w1,w2):
    return ["MODV",n,splitword(w1),splitword(w2)]
def MODN(feats,n,w1,w2):
    return ["MODN",n,splitword(w1),splitword(w2)]
def MODA(feats,n,w1,w2):
    return ["MODA",n,splitword(w1),splitword(w2)]
def MODR(feats,n,w1,w2):
    return ["MODR",n,splitword(w1),splitword(w2)]
def MODP(feats,n,w1,w2):
    return ["MODP",n,splitword(w1),splitword(w2)]
def CPLV(feats,n,w1,w2):
    return ["CPLV",n,splitword(w1),splitword(w2)]
def ATBSO(feats,n,w1,w2):
    return ["ATBSO",n,splitword(w1),splitword(w2)]
def PREPOBJ(feats,n,w1,w2):
    return ["PREPOBJ",n,splitword(w1),splitword(w2)]
def DETERM(feats,n,w1,w2):
    return ["DETERM",n,splitword(w1),splitword(w2)]
def NOUNPHRASE(feats,n,w1,w2):
    return ["NOUNPHRASE",n,splitword(w1),splitword(w2)]
def NEGAT(feats,n,w):
    return ["NEGAT",n,splitword(w)]
...
class generation:
    ...
    def __init__(self,chemin):
       ...
       xips.setdisplay(self.grm,xips.XIP_DISPLAY_REDUCED|xips.XIP_DISPLAY_ENTREE|xips.XIP_DISPLAY_CATEGORY|xips.XIP_DISPLAY_WORDNUM)
       self.frenchToLingve={}
       ...
       self.depidx={"DETERM":0,"SUJV":1,"CODV":2,"CODVPROPQUE":13,"PREPOBJ":3,"MODN":4,"MODV":5,"COMP":6,"MODA":6,"MODR":7,"ATBSO":8,"CPLV":13,"AUXV":10,"NOUNPHRASE":11,"COORD":12,"NEGAT":14}
       self.featuretransfers={"p":["gv","gn"],"gv":["verb","aux"],"gn":["det","noun","adj","ga"],"det":["num"],"gcoord":True,"adv":["adverb"],"ga":["adj"],"verb":["verb"]}    
    ...
    def gettranslation(self,w):
       ...
       if w['pos']=="det":
           ...
           if "PROPER" in w['features'] and w['pos'] in ["noun","adj"]:
                ...
                return               
           try:
              tr=self.frenchToLingve[w["pos"]][basewrd]
    ...
    def g_CODV(self,r,feat):
        ...
        if pos=="CONJQUE":
            ...
            if self.testnode(["gn",noun[2]]):
                hgn=self.getparent(["gn",noun])["parent"]
                hp=self.getparent(["p",verb])
                self.movechild(hp,hgn,0)
                cq=self.getnode(noun)
                self.cleancase(cq)
                cq["features"].extend(["KER"])
            else:                
                self.buildnode(["P",verb,"REL"],[["CONJQUE",noun,["KER","ACC"]]],0)
        elif noun[0]=="COORD":
        ...
        if feat!=None:
            ...
            if "NUM" in feat:
                ...
                if feat==None:
                    ...
                    feat.append("DETERM")
                self.buildnode(["GN",noun,feat],[["det",num,None]],0)
                return
    ...
    def gv_gn_prep(self,node):
        try:
            if "GP" in node["features"]:
                ...
                if chld[0]["pos"]=="prep":
                    i=1
                    while chld[1]["pos"]!="gn" or "GP" not in chld[1]["features"]:
                        i+=1
    ...
    def parse(self,sent):
       ...
       x=xips.parsestring(self.grm,s,xs)
       self.nodes={}
       x=x.replace(" ","*_*")
       x=x.replace("'^","\\'^")
       ls=x.split()
       self.idx=len(ls)
       ls.sort(self.compare)
       for i in ls:
       #extracting dependency features
           ...
           self.currentdep=i[:i.find("(")]
           if "_" in self.currentdep:
               self.currentdep=i[:i.find("_")]
           dep=i[:-1]
           ps=dep.find("_")
           psb=dep.find("(")
           thedep=dep[:psb]
           ...
           if ps!=-1:
              if ps<psb:
                feat=dep[ps+1:psb].split("_")
                thedep=dep[:ps]
                dep=dep[:ps]+"("+str(feat)+","+dep[psb+1:]
                modif=True
           if not modif:
               dep=dep[:psb]+"([],"+dep[psb+1:]

           #extracting the different nodes from the dependency
