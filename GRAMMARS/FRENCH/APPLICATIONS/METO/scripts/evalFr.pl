...
##########################################################
# Scriptname: evalFr.pl
#
# Call: perl evalFr.pl <submissionfile> <goldstandardfile>
#Example Call: perl evalFr.pl ../eval_fine.txt ../countriesFr.xml
...
sub readsubmission{
	...
	if($linenumber==1){
	    ...
	    if($_ =~ /(document_\d+)\|(.*)\s*/){
		$Article=$1;
		$assignedreading{$Article}=$2;
		if(&included($Article,@submissionArticles) ==1){
		    die "the samplenumber $Article is included twice in your submission. Each sample must be included exactly once!\n";
		}

		else{
		    push(@submissionArticles,$Article);

		}

	    }
	    if((&included($assignedreading{$Article},@readings) ==0) && (!($assignedreading{$Article} eq "unknown"))){
		die "the samplenumber $Article was assigned a reading by you which is not in the target categories!\n";
	    }
	}
    }
    close(SUB); 
...
sub getreadings{
	...
	@readings = ("literal","metonymic");


    }
    elsif($granularity eq "fine"){
	if($class eq "location"){
	    @readings = ("literal","place-for-people","place-for-event");

	}
	elsif($class eq "org"){
	    @readings = ("literal","organisation-for-members","organisation-for-facility","object-for-name");
	}
...
sub readgold {  #
	...
	if($_=~ /id\=\"(document_\d+)\"/){
	    $Article = $1;
	    push(@goldArticles,$Article);
	    
	}
	elsif($_=~ /reading\=\"([^\"]*)\"/){ #This contains the medium reading
	    $realreading{$Article}=$1;
	
	    if($granularity eq "coarse"){
		if(!($realreading{$Article} eq "literal")){
		    $realreading{$Article} = "non-literal";
		}
...
sub check{ #checks whether the same samplenumbers are collected
    ...
    @submissionArticles = sort @submissionArticles;
    #print "sub\n";
    #foreach $elem (@submissionArticles){
    #  print "$elem\n"
    #    };

    @goldArticles = sort @goldArticles;
#    print "gold\n";
#    foreach $elem (@goldArticles){
#      print "$elem\n";
#     }
#
    if(!(@submissionArticles eq @goldArticles)){
	die "The samplenumbers you submitted do not correspond to the samplenumbers in the gold file\n";
    }
...
sub compare{
    ...
    foreach $Article (@goldArticles){
	$nbArticles++; #counts number of samples
	$rr = $realreading{$Article};
	$ar = $assignedreading{$Article};
	#print STDOUT "$Article\|$ar|$rr\n";
	...
	elsif($ar eq "unknown"){  #deal with partial submissions
...
sub printme{
    ...
    $coverage = $covered/$nbArticles; 
    $allcoverage = $allcovered/$nbArticles;#should always be 1
    $ballacc=$balltrues/$nbArticles; 
    ...
    foreach $r (@readings){
	$percentage{$r} = $occurring{$r}/$nbArticles;
	unless($assigned{$r}==0){
    ...
    print "numberofArticles=$nbArticles\n\n";
...
#    print "baselinecovered (of all)=$Articles\n";
