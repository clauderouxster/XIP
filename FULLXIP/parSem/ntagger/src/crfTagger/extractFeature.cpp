...
void extractFeature::features(const vector<string> &tokenSeq,vector< vector <string> >& crfInput){
	...
	{   
		...
		if(token.length()!=0){
			...
			temp.push_back(toString(this->hyphon(token)));
...
bool extractFeature::initMap(const string &path,map<string, string>& Cluster,bool mkcls){
	...
	if (myfile.is_open())
	{
		...
		while ( myfile.good() )
		{
			...
			if(mkcls==1){
				if (i>1){
					code= extractFeature::binary(boost::lexical_cast<int> (arr[1]));
...
vector<string> extractFeature::clusterFeatures(const map<string, string> &myMap,const string &token){
	...
	if ( myMap.find(passToCluster) == myMap.end() ) {
		...
		static const boost::regex repeat("([\\w])\\1{1,}");
		static const boost::regex doublequota("[“”]");
		static const boost::regex otherquota("[‘’´`]");
		const std::string repl2 = "\"";
		const std::string repl1= "'";
		const std::string repl3 = "$1";
		passToCluster= boost::regex_replace(passToCluster, otherquota, repl1);
		passToCluster= boost::regex_replace(passToCluster, doublequota, repl2);
		passToCluster= boost::regex_replace(passToCluster, repeat, repl3);
...
bool extractFeature::hashtag(const string &s){
	...
	static const boost::regex hash("#[\\w\\W]+");
...
bool extractFeature::url(const string &s){
	...
	urlEnd = "(?:\\.\\.+|[<>]|\\s|$)";
	static const boost::regex  url("(?:"+urlStart1+"|"+urlStart2+")"+urlBody+"(?=(?:"+urlExtraCrapBeforeEnd+")?"+urlEnd+")");
...
string extractFeature::xXshape(const string &s ){
	...
	static const boost::regex upperCase("[A-Z]");
	static const boost::regex lowerCase("[a-z]");
	static const boost::regex digit("[0-9]");
	const std::string rep_Up = "X";
	const std::string rep_Low= "x";
	const std::string rep_Dig = "9";
	string transformed= boost::regex_replace(s, upperCase, rep_Up);
	transformed= boost::regex_replace(transformed, lowerCase, rep_Low);
	transformed= boost::regex_replace(transformed, digit, rep_Dig);
	return transformed;
...
string extractFeature::charclassshape(const string &s ){
	...
	for (int i=0;i<s.size();i++) {
		onechar=c_char_get((unsigned char*)s.c_str(),i);
...
string extractFeature:: cap(const string &s){
	static const boost::regex htt("http://?+");
	int t=0;
	int numChar = 0;
	int numCap = 0;
	for (t=0; t< s.size(); t++) {
...
bool extractFeature::EMO(const string &s){      
	...
	static const boost::regex  emoticon(Emoticon);
...
bool extractFeature::email(const string &s){      
	...
	static const boost::regex  email(Email);
...
bool extractFeature::numberWithCommas(const string &s){
	...
	static const boost::regex  nuWithCom(numberWithCommas);
...
bool extractFeature::arbitraryAbbrev(const string &s){
	...
	static const boost::regex  arbitraryAbbrev(artAbbrev);
...
bool extractFeature::arrows(const string &s){
	string arr = "(?:<*[-―—=]*>+|<+[-―—=]*>*)+";
	static const boost::regex  Arrows(arr);
...
bool extractFeature::timelike(const string &s){
	...
	static const boost::regex  tm(timeLike);
...
bool extractFeature::embeddedApostrophe (const string &s){
	...
	static const boost::regex  emappost(EmbeddedApostrophe);
