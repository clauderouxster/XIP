/* $Id: ntmload.c,v 1.5 2009/01/29 17:03:04 ait Exp $ */
...
typedef enum {SPACES, END_SPACES, SEPARATORS, END_SEPARATORS, ALIAS,
FILENAME, NEWLINE, NORMNET, LEXINET} Expectations;
...
inline unsigned char *fgetString(unsigned char *s, int n, FILE *in, unsigned char withspaces) {
    int c=0, i=0;
    ...
    s[i++]= (unsigned char) c;
    c=fgetc(in);
    while ( (i < n) && (c!=EOF) &&  ( (!withspaces && !isspace(c)) || (withspaces && c!='\n') ) ) {
        s[i++]= (unsigned char) c;
        c=fgetc(in);
    }
    s[i] = 0;
...
inline unsigned char *getString(unsigned char *s, int n, unsigned char* in, int *cnt, Expectations expected) {
    int c=0, i=0, counter=*cnt;
    ...
    if (expected==FILENAME) {
        /*In the case that we are reading a pathname, then spaces might be part of the pathname itself
         we then stop at a newline*/
        while ( (i < n) && (c!=0) && c!='\n') {
            s[i++]= (unsigned char) c;
            c=in[++counter];
        }
...
inline int getValueFromHexChar(char c) {
    if ( isnumeric(c) ) {
        return (c - '0');
    } else if (c >= 'A' && c <= 'F') {
        return ( 10 + c - 'A');
...
inline int getValueFromHexString(unsigned char *s) {
    int i=0, v=0, n=0;
    for(i=0; s[i]; i++) {
        n = getValueFromHexChar(s[i]);
        if ( n >= 0 ) {
            v = v*16 + n;
        } else {
            return -1;
...
inline int getCharDecimalCode(unsigned char *s) {
    ...
    if (s[0] == '\\') {
        if (s[1] == 'u') {
            n = getValueFromHexString(s+2);
            fprintf(stderr, "\nUnicodeDecimalValue: %d\n",n);
            return n;
        } else if (s[2] == 0) {
            return (int) s[1];
        } else {
...
void loadStrategyScript(NtmContext* ct, char *filename) {
    ...
    unsigned char withspacesflag = 0;
    ...
    if (inscript == NULL) {
        ...
        expected = ALIAS;
        while ( fgetString(s, NTM_FILENAME_SIZE, inscript, 0 ) ) {
            if (isnewline(s)) {
                if (expected == SPACES) {
                    expected = END_SPACES;
        ...
        while ( fgetString(s, NTM_FILENAME_SIZE, inscript, withspacesflag ) ) {
...
void loadStrategyScriptFromString(NtmContext* ct, char *scriptstring) {
    
    unsigned char s[NTM_FILENAME_SIZE];
    int code=0, classes_reset=0;
    NtmNetStruct nets[NTM_NETS_MAXNB];
    int netsNb=0;
    int counter=0;
    ...
    if (scriptstring == NULL) {
        errorHandler(ct, "Unable to read for string: string is NULL");
        return;
    }
    /* first part : SPACES & SEPARATORS */
    expected = ALIAS;
    while ( getString(s, NTM_FILENAME_SIZE, scriptstring, &counter, expected ) ) {
        if (isnewline(s)) {
            if (expected == SPACES) {
                expected = END_SPACES;
            } else if (expected == SEPARATORS) {
    ...
    while ( getString(s, NTM_FILENAME_SIZE, scriptstring, &counter, expected) ) {
        ...
        if (isnewline(s)) {
            ...
            if (expected == ALIAS) {
                ...
                /* a filename for defining an alias */
                if (ntmError(ct)) return;
                strcpy(nets[netsNb].filename, (char *)s);
                netsNb++;
                expected = NEWLINE;
