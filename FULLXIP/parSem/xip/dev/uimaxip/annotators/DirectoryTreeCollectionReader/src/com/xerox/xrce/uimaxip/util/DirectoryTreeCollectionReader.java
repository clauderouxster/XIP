/*
 * Modified version of FileSystemCollectionReader from the
 * UIMA distribution, which was distributed with the following
 * notice:
...
package com.xerox.xrce.uimaxip.util;
...
public class DirectoryTreeCollectionReader extends CollectionReader_ImplBase {
	  /**
	  ...
	  public void initialize() throws ResourceInitializationException {
	    ...
	    // get list of files in the tree rooted at the specified directory
	    mFiles = new ArrayList();
	    buildFileList(directory,mFiles);
	  }

	  private void buildFileList(File directory,ArrayList results) throws ResourceInitializationException {
		File[] files = directory.listFiles();
		for (int i = 0; i < files.length; i++) {
		  if (files[i].isDirectory()) {
			buildFileList(files[i],results);
		  } else {
			results.add(files[i]);
		  }
		}
	  ...
	  public void getNext(CAS aCAS) throws IOException, CollectionException {
	    ...
	    // XCAS input files
	      ...
	      try {
	        ...
	        UIMAFramework.getLogger(DirectoryTreeCollectionReader.class).log(Level.WARNING,
	  ...
	  public static CollectionReaderDescription getDescription() throws InvalidXMLException {
	    InputStream descStream = DirectoryTreeCollectionReader.class
	            .getResourceAsStream("DirectoryTreeCollectionReader.xml");
	    return UIMAFramework.getXMLParser().parseCollectionReaderDescription(
	            new XMLInputSource(descStream, null));
	  }
	  
	  public static URL getDescriptorURL() {
	    return DirectoryTreeCollectionReader.class.getResource("DirectoryTreeCollectionReader.xml");
	  }
	}
