...
public:
	...
	KifElement* MethodOptions(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (curloptions.find(option) == curloptions.end())
			return kifcode->Returnerror("URL(031): Unknown option");
	...
	KifElement* MethodURL(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (callfunc->Size() == 1) {
			...
			if (tmp == NULL)
				return kifcode->Returnerror("URL(009): Wrong filename");
...
extern "C" {
	Exporting bool Init_kifcurl(KifCode* kifcode, string version) {
		...
		if (kifGlobals->TestExternalType("curl") == true)
			return false;
		...
		KifType res = KifAddNewType("curl", CreatekifcurlElement);
		if (res == kifVOID)
			return false;
		curl_type = res;
	...
	Exporting bool kifcurl_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		MethodInitialization(kifcode, "proxy", &Kifcurl::MethodProxy, P_ONE, "proxy(string prox): Proxy connection");
		MethodInitialization(kifcode, "password", &Kifcurl::MethodPWD, P_TWO, "password(string user,string pwd): Provide the user password");
		MethodInitialization(kifcode, "url", &Kifcurl::MethodURL, P_ONE | P_TWO, "url(string path): Load a URL\rurl(string path,string filename): Load a url into a filename");
		MethodInitialization(kifcode, "execute", &Kifcurl::MethodExecute, P_NONE | P_ONE, "execute(string filename): Execute a curl action, filename is optional.");
		MethodInitialization(kifcode, "options", &Kifcurl::MethodOptions, P_TWO, "options(string option,string parameter): Set the options of the curl object");
