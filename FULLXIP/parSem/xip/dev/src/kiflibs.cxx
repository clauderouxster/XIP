...
Exported const char* kifErrorStrings[] = {
	...
	"KIF(266): Expecting one parameter for this object",
...
KifElement* KifCode::KifParseCall(x_node* xn, KifElement* kf) {
	...
	if (name == "_stacksize") {
		...
		if (xn->nodes.size() == 2) {
			...
			string message = "Missing parameter in '_stacksize'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}

	if (name == "_joinedlimit") {
		//very specific case, to set the stack size from within
		if (xn->nodes.size() == 2) {
			KifCallFunction call(NULL, NULL, idloadin);
			KifInstruction kxf(NULL, NULL);
			...
			string message = "Missing parameter in '_joinedlimit'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}

	if (name == "_threadlimit") {
		//very specific case, to set the stack size from within
		if (xn->nodes.size() == 2) {
			KifCallFunction call(NULL, NULL, idloadin);
			KifInstruction kxf(NULL, NULL);
			...
			string message = "Missing parameter in '_threadlimit'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}

	if (name == "_mirrordisplay") {
		//very specific case, to set the stack size from within
		if (xn->nodes.size() == 2) {
			KifCallFunction call(NULL, NULL, idloadin);
			KifInstruction kxf(NULL, NULL);
			...
			string message = "Missing parameter in '_mirrordisplay'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}

	if (name == "_garbagesize") {
		...
		if (xn->nodes.size() == 2) {
			...
			string message = "Missing parameter in '_garbagesize'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}

	if (name == "_garbagefunction") {
		if (xn->nodes.size() == 2) {
			KifElement* kfunc = NULL;
			...
			string message = "Missing parameter in '_garbagefunction'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}
	...
	if (name == "_setenv") {
		if (xn->nodes.size() != 2 || xn->nodes[1]->nodes.size() != 2) {
			string message = "Missing parameter in '_setenv'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		KifCallFunction call(NULL, NULL, idloadin);
...
KifElement* KifCode::KifParseFunction(x_node* xn, KifElement* kf) {
	...
	if (kf->type == kifFunction) {
		string message = "Error: You cannot declare a function within a function: '" + name + "'";
		throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
	}
		...
		if (predeclared != NULL) {
			if (predeclared->arguments.size() == kfunc->arguments.size()) {
				...
				if (found) {
					...
					string message = "Error: Cannot find a matching function to a pre-declared function (check the declaration order): '" + name + "'";
					throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
				}
			}
		}
