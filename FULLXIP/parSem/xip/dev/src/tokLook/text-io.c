/* $Id: text-io.c,v 1.8 2014/02/07 16:26:31 roux Exp $ */
...
NETptr seqs_to_net_create(id_type *(*seq_form)(void),NVptr *nets,STATEptr *final_state, STATEptr **start_destinations)
     /*****
      *
...
void seqs_to_net_add_next(id_type *(*seq_form)(void),NETptr *net,NVptr nets,STATEptr* final_state, STATEptr **start_destinations,int *result_count,int *sd_len)
     /*****
      *
      * Each call to SEQ_FORM returns a vectors of arc labels terminated by ID_EOS.
      * Returns a minimal network containing a path for every sequence
...
NETptr seqs_to_net_end(id_type *(*seq_form)(void),NETptr *net,NVptr nets,STATEptr* final_state, STATEptr *start_destinations,int result_count,int sd_len)
     /*****
      *
      * Each call to SEQ_FORM returns a vectors of arc labels terminated by ID_EOS.
      * Returns a minimal network containing a path for every sequence
...
//Claude Roux (21/09/2010). Modification to compile from a map
static id_type *two_lines_from_map_to_labels(void)
{
  ...
  //UPPER_BUF and LOWER_BUF must be loaded before
  if (WORD_BUF_UPPER==NULL || WORD_BUF_LOWER==NULL)
	  return NULL;
  strcpy(WORD_BUF,WORD_BUF_UPPER);
  line_to_labels(UPPER_BUF);
  strcpy(WORD_BUF,WORD_BUF_LOWER);
  line_to_labels(LOWER_BUF);

  if (*UPPER_BUF==0 || *LOWER_BUF==0)
	  return NULL;
...
//Claude Roux 21/09/2010
//Initialisation for creation of a net from a map
...
{       
  ...
  WORD_BUF_UPPER=NULL; 
  WORD_BUF_LOWER=NULL;
  net=seqs_to_net_create(two_lines_from_map_to_labels,nets,final_state, start_destinations);
  return(net);
}

void netword_add_from_map(char* upper,char* lower,
	NETptr *net,
	NVptr nets,
	STATEptr *final_state, 
	STATEptr **start_destinations,
	int *result_count,int *sd_len) {
	WORD_BUF_UPPER=upper;
	WORD_BUF_LOWER=lower;
	memset(UPPER_BUF,0,100);
	memset(LOWER_BUF,0,100);
	seqs_to_net_add_next(two_lines_from_map_to_labels,net,nets,final_state,start_destinations,result_count,sd_len);
}
...
{ 
  seqs_to_net_end(two_lines_from_map_to_labels,net,nets,final_state, start_destinations,result_count,sd_len);
  free((void *) UPPER_BUF);
  free((void *) LOWER_BUF);
  free((void *) WORD_BUF);
