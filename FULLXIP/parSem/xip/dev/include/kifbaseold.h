...
public:
	...
	Exported KifElement* EvaluateString(int param, KifDomain* domain) {
		KifElement* ret = parameters[param]->Exec(kifTypeSTRING, domain, kifNULL, kifGlobals->GetThreadid(), caller);
		if (clean && ret->reference == 0)
			toclean.push_back(ret);
		return ret;
	}
...
public:
	...
	KifElement* Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
		return this;
	}
