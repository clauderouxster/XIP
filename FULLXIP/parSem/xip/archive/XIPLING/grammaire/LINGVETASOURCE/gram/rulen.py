...
def m_token_14(fx,lreturn,tree,nb):
 ...
 # Or
 subtree={}
 if m_negation(fx,lret,subtree,nb+1) or m_string(fx,lret,subtree,nb+1) or m_point(fx,lret,subtree,nb+1) or m_interval(fx,lret,subtree,nb+1) or m_mot(fx,lret,subtree,nb+1) or m_chiffre(fx,lret,subtree,nb+1) or m_symbole(fx,lret,subtree,nb+1):
   if x_test_tree(subtree):
     tree['%NODES'].append(subtree)
 else:
   tree.pop('%NODES')
