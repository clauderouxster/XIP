...
def fillup(endingrules,pronending):
    ...
    for i in terminations.keys():
        ...
        if ending[-1] in nominalcategories:
            for se in semantics:
                basicfeatureupdate=basicfeature.replace("!"+i+"a","!"+i+se)
                enrichbasicfeature=semantics[se]+basicfeatureupdate
                formebase=i+se
                for j in cases.keys():
                    #we then build the case value with our features
                    #the ending stores our basic features
...
function ending(string mot) {
	string motmin=mot.lower();
	if (basicwords[motmin]!=null)
		return(basicwords[motmin]);
	...
	for (i=lg;i>1;i--) {
		base=mot[-1*i:];
		if  (onending[base]!=null) {
			res=onending[base];
			res["root"]=mot[:-1*i];
			println(onending[base]);
			return(res);
		}
	}
	return(null);
