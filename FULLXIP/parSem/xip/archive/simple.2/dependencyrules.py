...
def testcoordprep(depname,x,dps):
    ...
    #First we only coordinate similar nouns
    if prep1["case"]==prep2["case"] and prep1["root"]==prep2["root"]:
        #second, they must share the same parent, which is up their NP parent
        parent1=prep1["up"]["up"]
        parent2=prep2["up"]["up"]
        if parent1==parent2:                
...
def testcompnde(depname,x,dps):
    ...
    prep=pp["node"][0]
    if not "case" in prep or prep["case"]!="Gen" or prep["root"]!="de":
        return None
...
def testcomparison(depname,x,dps):
    ...
    prep1=pp1["node"][0]
    prep2=pp2["node"][0]
    if prep1["root"]!=prep2["root"]:
        return None
    if prep1["root"] not in ["mik","min","ek"]:
        return None
    
    depn=depname+"_"+prep1["root"]
...
def testrule(depname,x,dps):
    ...
    pronif=x[0]
    pronthen=x[1]
    if pronif["root"].lower()=="if" and pronthen["root"].lower()=="kom":
        return [depname,None,None]
    return None
...
def testpronverbsansant(depname,x,dps):    
    pron=x[0]
    if "antecedentcase" in pron or pron["root"]!="k":
        return None
...
def testsubjnul(depname,x,dps):
    ...
    if "MAIN" in dps:
        ...
        for l in lstmain:
            if verb==l[1]:
                key=dependencies.buildelement(verb)
                if "ARG" in dps:
                    for arg in dps["ARG"]:                        
