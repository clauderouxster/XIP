...
def buildsenses(dico,roots,language):
    ...
    for s in sensedata.senses.keys():
        ...
        for mytrans in mytranslations:
            ...
            try:
                s=grammar.parse(mysent,sentence,lexicals,dico,roots,gr,False,None,language)
                if variables!={}:
...
def gettranslationongraph(wx,node,ending,grbase,language,removed):
    if "SENSE" in node:
        return node["SENSE"]
    rt="re"+ending
    for wrd in [wx,rt]:
        if wrd in sensedata.senses:
            ...
            for mytrans in mytranslations:
                ...
                if language in mytrans:
                    ...
                    res=semantics.match(wrd,grbase,gr)       
                    if res:                        
