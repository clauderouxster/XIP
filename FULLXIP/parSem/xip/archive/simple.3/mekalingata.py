...
###Parsing the sentence
   ...
   def generationfull(self):
      ...
      for w in self.gen_text:
          try:
              ...
              if wx["pos"]=="Prep" and "antecedentcase" not in wx:
                 tradw={'translation': [w[1]], 'features': [], 'pos': 'prep',"base":w[1]}
                 tradw['features'].append(wx['case'].upper())                
                 self.pronouns[ix]=tradw
          except:
              pass
   ...
   def inferinput(self):
      ...
      for g in self.generationgraphes:
        if g["__language__"]!=None and g["__language__"]!=grammarcodes.currentlanguage:
            continue
        if semantics.match(gr,g):
            ...
            if '__equivalence__' not in g:
                ...
                listofbasicwords=[]
                wordbase=g['__basesentence__'].split()
                for w in wordbase:
                    we=ending.ending(w)
                ...
                for lbw in listofbasicwords:
                    for lw in listofwords[i:]:
                        if lbw["pos"]==lw['pos']:
                            ps=lbw['root'].find("_")
                            #We get the word in the solution sentence
                            lstequ[lbw['root'][:ps]]=wordlist[i]
                            i+=1
                            break
                        i+=1
                g['__equivalence__']=lstequ
            else:
