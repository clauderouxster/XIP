...
static char tab6[] = { '&', '+', '-', '*', '/', '%', '^', 0 };
static char tab7[] = { ';', ',', 0 };
static char tab8[] = { 'd', 'p', 'c', 'C', 'a', 'r', 's', '?', '%', 0 };
static char tab9[] = { '*', '+', 0 };
...
char bnf_kif::m_let_0_2(string& lreturn, x_node** tree) {
	...
	//BODYOR
	subtree = NULL;
	if (m_hmetafunctions(lret, &subtree) || m_wnexpressions(lret, &subtree))
		x_init_tree(tree, subtree, addsubtree);
	else {
		x_pop_node(tree, addsubtree);
		currentpos = pos;
...
char bnf_kif::m_range_0_1_3(string& lreturn, x_node** tree) {
	if (gFail == 1) return(0);
	string lret;
	long pos = currentpos;
	int itok = intoken;
...
char bnf_kif::m_wnexpressions_0_1_3_4(string& lreturn, x_node** tree) {
	if (gFail == 1) return(0);
	string lret;
	long pos = currentpos;
	int itok = intoken;
...
char bnf_kif::m_wnexpressions_0_1_3(string& lreturn, x_node** tree) {
	if (gFail == 1) return(0);
	string lret;
	long pos = currentpos;
	int itok = intoken;
	...
	//BODYOPTIONAL
	subtree = NULL;
	if (m_wnexpressions_0_1_3_4(lret, &subtree))
		x_init_tree(tree, subtree, addsubtree);
	else
		lret = "";
	return(1);
...
char bnf_kif::m_wnexpressions_0_1(string& lreturn, x_node** tree) {
	...
	//BODYSEQUENCE
	subtree = NULL;
	if (m_wnexpressions_0_1_3(lret, &subtree))
		x_init_tree(tree, subtree, addsubtree);
	else {
		x_pop_node(tree, addsubtree);
		currentpos = pos;
