...
STATEptr vget_vpu_state(StateVpuData* data_state, VirtualInfo *vinfo);
...
void index_arcs_by_label(ARCptr arcs, ARCptr *index, int side);
...
StateVpuData *get_vpu_data(STATEptr state) {
printf("GET_VPU_STATE\n");
  if (STATE_is_virtual(state)) {
    VirtualInfo *vinfo = STATE_virtual_info(state);
    assert(vinfo->methods->data_type == STATE_VPU_DATA);
  }
  return (StateVpuData *)STATE_state_data(state);
}

StateVpuData *create_vpu_data(STATEptr state, STATEptr* stateList, int listSize) {
  StateVpuData *vpu_data;
printf("CREATE_VPU_STATE\n");
  vpu_data = (StateVpuData *)calloc(sizeof(StateVpuData), 1);
  vpu_data->state = state;
  vpu_data->stateList = stateList;
  vpu_data->listSize = listSize;
  return vpu_data;
}
...
STATEptr vget_state(StateData *state_data, VirtualInfo *vinfo) {
  ...
  /* ---------------------------------------------------------- */
	switch (vinfo->methods->data_type) {
		...
		case STATE_VPU_DATA : {
			StateVpuData* vpu_data=state_data;
printf("VGET_STATE : VGET_VPU_STATE\n");
			return vget_vpu_state(vpu_data, vinfo);
		}
		default : {
			assert(0);
		}
...
printf("VGET_VPU_STATE\n");

	node = btree_insert_address(data_state, vinfo->btree, vinfo->btree_heap);
...
static STATEptr make_virtual_start_state(NETptr op1, NETptr op2, VMethods *methods, VirtualInfo *vinfo) {
	/* ------------------------------ */
	...
	/* in priority union, we must ensure that the virtual start state can't be found ! */
	if (methods->operation_id == Virtual_Priority_Union) {
		StateData *fake_data;
		STATEptr new_state;
		assert(op2);
		start2 = NET_start_state(op2);
		state_data = create_state_pair(start1, start2);
		fake_data = create_state_pair((void*)-1, (void*)-1);	/* the fake registering data for the start state */
		
		new_state=vget_state(fake_data, vinfo);

		STATE_state_data(new_state)=state_data;
		free(fake_data);
		
		return(new_state);
	
	} else {
	switch (methods->data_type) {
		case DATA_STATE : {
			assert(!op2);
			state_data = start1;
			break;
		...
		case STATE_VPU_DATA : {
			struct StateVpuData *VPUdata;
			assert(op1 && op2);
printf("MAKE_VIRTUAL_START_STATE:STATE_VPU_DATA\n");
			start2 = NET_start_state(op2);
			assert(start1 && start2);

			/* create a structure for the virtual start state */			
			VPUdata=create_vpu_data(
				start2, 			/* Register the start state of network B */
				(STATEptr*)start1,  /* Register the start state of network A */
				VPU_START_DATA		/* indicate that this data structure is for the start state */
			);
			
			/* return the structure as associated data : */
			state_data = VPUdata;
			break;
		}
		default : {
...
NETptr virtual_network(NETptr op1, NETptr op2, VMethods *methods,
  ...
  NET_is_virtual(vnetwork) = (bit)TRUE;
...
static void collect_states_from_op(NETptr vnetwork, NETptr op)
{
  ...
  if (NET_is_virtual(op))
    ...
    {
      prev = NULL;
      for (state = NET_states(op); state; state = next)
	{
	  next = next_state(state);

	  if (STATE_visit_mark(state) == IN_PROCESS)
	    {
	      /* state is accessible from vnetwork. Splice it out of
		 the operand network's state list and move it up
		 to the vnetworks list. */
		 ...
		 to free a STATE_PAIR or a STATE_LIST. */
	      ...
	      next_state(state) = NET_states(vnetwork);
		...
		NET_states(op) = next;
	      ...
	      NET_states(vnetwork) = state;
	      NET_num_states(vnetwork)++;
	      NET_num_states(op)--;

	      /* Add the states arcs to the arc count of vnet. */

	      for (arc = STATE_arc_set(state); arc; arc = next_arc(arc))
		{
		  NET_num_arcs(vnetwork)++;
		  NET_num_arcs(op)--;
		}
	    }
	  else
...
NETptr virtual_to_real_fsm(NETptr vnetwork, int minimize_p) {
 ...
 if (minimize_p) min_fsm(vnetwork, DONT_COPY);
  }
  return(vnetwork);
}
...
NETptr virtual_concat_fsm(NETptr op1, NETptr op2, int reclaim_p) {
  ...
  NET_pruned(new) = NET_pruned(op1) && NET_pruned(op2);
  NET_epsilon_free(new) = NET_epsilon_free(op1) && NET_epsilon_free(op2);
  set_virtual_net_operand_reclaimability(new, reclaim_p);
  return new;
}
...
printf("2\n");
	while (arc_list1) {
		i=ARC_label(arc_list1);
		dest1 = ARC_destination(arc_list1);
printf("%d ->\n", i);

		for (arc_list2=index2[i]; arc_list2 && (ARC_label(arc_list2) == i); arc_list2=next_arc(arc_list2)) {
...
NETptr better_virtual_intersect_fsm(NETptr op1, NETptr op2, int reclaim_p) {
  /* ------------------------------------------------------------- */
...
NETptr virtual_intersect_fsm(NETptr op1, NETptr op2, int reclaim_p) {
	...
	assert(NET_epsilon_free(op1) && NET_epsilon_free(op2));
	assert(op1 && op2);
printf("non c moi");
...
int vminus_final_p(STATEptr vstate, VirtualInfo *vinfo) {
  ...
  return (pair->state2) ? (virtual_final_p(pair->state1) && !virtual_final_p(pair->state2)) : virtual_final_p(pair->state1);
}

NETptr better_virtual_minus_fsm(NETptr op1, NETptr op2, int reclaim_p) {
  /* ------------------------------------------------------------- */
  /* A brute force definition of MINUS in terms of negation and    */ 
  /* intersection. To be replaced by the better_virtual_minus()    */
  /* below when the arc set and finality functions above have      */
  /* been defined.                                                 */
  /* ------------------------------------------------------------- */
...
NETptr virtual_minus_fsm(NETptr op1, NETptr op2, int reclaim_p) {
  ...
  new = virtual_network(op1, NET_deterministic(op2)?op2:virtual_determinize(op2, reclaim_p), methods, NULL, NULL);

  NET_deterministic(new) = NET_deterministic(op1);

  STATE_deterministic(NET_start_state(new)) =
    STATE_deterministic(NET_start_state(op1));

  NET_pruned(new) = FALSE; /*NET_pruned(op1);*/
  NET_epsilon_free(new) = NET_epsilon_free(op1);
  NET_closed_sigma(new) = NET_closed_sigma(op1);
  set_virtual_net_operand_reclaimability(new, reclaim_p);
  return new;
}
...
NETptr virtual_option_fsm(NETptr op, int reclaim_p) {
  ...
  STATEptr start;
  /* ---------------------------------------------------------------- */
  /* Derived from VIRTUALOPTIONFSM.                                   */
  /* Make a copy of op and set the start state to be final.           */
  /* Don't make a copy if the start state of op is already final.     */
  /* If reclaim_p is DO_RECLAIM, the operand will be freed when the   */
  /* resulting virtual net is freed.                                  */
  /* ---------------------------------------------------------------- */
  start = NET_start_state(op);
  if (virtual_final_p(start)) return op;
  new = virtual_copy_fsm(op, reclaim_p);
  start = NET_start_state(new);
  virtual_final_p(start);
  STATE_final(start) = TRUE;
  return new;
}
...
NETptr virtual_negate_fsm(NETptr operand, int reclaim_p) {
  ...
  NET_closed_sigma(new) = NET_closed_sigma(operand);
...
void sort_arcs(ARCptr *list, int orderfn(ARCptr, ARCptr)) {
  ARCptr *list2;
  ARCptr elt1, elt2;

  if ((*list) == NULL) return;
  
  /* do a bubble sort on the linked list */
  for (; *list; list = &((*list)->next)) {
    list2 = &((*list)->next);
    while (*list2) {
      elt1 = *list;
      elt2 = *list2;
      if (orderfn(elt1, elt2) > 0) {
	assert(orderfn(elt2, elt1) <= 0);
	/* move elt2 in front of elt1 */
	*list2 = elt2->next; /* splice elt2 out of the list */
	elt2->next = elt1; /* make elt1 follow elt2 */
	*list = elt2; /* put elt2 where elt1 was */
	list2 = &(elt1->next); /* continue the sort after elt1 */
      } else {
	/* go to next */
	list2 = &((*list2)->next);
      }
...
int arc_order(ARCptr arc1, ARCptr arc2) {
  STATEptr dest1, dest2;
  dest1 = ARC_destination(arc1);
  dest2 = ARC_destination(arc2);
  if (dead_state_p(dest1) && !dead_state_p(dest2)) return 1;
  if (dead_state_p(dest2) && !dead_state_p(dest1)) return 0;
  if (ARC_label(arc1) > ARC_label(arc2)) return 1;
  if (ARC_label(arc2) > ARC_label(arc1)) return 0;
  if ((int)arc1 > (int)arc2) return 1;
  return 0;
}

int count_arcs(STATEptr state) {
  ARCptr arc;
  int n = 0;
  for (arc = virtual_arc_set(state); arc; arc = next_arc(arc)) n++;
  return n;
}
...
ARCptr vunion_arc_set(STATEptr vstate, VirtualInfo *vinfo) {
  ...
  arc1 = arc2 = NULL;
  if (pair->state1) {
    n_arcs = count_arcs(pair->state1);
    arc1 = virtual_arc_set(pair->state1);
    sort_arcs(&arc1, arc_order);
    STATE_arc_set(pair->state1) = arc1;
    assert(count_arcs(pair->state1) == n_arcs);
  }
  if (pair->state2) {
    n_arcs = count_arcs(pair->state2);
    arc2 = virtual_arc_set(pair->state2);
    sort_arcs(&arc2, arc_order);
    STATE_arc_set(pair->state2) = arc2;
    assert(count_arcs(pair->state2) == n_arcs);
  }
  /* ----------------------------- */
  /* Merge the arc lists together. */
  /* ----------------------------- */
  while (arc1 || arc2) {
    if (arc1 && dead_state_p(ARC_destination(arc1))) {
      arc1 = next_arc(arc1);
      continue;
    }
    if (arc2 && dead_state_p(ARC_destination(arc2))) {
      arc2 = next_arc(arc2);
      continue;
    }
    dest1 = NULL;
    dest2 = NULL;
    if (arc1 && arc2 && 
	ARC_label(arc1) == ARC_label(arc2)) {
      label = ARC_label(arc1);
      dest1 = ARC_destination(arc1);
      arc1 = next_arc(arc1);
      dest2 = ARC_destination(arc2);
      arc2 = next_arc(arc2);
    } else if (arc1 && (!arc2 || ARC_label(arc1) <
			     ARC_label(arc2))) {
      label = ARC_label(arc1);
      dest1 = ARC_destination(arc1);
      arc1 = next_arc(arc1);
    } else {
      label = ARC_label(arc2);
      dest2 = ARC_destination(arc2);
      arc2 = next_arc(arc2);
    }
    pair = create_state_pair(dest1, dest2);
    dest = vget_state(pair, vinfo);
    new_arc = make_arc(label, dest, vinfo->network);
    new_arc->next = new_arcs;
    new_arcs = new_arc;
  }
  sort_arcs(&new_arcs, arc_order);
  return new_arcs;
}
...
NETptr virtual_union_fsm(NETptr op1, NETptr op2, int reclaim_p) {
  ...
  new = virtual_network(op1, op2, methods, NULL, NULL);
  NET_epsilon_free(new) = NET_epsilon_free(op1) && NET_epsilon_free(op2);
  NET_closed_sigma(new) = NET_closed_sigma(op1) && NET_closed_sigma(op2);
  set_virtual_net_operand_reclaimability(new, reclaim_p);
  return new;
}
 ...
 */
  ...
  NET_epsilon_free(new) = NET_epsilon_free(op);
  set_virtual_net_operand_reclaimability(new, reclaim_p);
  return new;
}
...
NETptr virtual_copy_fsm(NETptr operand, int reclaim_p) {
  ...
  new = virtual_network(op1, op2, methods, NULL, NULL);
  NET_epsilon_free(new) = NET_epsilon_free(operand);
  set_virtual_net_operand_reclaimability(new, reclaim_p);
...
ARCptr * create_index(int isize) {
	int index_size = sizeof(ARCptr) * (isize + 1);
	ARCptr* index = malloc(index_size);
printf("alloc %d ", index_size);
...
id_type get_arc_id(ARCptr the_arc, int side) {
	id_type id=ARC_label(the_arc);
	if (LABEL_arity(id_to_label(id)) == 2) {
		switch(side) {
			case UPPER : return id_to_upper_id(id);
			case LOWER : return id_to_lower_id(id);
			default : assert(0);
		}
	} else {
		assert(LABEL_arity(id_to_label(id)) == 1);
		return id;
	}
}


static void fetch_max_label2(id_type *max, ARCptr arc_list, int side) {
	ARCptr arc;

	for (arc = arc_list; arc; arc = next_arc(arc)) {
		id_type id=get_arc_id(arc, side);
		if (id > *max) *max = id;
...
ARCptr vpriority_union_arc_set(STATEptr vstate, VirtualInfo *vinfo) {
	StatePair *pair = get_state_pair(vstate);
	STATEptr s1 = pair->state1;
	STATEptr s2 = pair->state2;
	ARCptr scan_arc, new_arcs = NULL;

	assert(s1); /* in all cases, state1 must be non null */

printf("\nRealizing %x :[%x:%x]\n", vstate, s1, s2);
	/* if we are processing the start state or a unique state, we must add all the outgoing arcs of s1 */
	if (!s2 || ((s1 == NET_start_state(vinfo->op1)) && (s2 == NET_start_state(vinfo->op2)))) {
printf("\tcopying singles : ");
if (s2) printf("[start] ");
		for (scan_arc=virtual_arc_set(s1); scan_arc; scan_arc=next_arc(scan_arc)) {
			/* create the pair : */
			StatePair *new_pair=create_state_pair(ARC_destination(scan_arc), NULL);
			ARCptr new_arc;

			/* create the new virtual state : */
			STATEptr dest=vget_state(new_pair, vinfo);
printf("%x=%x ", ARC_destination(scan_arc), dest);
			
			/* create and insert a new arc in the list */
			new_arc = make_arc(ARC_label(scan_arc), dest, vinfo->network);
			new_arc->next = new_arcs;
			new_arcs = new_arc;
		}
	}
printf("\n");	
	if (s2) {
		/* determine on which side to perfor; the priority union : */
		int side=(int)vinfo->extra_info;	
		id_type  max_label=0;	/* the higher label ID */
	  	ARCptr *index;			/* an index on arcs list of s1*/
		
		ARCptr arc_list=virtual_arc_set(s2);

printf("\tprocessing 2 states : \n");

		/* determine the maximum label id in both state : */	
		fetch_max_label2(&max_label, STATE_arc_set(s1), side);

		fetch_max_label2(&max_label, STATE_arc_set(s2), side);


		/* create the index : */
		index=create_index(max_label);
		assert(index);
  		index_arcs_by_label(virtual_arc_set(s1), index, side);

		
		/* for each arcs from s2 */
		for (scan_arc=virtual_arc_set(s2); scan_arc; scan_arc=next_arc(scan_arc)) {
			STATEptr dest2;
			id_type id;
			ARCptr matching_arc;
			ARCptr new_arc;
			
			dest2=ARC_destination(scan_arc);
			assert(dest2);
			id=get_arc_id(scan_arc, side);
			
			assert(id<=max_label);
			matching_arc=index[id];
			
			if (matching_arc) {
printf("\tadding tuple : ");
				for( ; matching_arc; matching_arc=next_arc(matching_arc)) {
					StatePair *new_pair;
					STATEptr dest1=ARC_destination(matching_arc);
					assert(dest1);
				
					/* create the pair : */
					new_pair=create_state_pair(dest1, dest2);
printf("%x:%x=", dest1, dest2);

					/* create the new virtual state (dest2 become the new created virtual state) : */
					dest2=vget_state(new_pair, vinfo);
printf("%x ", dest2);
					
				}			
printf(" | \n");
			} else { /* arc with no equivalent in the first network */
				/* create the pair : */
				StatePair *new_pair=create_state_pair(dest2, NULL);

				/* create the new virtual state : */
printf("\tadding single %x=", dest2);
				dest2=vget_state(new_pair, vinfo);
printf(" %x \n", dest2);
			...
			/* create and insert a new arc in the list */
			new_arc = make_arc(ARC_label(scan_arc), dest2, vinfo->network);
			new_arc->next = new_arcs;
			new_arcs = new_arc;
		}
	}
...
int vpriority_union_final_p(STATEptr vstate, VirtualInfo *vinfo) {
	...
	if (s2) { /* this is a pair */
		/* the empty word must be recognized even both operands recognize it */
		if ((s1 == NET_start_state(vinfo->op1)) && (s2 == NET_start_state(vinfo->op2))) {
			return STATE_final(s1) || STATE_final(s2);
		} else {
			/* must be final in B but not in A */
			return (STATE_final(s2) && (!STATE_final(s1)));
		}
	} else {		/* this is a single state */
		return STATE_final(s1);
	}
...
NETptr virtual_priority_union_common(NETptr op1, NETptr op2,int side,int reclaim_p) {
	NETptr new;
	VMethods *methods =  get_virtual_methods(Virtual_Priority_Union);

printf("vpriority\n");
	assert(op1 && op2);
	/* the two net must be epsilon-free */
	assert(NET_epsilon_free(op1) && NET_epsilon_free(op2));

	/* Get the side to perform the union on for the vpriority_union_arc_set function : */
	
	new = virtual_network(op1, op2, methods, (void*)side, NULL);

	NET_deterministic(new) = FALSE;

	STATE_deterministic(NET_start_state(new)) = FALSE;

	NET_pruned(new) = FALSE;
	NET_epsilon_free(new) = TRUE;
	set_virtual_net_operand_reclaimability(new, reclaim_p);

printf("vpriority_end\n");
	return new;
}
...
NETptr virtual_priority_union(NETptr op1, NETptr op2,int reclaim_p) {
	return virtual_priority_union_common(op1, op2, UPPER, reclaim_p);
}
...
ARCptr vcompose_arc_set(STATEptr state, VirtualInfo *vinfo) {
  ...
  STATEptr final1 = (STATEptr)vinfo->extra_info;
  ...
  if (up_has_non_eps) {
    ...
    for (low_arc = index[EPSILON]; low_arc; 
	 low_arc = next_arc(low_arc)) {
      ...
      if (uofl != EPSILON) break;
      new_arcs = vcompose_arc(EPSILON, lofl,
			      final1, ARC_destination(low_arc), 0,
			      new_arcs, vinfo, index2, index2_size);
    }
  }
  update_match_tables();
...
NETptr virtual_compose_fst(NETptr op1, NETptr op2, int reclaim_p) {
  ...
  /* ----------------------------------------------------- */
  NETptr new;
  STATEptr final1 = NULL;
  VMethods *methods = get_virtual_methods(Virtual_Compose);

  /* ---------------------------------------------------- */
  /* Look for an empty final state for each op.           */
  /* If there is none, then create one.                   */
  /* These are stored in extra_data for vcompose_arc_set. */
  /* ---------------------------------------------------- */
  if (!NET_is_virtual(op1)) {
    for (final1 = NET_states(op1); final1; final1 = next_state(final1)) {
      if (STATE_final(final1) && !STATE_arc_set(final1)) break;
    }
  }
  if (!final1) {
    /* - - - - - - - - - - - - - - - - - - - */
    /* Should storage should be in new?      */
    /* - - - - - - - - - - - - - - - - - - - */
    final1 = make_state(NULL, op1);
    STATE_deterministic(final1) = FALSE;
    STATE_final(final1) = TRUE;
  }
  new = virtual_network(op1, op2, methods, final1, NULL);
  set_virtual_net_operand_reclaimability(new, reclaim_p);
  return new;
}
...
static VMethods *get_virtual_methods(int operation_id)
{
  ...
  switch(operation_id)
	...
	case Virtual_Priority_Union:
		...
		return(vpriority_union_methods);
		break;
