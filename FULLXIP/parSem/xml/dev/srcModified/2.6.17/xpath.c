...
/************************************************************************
 *                                                                      *
 *    C.R. functions added to improve XPath indexing                    *
 *                                                                      *
 *************************************************************************/

void AddValueNodeWithIndex(xmlNodePtr node,int index) {
    xpathvaluePtr valxpath;
    if (node==NULL || node->_private!=NULL)
        return;
    valxpath=(xpathvaluePtr)xmlMalloc(sizeof(xpathvalue));
    valxpath->str=(xmlChar*)node->name;
    valxpath->index=index;
    node->_private=valxpath;
}

char AddValueAttributeWithIndex(xmlXPathParserContextPtr ctxt,
	       	xmlXPathObjectPtr arg, xmlNodePtr cur, int index) {
   xmlNodeSetPtr ns;
   xmlNodePtr node;
   xpathvaluePtr valxpath;
   if ((arg == NULL) ||
    ...
    if (node->_private!=NULL)
        return(0);
     //We get the str value of that attribute
     valxpath=(xpathvaluePtr)xmlMalloc(sizeof(xpathvalue));
     valxpath->str=node->children->content;
     valxpath->index=index;
     node->_private=valxpath;
     return 1;
}
...
/* ------------------------------------------End added by C.R ------------------------------- */
	...
	switch (arg2->type) {
	    ...
	    case XPATH_STRING:
/* ------------------------------------------Begin added by C.R ------------------------------- */
		//C.R. We introduce a modification, il the value is "%n" with n being any number
		//In that case, the comparison is always true and we save the node value in a specific
		//table of tables. "n" is the position of the value.
		if (arg2->stringval[0]=='_' && (arg2->stringval[1]>='0' && arg2->stringval[1]<='9') ){
		        //We compute the index value...
		        index=arg2->stringval[1]-48;
		        ret=AddValueAttributeWithIndex(ctxt,arg1,ctxt->context->node,index);
		        break;
		}
/* ------------------------------------------End added by C.R ------------------------------- */
			   ...
			   xmlNodePtr * first, xmlNodePtr * last)
{
    ...
    xmlNodePtr tmp;
/* ------------------------------------------Begin added by C.R ------------------------------- */
    xmlChar* mothernode=ctxt->context->mothernode;
/* ------------------------------------------End added by C.R ------------------------------- */
...
/* ------------------------------------------Begin added by C.R ------------------------------- */
	 //Added by C.R. We create a specific entree for a node Variable
	if (mothernode!=NULL &&
	    mothernode[0]=='_' &&
	    mothernode[1]>='0' &&
	    mothernode[1]<='9' &&
	    list->nodeNr) {
	    nodelist->nodeTab[i]->_private=(void*)mothernode[1];
 	    AddValueNodeWithIndex(nodelist->nodeTab[i],mothernode[1]-48);
	}
/* ------------------------------------------End added by C.R ------------------------------- */
        /*
