...
BEGIN_MESSAGE_MAP(CKIFFApp, CWinAppEx)
	...
	ON_COMMAND(ID_FICHIER_SETKIFLIBS, &CKIFFApp::OnFichierSetkiflibs)
END_MESSAGE_MAP()
...
CKIFFApp::CKIFFApp()
{
	...
	SetAppID(_T("KIFF.AppID.NoVersion"));
...
BOOL CKIFFApp::InitInstance()
{
	...
	//HKEY hKey = OpenKey(HKEY_LOCAL_MACHINE,L"SOFTWARE\\Classes\\CLSID\\filtergraph\\InprocServer32");
	//SetValKey(hKey,L"ThreadingModel",L"Free");
	//RegCloseKey(hKey);
	InitKIFLIBS();
	return TRUE;
}
...
void CKIFFApp::OnFichierSetkiflibs()
{
