...
BEGIN_MESSAGE_MAP(CKIFFView, CRichEditView)
	...
	ON_COMMAND(ID_KIF_COMMENTER, &CKIFFView::OnKifCommenter)
	...
	ON_COMMAND(ID_EDITION_ARGUMENTS, &CKIFFView::OnEditionArguments)
	...
	ON_MESSAGE(ID_OUVRIR_DOCUMENT, &CKIFFView::ouvrir)
	ON_MESSAGE(IDS_AFFICHAGE_COURANT,&CKIFFView::AfficheCourant)
	...
	ON_COMMAND(ID_EDITION_ABANDONNE, &CKIFFView::OnEditionAbandonne)
	ON_UPDATE_COMMAND_UI(ID_EDIT_UNDO, &CKIFFView::OnUpdateEditUndo)
	ON_UPDATE_COMMAND_UI(ID_EDIT_REDO, &CKIFFView::OnUpdateEditRedo)
	ON_COMMAND(ID_KIF_SANSINTERUPTION, &CKIFFView::OnKifSansinteruption)
	ON_UPDATE_COMMAND_UI(ID_KIF_SANSINTERUPTION, &CKIFFView::OnUpdateKifSansinteruption)
	...
	ON_COMMAND(ID_EDITION_SUGGESTION, &CKIFFView::OnEditionSuggestion)
	ON_UPDATE_COMMAND_UI(ID_EDITION_SUGGESTION, &CKIFFView::OnUpdateEditionSuggestion)
	ON_COMMAND(ID_EDITION_TAILLEINDENTATION, &CKIFFView::OnEditionTailleindentation)
END_MESSAGE_MAP()
...
LRESULT CKIFFView::SUGGERE(WPARAM wParam, LPARAM lParam) {
	...
	pCurseur = e.PosFromChar(d);
	ClientToScreen(&pCurseur);
...
void CKIFFView::Choixligne() {
	if (firstview==this)
		return;
	CKIFFView* vuecourante=Lavue(kifGlobals->Currentfilename());
	if (vuecourante==NULL) {
		firstview->currentname=kifGlobals->debuginfocurrent->currentfile;
		::SendMessage(firstview->GetSafeHwnd(),ID_OUVRIR_DOCUMENT,0,0);
		vuecourante=Lavue(kifGlobals->Currentfilename());	
		if (vuecourante==NULL)
			return;
	}
...
LRESULT CKIFFView::ouvrir(WPARAM wParam, LPARAM lParam) {	
	wstring code;
	s_utf8_to_unicode(code,STR(currentname));
	if (code.size()==0)
		return 0;
	CKIFFDoc* doc=(CKIFFDoc*)theApp.OpenDocumentFile((WCHAR*)code.c_str());		
	return 1;
}
...
void CKIFFView::runkif(CString& s) {	
	...
	if (s!="") {
		...
		s_unicode_to_utf8(args,baseargs);
...
void CKIFFView::OnKifSansinteruption()
{
	// TODO: Add your command handler code here
	if (!closekifsession())
		return;
				...
				if (exec) {
					if (!lexchange.encours) {
						...
						if (accumulecode!="") {
							...
							nbsquare=0;
							accumulecode=_T("");
							DisplayValue("Exécution en cours. Je ne peux exécuter votre instruction\r",NULL);
						}
					}
				}
			}
...
long CKIFFView::FirstChar(long d,long f,CString& commande) {
	...
	lines.push_back("@;");
	string codeindente;
	IndentationCode(codeindente,lines,blancs,0,false,"sinon","sinonsi","si");
	return blancs.back();
}
...
void CKIFFView::OnEditionArguments()
{
	// TODO: Add your command handler code here
	Arguments argums;
...
void CKIFFView::OnEditionAbandonne()
{
	// TODO: Add your command handler code here
	nbcurly=0;
	nbsquare=0;
...
void CKIFFView::OnUpdateKifSansinteruption(CCmdUI *pCmdUI)
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(1-lexchange.encours);
}
...
void CKIFFView::OnEditionSuggestion()
{
	// TODO: Add your command handler code here
...
void CKIFFView::OnUpdateEditionSuggestion(CCmdUI *pCmdUI)
{
	// TODO: Add your command update UI handler code here
...
void CKIFFView::OnEditionTailleindentation()
{
	// TODO: Add your command handler code here
	wkiflibs argums;
	int l;
	...
	argums.kiflibspath = val.c_str();
	if (argums.DoModal() == IDOK) {
		l = wcstol(LPCWSTR(argums.kiflibspath), NULL, 10);
		SetBlankSize(l);
		SetKeyVal(L"SZBLANKS", argums.kiflibspath);
	}
}
