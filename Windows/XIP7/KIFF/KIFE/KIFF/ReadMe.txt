...
L'Assistant Application a créé cette application KIFF pour 
...
KIFFDoc.h, KIFFDoc.cpp - le document
    Ces fichiers contiennent votre classe CKIFFDoc.  Modifiez ces 
fichiers pour 
    ajouter les données de document spéciales et implémenter l'enregistrement 
    et le chargement des fichiers (via CKIFFDoc::Serialize).
    Le document contiendra les chaînes suivantes :
        Extension de fichier :         kif
        ID du type de fichier :        KIFF.Document
        Titre du frame principal :     KIFF
        Nom du type de document :      KIFF
        Nom de filtre :                Fichiers KiF (*.kif)
        Nom court de nouveau fichier : KIFF
        Nom long du type de fichier :  KIFF.Document

KIFFView.h, KIFFView.cpp - la vue du document
    Ces fichiers contiennent votre classe CKIFFView.
    Les objets CKIFFView servent à afficher les objets 
    CKIFFDoc.

res\KIFFDoc.ico
    Il s'agit d'un fichier icône, qui est utilisé comme icône des fenêtres 
    enfants MDI pour la classe CKIFFDoc.  Cette icône est incluse 
    par le fichier de ressources principal KIFF.rc.
...
StdAfx.h, StdAfx.cpp
    ...
    (PCH) nommé KIFF.pch et un fichier de type précompilé 
    nommé Stdafx.obj.
