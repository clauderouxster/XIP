...
template <class Z> class VECTA {
 public:
    ...
    void nulle(Z v) {
     ...
     inline int ajoute(Z val,int inc=3) {
     ...
     inline int push_back(Z val,int inc=3) {
     ...
     inline Z operator [](unsigned int i) {
         if (vecteur==NULL || i >= (unsigned int)dernier)
             return zero;
     ...
     inline Z insertion(int pos,Z val) {
         ...
         //si l'element est queue, on l'ajoute a sa position
         if (pos>=dernier) {
             affecte(pos,val);
             return vecteur[pos];
         }
