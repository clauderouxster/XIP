...
bool addEntrytoOutlook(string subject, string body, string location, time_t tstart, time_t tend) {
	...
	wstring wbody;
	wstring wlocation;
	s_utf8tounicode(wsubject, STR(subject));
	s_utf8tounicode(wbody, STR(body));
	s_utf8tounicode(wlocation, STR(location));
	...
	////MAPIFolderPtr calendar = pMAPI->GetDefaultFolder(olFolderCalendar);

	//_ItemsPtr mesitems = calendar->GetItems();
	

	//_AppointmentItemPtr rdv = mesitems->Add(olAppointmentItem);
...
public:
	...
	KifElement* MethodisDirectory(KifElement* contextualpattern,KifDomain* domain,KifCallFunction* callfunc,int idthread) {
		string dirName_in=callfunc->Evaluate(0,domain)->String();
#ifdef WIN32
	...
	KifElement* MethodEnv(KifElement* contextualpattern,KifDomain* domain,KifCallFunction* callfunc,int idthread) {
        ...
        if (callfunc->Size()==1) {
            ...
            KifElement* kvalue=callfunc->Evaluate(1,domain);
            if (lastcommand!="") {
	...
	KifElement* MethodCreateDirectory(KifElement* contextualpattern,KifDomain* domain,KifCallFunction* callfunc,int idthread) {
		...
		KifElement* kpath=callfunc->Evaluate(0,domain);
	...
	KifElement* MethodRendezvous(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		addEntrytoOutlook(subject, body, location, ((KifDate*)kstart)->value, ((KifDate*)kend)->value);
		return kifTRUE;
	}
#endif
	//---------------------------------------------------------------------------------------------------------------------
...
extern "C" {
	Exported bool Init_kifsys(KifCode* kifcode,string version) {
	...
	Exported bool kifsys_KifInitialisationModule(KifCode* kifcode,string version) {
		...
		MethodInitialization("command",&KifSys::MethodCommand,P_ONE,"command(string com): execute the command 'com'");
...
#ifdef WIN32
		MethodInitialization("rendezvous", &KifSys::MethodRendezvous, P_FIVE, "rendezvous(string subject,string body, string location, date start,date end): add an appointement in Outlook");
#endif
	...
	Exported bool kifsys_Terminate(KifCode* kifcode) {
