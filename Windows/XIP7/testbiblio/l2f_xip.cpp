...
public:
    ...
    void loading(char* path, VECTA<char*>& fileparam, Options const& options);
...
void separe(char* reading_string) {
	char* pt;
    ...
    char* start = reading_string;
	//Specific case: The first character is a pipe...
    int counter = 0;
	if (start[0]=='|') {
		//we read all the pipe until we find a non pipe
		int p=0;
...
void GlobalParseurTry::SentenceAnalysis(char renumerote, char arbre) {
    ...
    while(loop) {
        ...
        if(testMarkupTags == NON) {
            //std::cerr<< buffer << std::endl;                 // usefull if debugging
...
//	int i=XipGrmFile(xip_spec);
//	GlobalParseur* parser=gVecteurParseurs[i];
...
//	parser->ParametresAffichage |=AFFICHAGE_SURFACE | AFFICHAGE_REDUIT | AFFICHAGE_ARBRE_CHUNK | ARBRE_AFFICHE;
//
...
//// The following is an alternative to get the results of "printrules"
//    XipFreeAllGrammars();
//	 DetruitEcran();
//
//    return 0;
...
//We traverse the tree TOP DOWN... See xipstlres.h for a description of a XipNode 
//and of a XipLeaf...
void displayxipnode(XipNode* xip_node) {	
	if (xip_node->Type()==XIPNODE)
		cout<<"NODE:"<<xip_node->category;
	else
		cout<<" LEAF:"<<((XipLeaf*)xip_node)->lemma.c_str();
	cout<<" "<<xip_node->left<<":"<<xip_node->right<<endl;
	for (int i=0;i<xip_node->daughters.size();i++)
		displayxipnode(xip_node->daughters[i]);
}

int main(int argc,char* argv[]) {
	int ipar=XipGrmFile("R:\\XIPCVS\\parSem\\xip\\dev\\GRAMMARS\\ENGLISH\\GRMFILES\\GRAM_GEN\\gram_gen_entit.grm");
	XipSetDisplayMode(ipar, DISPLAY_OFFSET|DISPLAY_SENTENCE|DISPLAY_LEMMA|DISPLAY_CHUNK_TREE);

	ostringstream os;
	XipParseStringOS("The lady is happy.",0,&os);
	cout<<(char*)os.str().c_str()<<endl;
	int i;
	//We create a XipResult object, in which results will be stored
	XipResult* xip=XipCreateCurrentXipResult(ipar);

	//We then apply XipParseString to fill in this object
	XipParseString("User Manuals and Videos Help",ipar, xip,1);
	//we display the chunk tree of the root. As the input might contain more than one sentence at a time
	//we loop among these different outputs...
	for (i=0;i<xip->sentences.size();i++)
		displayxipnode(xip->sentences[i]->root);
	cout<<endl;
	//We clean this object in order to start with a new context...
	XipCleanCurrentXipResult(ipar);

	//And so on
	XipParseString("Cleaning",ipar, xip,1);
	for (i=0;i<xip->sentences.size();i++)
		displayxipnode(xip->sentences[i]->root);
	cout<<endl;
	XipCleanCurrentXipResult(ipar);

	XipParseString("application",ipar, xip,1);
	for (i=0;i<xip->sentences.size();i++)
		displayxipnode(xip->sentences[i]->root);

	//Finally we destroy this XipResult object...
	XipDeleteCurrentXipResult(ipar);
	cout<<endl;
}

	//ostringstream os2;
	//XipParseStringOS("Cleaning",ipar, &os2,XML_NONE,1);
	//XipParseStringOS("Troubleshooting Pages Help",ipar, &os2,XML_NONE,1);
	//XipParseStringOS("User Manuals and Videos Help",ipar, &os2,XML_NONE,1);
	//XipParseStringOS("application",ipar, &os2,XML_NONE,1);
	//XipParseStringOS("Cleaning",ipar, &os2,XML_NONE,1);
	//cout<<"Res:"<<(char*)os2.str().c_str()<<endl;
