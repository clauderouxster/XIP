/* $Id: ntmlevels.c,v 1.3 2007/02/16 16:51:39 ait Exp $ */ 
...
inline id_type readInputCharId(NtmContext* ct, Levels* lev, long pos);
#include "ntmstdinputobject.h"
...
void initLevels(Levels* lev) {
    ...
    memset(lev->inputIdVect,0,sizeof(id_type)*(NTM_ID_SIZE));
...
inline int endOfInput(NtmContext* ct, Levels* lev, long pos) {
    return (readInputCharId(ct, lev, pos) == ID_EOT);
}
...
*/
int getInputId_Check(NtmContext *ct, Levels* lev, long pos, id_type *pId) {
    int return_code=0;
    ...
    if (pos > lev->lastInputPos) {
        ...
        if ( EOF == return_code) {
            ...
            lev->inputLevel[lev->inputEnd] = ID_EOT;
            errorHandler(ct, "The input data contains an illegal UTF-8 character at position: %d", pos);
            return ID_EOT;
        } else { 
...
inline id_type readInputCharId(NtmContext* ct, Levels* lev, long pos) {
	id_type id;
	getInputId_Check(ct, lev, pos, &id);
	return id;
}
...
inline FsmNTMExport NormElement *readNormElement(NtmContext *ct, long pos, NETptr norm_net) {
    ...
    if (pos > lev->lastNormalizedPos) {
        if (readInputCharId(ct, lev, pos) == ID_EOT) {
