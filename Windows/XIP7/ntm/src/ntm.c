/* $Id: ntm.c,v 1.8 2009/01/26 10:34:36 ait Exp $ */
...
void ntmCopyright() {
    ...
    Created by Salah A�t-Mokhtar, Xerox Research Centre Europe\n\n",
    NTM_VERSION_STRING);
    
}

char *ntmWhoami() {
    sprintf(ntmWhoamiString, "NTM %s", NTM_VERSION_STRING);
    return ntmWhoamiString;
}
 ...
 position pointing on the first non space
    ...
    inline void skipSpaces(NtmContext* ct) {
        ...
        while ( ( c != ID_EOT) && isSpace(ct, c) ) {
            ...
            c = readInputCharId(ct, &(ct->levels), ct->inputPos);
        }
    }
    ...
    inline void ntmSetSeparator(NtmContext* ct, int char_code) {
        id_type id;
        /* intern_char(&id, char_code ); */
        getInternalCharId(&ct->levels, char_code, &id);
        if (id != ID_EOT) {
            ct->separators[id] = 1;
        }
    }
   ...
   strategies from a script file and uses them to initialize ct
    ...
    /* Checks if s (considered as a SINGLE token) is recognized using the
  strategies in ct. If so its NtmToken struct is returned
  Otherwise (s, as a single token, is not in the lexicons) the
  Unknown token is returned.
  s is an ISO_LATIN1 input string
     **************************************************************************/
    
    NtmToken* ntmAnalyzeThisToken(NtmContext* ct, unsigned char* s) {
        long initpos = 0, strategy_ind=0;
        NtmToken *token=NULL, *t=NULL;
        
        ntmSetPrintByteIndices(ct, 0);
        ntmSetInputTextString(ct, s);
        
        token = ntmReadTokens(ct);
        if ( !ntmError(ct) ) {
            if (token->inputLength == strlen((char*) s) ) {
                return token;
            } else {
                ntmSetInputTextString(ct, s);
                return ( getUnknownToken(ct) );
            }
        } else {
            return NULL;
        }
    ...
    /* Same as ntmAnalyzeThisToken above, except that it takes UTF8 input strings
     **************************************************************************/
    
    
    NtmToken* ntmAnalyzeThisTokenUTF8(NtmContext* ct, unsigned char* s) {
        long initpos = 0, strategy_ind=0;
        NtmToken *token=NULL, *t=NULL;
        
        ntmSetPrintByteIndices(ct, 1);
        ntmSetInputUTF8String(ct, s);
        
        token = ntmReadTokens(ct);
        if ( !ntmError(ct) ) {
            if (token->inputByteLength == strlen((char*) s) ) {
                return token;
            } else {
                ntmSetInputUTF8String(ct, s);
                return ( getUnknownToken(ct) );
