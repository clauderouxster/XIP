...
public:
	...
	static map<string,FrameTemplateMethod> linkedmethods;
...
//---------------------------------------------------------------------------------------------------------------------
	KifFrameTemplate(KifCode* kcode,KifElement* base,string& name) : KifFrame(kcode,base,name) {
		...
		Newfield("i",new KifInteger(kcode,NULL,0));		
		Newfield("s",new KifString(kcode,NULL,"ici"));
	}
...
//---------------------------------------------------------------------------------------------------------------------
	...
	KifElement* ExecuteMethod(string name,KifElement* contextualpattern,KifDomain* domain,KifCallFunction* callfunc) {
...
map<string,KifCallMethod> KifFrameTemplate::kifexportedmethods;
map<string,FrameTemplateMethod>  KifFrameTemplate::linkedmethods;
...
static KifElement* CreatekifFrameTemplateElement(KifCode* kifcode,x_node* xn,KifElement* base) {
	...
	if (localDefinition==NULL)
		localDefinition=new KifFrameTemplate(kifcode,NULL,name);
...
static void MethodInitialization(string name,FrameTemplateMethod func) {
	//A KifCallMethod object is used by KiF to call the ExecuteMethod on our object
	KifCallMethod kcm(name);
	KifFrameTemplate::kifexportedmethods[name]=kcm;
	KifFrameTemplate::linkedmethods[name]=func;
}

//we export this function as a C one to avoid fumbling with C++ mangling
extern "C" {
	Exported bool KifInitialisationModule(KifCode* kifcode,string version) {
		...
		FrameTemplate_type=kifcode->KifAddNewType("tframe",CreatekifFrameTemplateElement);
		...
		MethodInitialization("sample",&KifFrameTemplate::MethodSample);
