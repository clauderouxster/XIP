...
KifElement* PreMode(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if ((callfunc->kifcode->checkprobabilities & PRED_WEIGHTSTACK) == PRED_WEIGHTSTACK) {
		if (callfunc->Size() > 1) {
			...
			if (pred != kifNULL)
				return callfunc->kifcode->Returnerror(KIFPREDICATE[0]);
			if (callfunc->Size() == 3) {
...
Exported KifElement* KifPredicate::Map(KifElement* dom, KifMap* m) {
	...
	local.kifcode = kifcode;
	local.value = KifIdString(name);
	kmap->Push((char*)KIFPREDICATE[9], &local);
	char buff[] = { '0', 0, 0 };
	int pos = 0;
	KifElement* e;
	for (int i = 0; i < parameters.size(); i++) {
...
KifElement* KifElement::Predicate(KifDomain* dom) {
	...
	if (aMapContainer()) {
		e = ValueOnKey(KIFPREDICATE[9]);
		if (e == kifNOELEMENT)
			return kifcode->Returnerror(KIFPREDICATE[10]);
		name = e->String();
		...
		for (int i = 0; i < sz; i++) {
			...
			e = ValueOnKey(buff);
			if (e == kifNOELEMENT)
				return kifcode->Returnerror(KIFPREDICATE[11]);
			pred->parameters.push_back(e->Atom(true));
			buff[pos]++;
		}
	...
	return kifcode->Returnerror(KIFPREDICATE[12]);
}
...
KifPredicate* KifInstructionEvaluate::PredicateUnification(VECTA<KifElement*>& stack, int& posreplace, int& from) {
	...
	for (int i = 0; i < stack.size(); i++) {
		...
		if (e->type == kifPredicate) {
			...
			if (e == kifCUT)
				test = false;
			else //unifiedDeclaration tests whether the current predicate is fully KIFPREDICATE[13]: all variables have been unified...
...
KifElement* KifInstructionEvaluate::PredicateCreate(KifPredicate* headrule, int depth) {
	...
	if (trace) {
		...
		if (pv->weight->Float() != 1)
			s << KIFPREDICATE[14] << depth << "(w=" << pv->weight->Float() << ")=" << pv->String() << endl;
		else
			s << KIFPREDICATE[14] << depth << "=" << pv->String() << endl;
		PreLocalPrint(s.str());
	}
...
KifElement* KifInstructionEvaluate::PredicateEvalue(VECTA<KifElement*>& stack, KifPredicate* currenthead, int depth) {
	...
	if (rules.find(headpredicate->name) == rules.end()) {
		if (headpredicate->Negation()) {
			if (res == kifFALSE) {
				...
				return PredicateEvalue(Oo->localstack, currenthead, depth + 1);
			}
			return kifFALSE;
		}
	...
	if (headpredicate->Negation()) {
		...
		for (i = 0; i < sz; i++) {
			...
			if (rulebase[i]->head->parameters.size() != headpredicate->parameters.size())
				continue;
			...
			localres = rulebase[i]->Evalue(this, headpredicate, Oo->localstack, posreplace - from, depth, true);
...
Exported KifElement* KifPredicateRule::Evalue(KifInstructionEvaluate* contextualpattern, KifPredicate* currenthead, VECTA<KifElement*>& stack, int pos, int depth, bool cut) {
	...
	if (newhead == NULL) {
		for (it = dom->declarations.begin(); it.table != NULL; it++) {
			if (!localdom.check(it->first) || localdom[it->first] != it->second) {
	...
	//Oloc->toclean.push_back(newhead);
	...
	if (instructions.size() != 0) {
		...
		for (j = 0; j < instructions.size(); j++) {
			ee = (KifPredicateRuleItem*)instructions[j];
			if (ee->item->type == kifPredicate) {
				//In that case, it is worth noting that the object IS NOT STORED in the garbage collector
				//The same applies to kpredi below... We will then be able to destroy them in one single pass
				...
				if (kpred->name == idfail)
					...
					rulestack.push_back(kpred);
				}
			}
			else
				rulestack.push_back(new KifPredicateLocalInstruction(kifcode, contextualpattern->dico, ee->item, ee->negation, ee->disjunction));
		}

		if (res == kifTRUE) {
			for (j = rulestack.size() - 1; j >= 0; j--)
				stack.insert(pos, rulestack[j]);
		}
	}
	...
	if (res == kifTRUE) {
		...
		res = contextualpattern->PredicateEvalue(stack, newhead, depth + 1);
		if ((kifcode->checkprobabilities & PRED_WEIGHTSTACK) == PRED_WEIGHTSTACK) {
			//if the weight has been changed in the final weight stack function, then we modify 
			//the rule weight...
...
KifElement* KifPredicateDatabase::Exec(KifElement* contextualpattern, KifDomain* dom, KifElement* val, int idthread, KifCallFunction* callfunc) {
	if (database->type != kifGlobals->kifTypeString["sqlite"]) {
		...
		if (database->type != kifGlobals->kifTypeString["sqlite"])
			return kifcode->Returnerror(KIFPREDICATE[16]);
	}
