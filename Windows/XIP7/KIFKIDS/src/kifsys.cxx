...
public:
	...
	KifElement* MethodCommand(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (lastcommand != "") {
			if (filename != "") {
				...
				FILE* stream = fopen(STR(filename), KIFSYSSTRINGS[0]);
				if (stream == NULL)
					return kifcode->Returnerror(KIFSYSSTRINGS[1]);
				...
				fclose(stream);
				DUP2(o, 1);
				stream = fopen(STR(filename), KIFSYSSTRINGS[2]);
				string content;
				char buffer[2049];
				size_t nb;
				while (!feof(stream)) {
	...
	KifElement* MethodFileInfo(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (stcible >= 0) {
			...
			mp->Push((char*)KIFSYSSTRINGS[3], size);
			mp->Push((char*)KIFSYSSTRINGS[4], change);
			mp->Push((char*)KIFSYSSTRINGS[5], adate);
			mp->Push((char*)KIFSYSSTRINGS[6], cdate);
			mp->Push((char*)KIFSYSSTRINGS[7], kpath);
			if ((scible.st_mode & S_IFMT) == S_IFDIR)
				mp->Push((char*)KIFSYSSTRINGS[8], kifTRUE);
			else
				mp->Push((char*)KIFSYSSTRINGS[8], kifFALSE);
			return mp;
		}
		return kifNULL;
	}
	...
	KifElement* ExecMethod(short name, KifElement* contextualpattern, KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		//This call is a bit cryptic. It takes the method (function) pointer that has been associated in our map with KIFSYSSTRINGS[9]
...
extern "C" {
	...
	Exporting bool kifsys_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		MethodInitialization(KIFSYSSTRINGS[12], &KifSys::MethodCommand, P_ONE | P_TWO, KIFSYSSTRINGS[13]);
		MethodInitialization(KIFSYSSTRINGS[14], &KifSys::MethodisDirectory, P_ONE, KIFSYSSTRINGS[15]);
		MethodInitialization(KIFSYSSTRINGS[16], &KifSys::MethodCreateDirectory, P_ONE, KIFSYSSTRINGS[17]);
		MethodInitialization(KIFSYSSTRINGS[18], &KifSys::MethodCreateDirectory, P_ONE, KIFSYSSTRINGS[19]);
		MethodInitialization(KIFSYSSTRINGS[20], &KifSys::MethodListDirectory, P_ONE, KIFSYSSTRINGS[21]);
		MethodInitialization(KIFSYSSTRINGS[22], &KifSys::MethodListDirectory, P_ONE, KIFSYSSTRINGS[23]);
		MethodInitialization(KIFSYSSTRINGS[24], &KifSys::MethodRealPath, P_ONE, KIFSYSSTRINGS[25]);
		MethodInitialization(KIFSYSSTRINGS[26], &KifSys::MethodFileInfo, P_ONE, KIFSYSSTRINGS[27]);
		MethodInitialization(KIFSYSSTRINGS[28], &KifSys::MethodEnv, P_ONE | P_TWO, KIFSYSSTRINGS[29]);
