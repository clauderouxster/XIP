...
void KifAutomaton::dump(string& path) {
	...
	if (dumping.fail()) {
		kifcode->Returnerror(KIFAUTOMATON[0]);
		return;
	}
...
KifElement* KifAutomaton::setpath(KifMap* m) {
	...
	for (it = m->values.begin(); it != m->values.end(); it++) {
		if (it->second->aMapContainer() == false) {
			paths.clear();
			return kifcode->Returnerror(KIFAUTOMATON[1]);
		}
		akey = it->first;
		key = akey.code(0);
		if (it->second->type == kifMapStringInteger) {
			...
			if (it->second->type == kifMapStringFloat) {
				...
				if (it->second->type == kifMap) {
					...
					paths.clear();
					return kifcode->Returnerror(KIFAUTOMATON[2]);
				}
			}
		}
	}
...
bool KifAutomaton::load(string& path, string& featchar) {
	KifContainerLock _lock(this);
	char buff[2048];
	FILE* f = fopen(STR(path), KIFAUTOMATON[3]);
	if (f == NULL) {
		kifcode->Returnerror(KIFAUTOMATON[4]);
		return false;
	}
	pathname = path;
	agnostring s;
	if (featchar.size() > 1) {
		kifcode->Returnerror(KIFAUTOMATON[5]);
		return false;
	}
	if (featchar.size() == 1)
...
bool KifAutomaton::loaddouble(string& path, string& featchar) {
	...
	while (!feof(f)) {
		...
		if (feof(f)) {
			fclose(f);
			if (s.size()) {
				kifcode->Returnerror(KIFAUTOMATON[6]);
				return false;
			}
			return true;
		}
		...
		if (!s.size() || !lf.size()) {
			...
			if (!s.size() && !lf.size())
				return true;
			kifcode->Returnerror(KIFAUTOMATON[7]);
			return false;
		}
		addWorddouble(s, lf);
	}
...
KifElement* KifAutomaton::setcodefeat(string& c) {
	if (codefeat == 0) {
		if (c.size() != 1)
			return kifcode->Returnerror(KIFAUTOMATON[8]);
		c_utf8_to_unicode(USTR(c), codefeat);
		return kifTRUE;
	}
	return kifcode->Returnerror(KIFAUTOMATON[9]);
}
...
KifElement* KifAutomaton::evaluate(KifElement* contextualpattern, KifDomain* dom, KifElement* e, KifElement* o) {
	KifContainerLock _lock(this);
	if (function == NULL)
		return kifcode->Returnerror(KIFAUTOMATON[10]);
...
bool kifstate::evaluate(KifDomain* d, KifAutomaton& a, KifCallFunctionGeneral& kfunc,
	...
	if (kmap->type != kifMapStringFloat && kmap->type != kifVector) {
		d->kifcode->Returnerror(KIFAUTOMATON[11]);
		kmap->Release();
		return false;
	}
...
Exported KifElement* KifAutomaton::Store(KifElement* kpath, KifDomain* dom) {
	if (kpath == NULL)
		return kifcode->Returnerror(KIFAUTOMATON[12]);

	KifContainerLock _lock(this);
	string path = kpath->String();
	ofstream dumping(STR(path), ios::binary);
	if (dumping.fail())
		return kifcode->Returnerror(KIFAUTOMATON[0]);
	map<BULONG, short> alphabet;
	start.getalphabet(alphabet);
	if (alphabet.size() >= 4095)
		return kifcode->Returnerror(KIFAUTOMATON[13]);
	if (codefeat != 0)
		dumping << "codefeat" << "\n" << codefeat << "\n";
	dumping << KIFAUTOMATON[14] << "\n";
...
KifElement* KifAutomaton::loadcompact(string path, string& featchar) {
	...
	if (f == NULL)
		return kifcode->Returnerror(KIFAUTOMATON[4]);
	...
	BULONG l;
	short i;
	if (!strcmp(buff, KIFAUTOMATON[14])) {
		fgets(buff, 2048, f);
		trimming(buff);
		...
		if (!strcmp(buff, "features")) {
			fgets(buff, 2048, f);
			trimming(buff);
			while (!feof(f) && strcmp(buff, KIFAUTOMATON[15])) {
				pt = strchr(buff, '\t');
				if (pt != NULL) {
					*pt = 0;
					i = atoi(buff);
			...
			if (!strcmp(buff, KIFAUTOMATON[15])) {
				short v;
				uchar st;
				readonearc(f, st, v);
				start.status |= st;
	...
	return kifcode->Returnerror(KIFAUTOMATON[16]);
}
...
KifElement* KifGrammar::Loader(string& vrule) {
	...
	if (kifGlobals->bnfkif->m_rules(lret, &xn) != 1 || kifGlobals->bnfkif->currentpos != kifGlobals->bnfkif->fx->stack.size()) {
		if (kifGlobals->bnfkif->currentpos < kifGlobals->bnfkif->fx->stack.size()) {
			string content(KIFAUTOMATON[17]);
		...
		return kifcode->Returnerror(KIFLIBSTRINGS[244]);
	}
	...
	for (int i = 0; i < xn->nodes.size(); i++) {
		...
		if (!gramtraverse(automaton, xn->nodes[i], gs, e)) {
			delete xn;
			return kifcode->Returnerror(KIFAUTOMATON[18]);
		}
	}
	delete xn;
	...
	for (int i = 0; i < e.size(); i++) {
		...
		if (automaton->dictionary.find(e[i]) == automaton->dictionary.end()) {
			...
			if (!found) {
				string message = KIFAUTOMATON[19];
				message += " ";
...
Exported KifElement* KifTransducer::constructor(KifDomain* dom, KifElement* params, int idthread, KifCallFunction* callfunc) {
	...
	if (!automaton->load(name))
		return kifcode->Returnerror(KIFAUTOMATON[20]);
