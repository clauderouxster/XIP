...
public:
	...
	KifElement* MethodOpen(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		if (db != NULL)
			return kifcode->Returnerror(KIFSQLITESTRINGS[0]);
		...
		if (rc) {
			string message = KIFSQLITESTRINGS[1];
			message += dbname;
			message += "' --> ";
	...
	KifElement* MethodClose(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		if (db == NULL)
			return kifcode->Returnerror(KIFSQLITESTRINGS[2]);
		sqlite3_close(db);
		db = NULL;
		return kifTRUE;
	}
	...
	KifElement* MethodRun(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		if (db == NULL)
			return kifcode->Returnerror(KIFSQLITESTRINGS[3]);
	...
	KifElement* MethodPredicate(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		if (db == NULL)
			return kifcode->Returnerror(KIFSQLITESTRINGS[6]);
		...
		if (sqlite3_exec(db, STR(sqlc.str()), NULL, 0, &zErrMsg) != SQLITE_OK) {
			string message = KIFSQLITESTRINGS[7];
			message += zErrMsg;
			return kifcode->Returnerror(message);
		}
		return kifTRUE;
	...
	KifElement* MethodStorepredicate(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (rc != SQLITE_OK) {
			string message = KIFSQLITESTRINGS[8];
			message += zErrMsg;
			return kifcode->Returnerror(message);
		}
	...
	KifElement* MethodRemovepredicate(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		int rc = sqlite3_exec(db, STR(sqlc.str()), NULL, NULL, &zErrMsg);
		if (rc != SQLITE_OK) {
			string message = KIFSQLITESTRINGS[9];
			message += zErrMsg;
			return kifcode->Returnerror(message);
		}
	...
	KifElement* MethodCountpredicate(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		for (int i = 0; i < current->headrule->parameters.size(); i++) {
			if (current->headrule->parameters[i]->unifiedDeclaration(domain)) {
				e = current->headrule->parameters[i]->Exec(kifNULL, domain, kifNULL, idthread, callfunc);
				if (e == kifUNIVERSAL)
					continue;
		...
		if (rc != SQLITE_OK) {
			string message = KIFSQLITESTRINGS[10];
			message += zErrMsg;
			return kifcode->Returnerror(message);
		}
		return kifTRUE;
	...
	KifElement* MethodReadpredicate(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (rc != SQLITE_OK) {
			string message = KIFSQLITESTRINGS[11];
			message += zErrMsg;
			return kifcode->Returnerror(message);
		}
		return kifTRUE;
	...
	KifElement* MethodInsertThroughBind(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		if (db == NULL)
			return kifcode->Returnerror(KIFSQLITESTRINGS[3]);
		if (callfunc->Size() < 2)
			return kifcode->Returnerror(KIFSQLITESTRINGS[12]);
		...
		if (bindcommand == "ok") {
			...
			if (rc != SQLITE_OK)
				return kifcode->Returnerror(KIFSQLITESTRINGS[13]);
			...
			for (pos = 0; pos < vresults->values.size(); pos++) {
				command = ((KifMapStringString*)vresults->values[pos])->values[KIFSQLITESTRINGS[14]];
			...
			if (rc != SQLITE_OK)
				return kifcode->Returnerror(KIFSQLITESTRINGS[15]);

		}

		if (stmt == NULL)
			return kifcode->Returnerror(KIFSQLITESTRINGS[16]);
		...
		if (failure)
			return kifcode->Returnerror(KIFSQLITESTRINGS[18]);
	...
	KifElement* MethodInsert(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (callfunc->Size() == 0 || (callfunc->Size() % 2) == 0)
			return kifcode->Returnerror(KIFSQLITESTRINGS[21]);

		string lacommande = "insert into ";
		//A typical call would be:
		// mydb.insert("table1",KIFSQLITESTRINGS[22],"toto",KIFSQLITESTRINGS[23],12);
		//The first parameter is the table name
		KifElement* table = callfunc->Evaluatethread(0, domain, idthread);
		...
		if (rc != SQLITE_OK) {
			lacommande += KIFSQLITESTRINGS[24];
			lacommande += zErrMsg;
			sqlite3_free(zErrMsg);
	...
	KifElement* ExecMethod(short name, KifElement* contextualpattern, KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		//This call is a bit cryptic. It takes the method (function) pointer that has been associated in our map with KIFSQLITESTRINGS[22]
...
KifElement* KifIteratorSQLite::Begin() {
	...
	if (rc != SQLITE_OK) {
		string message = KIFSQLITESTRINGS[25];
...
extern "C" {
	Exporting bool Init_kifsqlite(KifCode* kifcode, string version) {
		if (version != KIFVERSION) {
			cerr << KIFSQLITESTRINGS[26] << endl;
			return false;
		}
#ifndef KIF_RAW
		cerr << KIFSQLITESTRINGS[27] << endl;
#endif
		kifcode->KifInitLib();
	...
	Exporting bool kifsqlite_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		//our new class.
		MethodInitialization(KIFSQLITESTRINGS[28], &KifSQLite::MethodOpen, P_ONE, KIFSQLITESTRINGS[29]);
		MethodInitialization(KIFSQLITESTRINGS[30], &KifSQLite::MethodPredicate, P_TWO, KIFSQLITESTRINGS[31]);
		MethodInitialization(KIFSQLITESTRINGS[32], &KifSQLite::MethodReadpredicate, P_NONE, KIFSQLITESTRINGS[33]);
		MethodInitialization(KIFSQLITESTRINGS[34], &KifSQLite::MethodCountpredicate, P_NONE, KIFSQLITESTRINGS[35]);
		MethodInitialization(KIFSQLITESTRINGS[36], &KifSQLite::MethodStorepredicate, P_NONE, KIFSQLITESTRINGS[37]);
		MethodInitialization(KIFSQLITESTRINGS[38], &KifSQLite::MethodRemovepredicate, P_NONE, KIFSQLITESTRINGS[39]);

		MethodInitialization(KIFSQLITESTRINGS[40], &KifSQLite::MethodClose, P_NONE, KIFSQLITESTRINGS[41]);
		MethodInitialization(KIFSQLITESTRINGS[42], &KifSQLite::MethodCreate, 65534, KIFSQLITESTRINGS[43]);
		MethodInitialization(KIFSQLITESTRINGS[44], &KifSQLite::MethodInsert, 65534, KIFSQLITESTRINGS[45]);
		MethodInitialization(KIFSQLITESTRINGS[46], &KifSQLite::MethodInsertThroughBind, 65534, KIFSQLITESTRINGS[47]);
		MethodInitialization(KIFSQLITESTRINGS[48], &KifSQLite::MethodRun, P_ONE, KIFSQLITESTRINGS[49]);
		MethodInitialization(KIFSQLITESTRINGS[50], &KifSQLite::MethodExecute, P_ONE, KIFSQLITESTRINGS[51]);
		MethodInitialization(KIFSQLITESTRINGS[52], &KifSQLite::MethodBegin, P_NONE | P_ONE, KIFSQLITESTRINGS[53]);
		MethodInitialization(KIFSQLITESTRINGS[54], &KifSQLite::MethodCommit, P_NONE, KIFSQLITESTRINGS[55]);
		MethodInitialization(KIFSQLITESTRINGS[56], &KifSQLite::MethodMemory, P_NONE, KIFSQLITESTRINGS[57]);
