...
public:
	...
	KifElement* loading() {
		...
		if (path.find(".mp3") != string::npos || path.find(".MP3") != string::npos) {
			...
			if (mpg123_open(mh, STR(path)) != MPG123_OK)
				return kifcode->Returnerror(KIFSOUNDSTRINGS[0]);

			if (mpg123_getformat(mh, &rate, &channels, &encoding) != MPG123_OK)
				return kifcode->Returnerror(KIFSOUNDSTRINGS[1]);
			...
			if (device == NULL)
				return kifcode->Returnerror(KIFSOUNDSTRINGS[2]);
		...
		//In this case, we need to do it live from the buffer initialized with the mp3 decoder
		if (device == NULL)
			return kifcode->Returnerror(KIFSOUNDSTRINGS[3]);
	...
	KifElement* MethodLoad(KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		...
		if (loaded)
			return kifcode->Returnerror(KIFSOUNDSTRINGS[4]);
	...
	KifElement* MethodPlay(KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		...
		if (!loaded)
			return kifcode->Returnerror(KIFSOUNDSTRINGS[5]);
		...
		if (error) {
			...
			ao_close(device);
			device = ao_open_live(driver, &format, NULL);
			return kifcode->Returnerror(KIFSOUNDSTRINGS[6]);
		}
	...
	KifElement* MethodEncode(KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		...
		if (device == NULL)
			return kifcode->Returnerror(KIFSOUNDSTRINGS[7]);

		KifElement* k = callfunc->Evaluatethread(0, domain, idthread);

		if (k->type != kifVectorInteger)
			return kifcode->Returnerror(KIFSOUNDSTRINGS[8]);
	...
	KifElement* MethodInfos(KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		...
		if (callfunc->Size() == 0) {
			...
			k = KIFSOUNDSTRINGS[9]; v = format.channels;
			kimap->storekey(k, v);
			k = KIFSOUNDSTRINGS[10]; v = format.rate;
			kimap->storekey(k, v);
			k = KIFSOUNDSTRINGS[11]; v = format.byte_format;
			kimap->storekey(k, v);
			k = KIFSOUNDSTRINGS[12]; v = mp3;
			kimap->storekey(k, v);
			k = KIFSOUNDSTRINGS[13]; v = format.bits;
			kimap->storekey(k, v);
			k = KIFSOUNDSTRINGS[14];
			if (mp3)
				v = mp3_buffer_size;
			else
				v = BUFFER_SIZE;
		...
		if (!kmap->aMapContainer())
			return kifcode->Returnerror(KIFSOUNDSTRINGS[15]);
		...
		for (itrloc->Begin(); itrloc->End() != kifTRUE; itrloc->Next()) {
			...
			if (k == KIFSOUNDSTRINGS[9])
				format.channels = v;
			else
			if (k == KIFSOUNDSTRINGS[10])
				format.rate = v;
		}
		itrloc->Release();
	...
	KifElement* MethodOpen(KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		...
		for (itrloc->Begin(); itrloc->End() != kifTRUE; itrloc->Next()) {
			...
			if (k == KIFSOUNDSTRINGS[9])
				format.channels = v;
			else if (k == KIFSOUNDSTRINGS[10])
				format.rate = v;
			else if (k == KIFSOUNDSTRINGS[11])
				format.byte_format = v;
			else if (k == KIFSOUNDSTRINGS[13])
				format.bits = v;
			else if (k == KIFSOUNDSTRINGS[12])
				mp3 = v;
			else if (k == KIFSOUNDSTRINGS[14])
				sz = v;
		}
		itrloc->Release();
...
extern "C" {
	Exporting bool Init_kifsound(KifCode* kifcode, string version) {
		if (version != KIFVERSION) {
			cerr << KIFSOUNDSTRINGS[16] << endl;
			return false;
		}

		kifcode->KifInitLib();

		if (kifGlobals->TestExternalType(KIFSOUNDSTRINGS[17]) == true)
			return false;
		...
		KifType res = KifAddNewType(KIFSOUNDSTRINGS[17], CreatekifsoundElement);
		if (res == kifVOID)
			return false;
		sound_type = res;
	...
	Exporting bool kifsound_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[18], &Kifsound::MethodLoad, P_ONE, KIFSOUNDSTRINGS[19]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[20], &Kifsound::MethodPlay, P_NONE | P_ONE, KIFSOUNDSTRINGS[21]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[22], &Kifsound::MethodStop, P_NONE, KIFSOUNDSTRINGS[23]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[24], &Kifsound::MethodInfos, P_NONE | P_ONE, KIFSOUNDSTRINGS[25]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[26], &Kifsound::MethodReset, P_NONE, KIFSOUNDSTRINGS[27]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[28], &Kifsound::MethodOpen, P_ONE, KIFSOUNDSTRINGS[29]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[30], &Kifsound::MethodClose, P_NONE, KIFSOUNDSTRINGS[31]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[32], &Kifsound::MethodDecode, P_ONE, KIFSOUNDSTRINGS[33]);
		MethodInitialization(kifcode, KIFSOUNDSTRINGS[34], &Kifsound::MethodEncode, P_ONE, KIFSOUNDSTRINGS[35]);
