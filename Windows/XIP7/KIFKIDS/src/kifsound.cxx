...
public:
	...
	//-----------------------------------------------------------------------------------------------------------------------
#ifdef MACSOUND
	//this function is called to test the kind of function we expect from the library with the KIFMPSTRING[0] operator...
	...
	Exported KifElement* constructor(KifDomain* domloc, KifElement* params, int idthread, KifCallFunction* callfunc) {
		...
		if (createSoundId(path.c_str(), &sound) == false)
			return domloc->kifcode->Returnerror(KIFMPSTRING[1]);
		if (function != NULL)
	...
	KifElement* MethodLoad(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
...
#ifdef MACSOUND
		if (createSoundId(path.c_str(), &sound) == false)
			return callfunc->kifcode->Returnerror(KIFMPSTRING[1]);
		if (function != NULL)
			addSoundFunction(&sound, completionCallback, (void*)this);
#endif
		...
		if (sound.Load((WCHAR*)base.c_str()) == false)
			return callfunc->kifcode->Returnerror(KIFMPSTRING[2]);
#endif
		loaded = true;
		created = true;
		return kifTRUE;
	...
	KifElement* MethodPlay(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		if (!loaded)
			return callfunc->kifcode->Returnerror(KIFMPSTRING[3]);
#ifdef MACSOUND
		playSoundId(&sound);
#endif
#ifdef WIN32
...
void completionCallback(SystemSoundID  mySSID, void* data) {
	...
	threadblocinit;
	KifElement* ret = kfunc.Exec(kifNULL, ksnd->dom, kifNULL, idthread, callfunc);
	ksnd->Resetreference();
	ret->Release();
}
#endif
...
extern "C" {
	Exported bool Init_kifmp3(KifCode* kifcode, string version) {
		if (version != KIFVERSION) {
			cerr << KIFMPSTRING[4] << endl;
			return false;
		}

		kifcode->KifInitLib();

		if (kifGlobals->TestExternalType(KIFMPSTRING[5]) == true)
			return false;
		...
		KifType res = KifAddNewType(KIFMPSTRING[5], Createkifmp3Element);
		if (res == kifVOID)
			return false;
		sound_type = res;
		return true;
	...
	Exported bool kifmp3_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		MethodInitialization(kifcode, KIFMPSTRING[6], &kifmp3::MethodLoad, P_ONE, KIFMPSTRING[7]);
		MethodInitialization(kifcode, KIFMPSTRING[8], &kifmp3::MethodPlay, P_NONE, KIFMPSTRING[9]);
		MethodInitialization(kifcode, KIFMPSTRING[10], &kifmp3::MethodStop, P_NONE, KIFMPSTRING[11]);
