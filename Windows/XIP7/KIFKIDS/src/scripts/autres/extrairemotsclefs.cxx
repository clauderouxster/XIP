file ftrad('/Users/rouxclaude/Développement/kifkids/src/frenchstrings.cxx','r');
string s,l;
...
for (s in ml) {
    if (ml[s]==true && "curseur" not in s && "affiche" not in s && s[0]!='_' && "ramasse" not in s && s.size()>1 && "racine" not in s && "eucli" not in s)
        unique.push(s);
}
...
clef+='\\bpas\\b|\\bdéfaut\\b|\\bcurl\\b|\\bsinon\\b|\\bsinonsi\\b|\\bavec\\b|\\.[a-zA-Z\\x80-\\xFF][0-9a-zA-Z\\x80-\\xFF]*\\(|\"|/|\\n|\\r';
clef+="|'\"];";
println(clef);
