...
extern "C" {
	...
	Exporting bool kifregex_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		kifGlobals->KifCreatePredefined(kifcode, KIFREGEXSTRINGS[4], P_TWO, &PreSplitRegex);
		kifGlobals->KifCreatePredefined(kifcode, KIFREGEXSTRINGS[5], P_THREE, &PreReplaceRgx);

		InitMapBooleanString(kifGlobals->kifStringFunctions, "regex");
		InitMapBooleanString(kifGlobals->kifStringFunctions, KIFREGEXSTRINGS[4]);
		InitMapBooleanString(kifGlobals->kifStringFunctions, KIFREGEXSTRINGS[5]);
		kifGlobals->kifReturnString[KifNewId(KIFREGEXSTRINGS[5])] = true;
