...
public:
	...
	Exported bool Setvalue(KifElement* ke, KifElement* idx, KifElement* dom) {
		...
		if (idx->type != kifIndex) {
			kifcode->Returnerror(KIFSPARSESTRINGS[0]);
			return true;
		}
		...
		kifcode->Returnerror(KIFSPARSESTRINGS[1]);
		return true;
	}
	...
	KifElement* Exec(KifElement* contextualpattern, KifDomain* dom, KifElement* idx, int idthread, KifCallFunction* callfunc) {
		...
		if (idx->type != kifIndex)
			return kifcode->Returnerror(KIFSPARSESTRINGS[0]);
	...
	KifElement* ExecMethod(short name, KifElement* contextualpattern, KifDomain* domain, int idthread, KifCallFunction* callfunc) {
		//This call is a bit cryptic. It takes the method (function) pointer that has been associated in our map with KIFSPARSESTRINGS[2]
	...
	//If you need to return a specific size for your object (the KIFSPARSESTRINGS[3] method is already available in Predefined)
	long Size() {
	...
	Exported KifElement* multiply(KifElement* a, KifElement* b, KifElement* context, bool autoself) {
		...
		if (ma->columnsize != mb->rowsize)
			return kifcode->Returnerror(KIFSPARSESTRINGS[4]);
	...
	KifElement* divide(KifElement* a, KifElement* b, KifElement* context, bool autoself) {
		...
		if (b->type != kifmatrixinteger_type) {
			long vb = b->Integer();
			if (vb == 0)
				return kifcode->Returnerror(KIFSPARSESTRINGS[5]);
		...
		if (!mb->inversion(inverted))
			return kifcode->Returnerror(KIFSPARSESTRINGS[6]);
...
extern "C" {
	Exported bool Init_kifsparse(KifCode* kifcode, string version) {
		...
		KifType res = KifAddNewType(KIFSPARSESTRINGS[8], CreatekifmatrixfloatElement);
		if (res == kifVOID)
			return false;
		kifmatrixfloat_type = res;
		res = KifAddNewType(KIFSPARSESTRINGS[9], CreatekifmatrixintegerElement);
		if (res == kifVOID)
			return false;
	...
	Exported bool kifsparse_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		MethodInitializationMatriceFloat(kifcode, KIFSPARSESTRINGS[10], &Kifmatrixfloat::MethodTransposed, P_NONE, KIFSPARSESTRINGS[11]);
		MethodInitializationMatriceFloat(kifcode, KIFSPARSESTRINGS[12], &Kifmatrixfloat::MethodMatrixSize, P_NONE | P_TWO, KIFSPARSESTRINGS[13]);
		MethodInitializationMatriceFloat(kifcode, KIFSPARSESTRINGS[14], &Kifmatrixfloat::MethodDeterminant, P_NONE, KIFSPARSESTRINGS[15]);
		MethodInitializationMatriceFloat(kifcode, KIFSPARSESTRINGS[16], &Kifmatrixfloat::MethodSum, P_NONE, KIFSPARSESTRINGS[17]);
		MethodInitializationMatriceFloat(kifcode, KIFSPARSESTRINGS[18], &Kifmatrixfloat::MethodProduct, P_NONE, KIFSPARSESTRINGS[19]);
		MethodInitializationMatriceFloat(kifcode, KIFSPARSESTRINGS[20], &Kifmatrixfloat::MethodInversion, P_NONE, KIFSPARSESTRINGS[21]);
		...
		MethodInitializationMatrixInteger(kifcode, KIFSPARSESTRINGS[10], &Kifmatrixinteger::MethodTransposed, P_NONE, KIFSPARSESTRINGS[11]);
		MethodInitializationMatrixInteger(kifcode, KIFSPARSESTRINGS[12], &Kifmatrixinteger::MethodMatrixSize, P_NONE | P_TWO, KIFSPARSESTRINGS[13]);
		MethodInitializationMatrixInteger(kifcode, KIFSPARSESTRINGS[14], &Kifmatrixinteger::MethodDeterminant, P_NONE, KIFSPARSESTRINGS[15]);
		MethodInitializationMatrixInteger(kifcode, KIFSPARSESTRINGS[16], &Kifmatrixinteger::MethodSum, P_NONE, KIFSPARSESTRINGS[17]);
		MethodInitializationMatrixInteger(kifcode, KIFSPARSESTRINGS[18], &Kifmatrixinteger::MethodProduct, P_NONE, KIFSPARSESTRINGS[19]);
		MethodInitializationMatrixInteger(kifcode, KIFSPARSESTRINGS[20], &Kifmatrixinteger::MethodInversion, P_NONE, KIFSPARSESTRINGS[21]);
