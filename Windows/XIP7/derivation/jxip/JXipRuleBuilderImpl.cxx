 ...
 *
 */
JNIEXPORT jstring JNICALL Java_com_xerox_jxip_RuleBuilder_buildRuleImplementation(JNIEnv *env, jobject obj, jlong boundaryIndex, jstring categor, jint ruleType, jint dependencyArity) {
    ...
    try {
        ...
        if (boundaryIndex == -1) {
            ...
            boundary = xipNodes[(long) boundaryIndex];
            //cout <<"JXipRuleBuilderImpl::buildRuleImplementation() boundary->category = " <<boundary->category <<endl;
        }
        string category = jstringToString(env, categor);
        char typeRule = (char) ruleType;
        string ruleString = ""; // the script rule generated
        char res = ruleGeneration->GenerateRule(focus, ruleString, selectedNodes, selectedDependencies, boundary, category, dependencyArity, typeRule);
        script = jstringFromString(env, ruleString);
        // call the java call back routine RuleBuilder::addCategoryDependency()
        jclass cls = env->GetObjectClass(obj);
        char* methodName = "addCategoryDependency";
        jmethodID addCategoryDependencyMID = env->GetMethodID(cls, methodName, "(Ljava/lang/String;)V");
        if (addCategoryDependencyMID == 0) {
            cerr << methodName << " not found" << endl;
            return jstringFromString(env, "");
        }
        for (int i = 0; i < ruleGeneration->categorydependency.size(); i++) {
        ...
        //cout <<"JXipRuleBuilderImpl::buildRuleImplementation() ruleString = " <<ruleString <<endl;
        if (res != 0) {
            cout << "JXipRuleBuilderImpl::buildRuleImplementation() Error   errorMessage = " << ruleString << endl;
            jclass exceptionClass = env->FindClass("java/lang/Exception");
            if (exceptionClass != 0) {
                env->ThrowNew(exceptionClass, ruleString.c_str());
...
JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addSelectedFeatureToSelectedNodeImplementation(JNIEnv *env, jobject obj, jlong indSelectNode, jlong indSelectFeature) {
    long indexSelectedNode = (long) indSelectNode;
    long indexSelectedFeature = (long) indSelectFeature;
    //cout <<"JXipRuleBuilderImpl::addSelectedFeatureToSelectedNodeImplementation() indSelectNode = " <<indSelectNode <<" indSelectFeature = " <<indSelectFeature <<endl;
    XipNode* xipNode = xipNodes[indexSelectedNode];
    //cout <<"JXipRuleBuilderImpl::addSelectedFeatureToSelectedNodeImplementation() XipNode category = "  <<xipNode->category <<endl;
    XipFeature* xipFeature = xipNode->features[indexSelectedFeature];
    string featureStr = xipFeature->attribute + ":" + xipFeature->value;
    //cout <<"JXipRuleBuilderImpl::addSelectedFeatureToSelectedNodeImplementation() featureStr = " <<featureStr <<endl;
    // selectedNode is a global and has been initialized by addSelectedNode
    (selectedNode->features).push_back(featureStr);
}

//rajoute par Vianney et Claude le 21/06/05

JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addSelectedFeatureStringToSelectedNodeImplementation(JNIEnv *env, jobject obj, jstring featureAttribute, jstring featureValue) {
...
void addSelectedNode(long index, char any) {
    ...
    XipNode* xipNode = xipNodes[index];
    if (xipNode == NULL) {
        cout << "JXipRuleBuilderImpl::addSelectedNode() node NOT FOUND index = " << index << endl;
    } else {
        //cout <<"JXipRuleBuilderImpl::addSelectedNode() xipNode->index = " <<xipNode->index <<endl;
        //cout <<"JXipRuleBuilderImpl::addSelectedNode() index = " <<index <<" category = " <<xipNode->category <<endl;
        // selectedNode is a global and will used by addSelectedFeatureToSelectedNodeImplementation
...
JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addSelectedXipNodeImplementation(JNIEnv *env, jobject obj, jlong ind, jboolean anyB) {
    long index = (long) ind;
    char any;
    if (anyB) {
        any = 1;
...
JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addSelectedXipLeafImplementation(JNIEnv *env, jobject obj, jlong ind, jboolean anyB) {
    long index = (long) ind;
    char any;
    if (anyB) {
        any = 1;
...
JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addSelectedFeatureToSelectedDependencyImplementation(JNIEnv *env, jobject obj, jlong indSelectDependency, jlong indSelectFeature) {
    ...
    XipUnit* xipUnit = xipResult->sentences[0];
    //cout <<"JXipRuleBuilderImpl::addSelectedFeatureToSelectedDependencyImplementation() language = " + xipUnit->language <<endl;
    XipDependency* xipDependency = xipUnit->dependencies[indexSelectedDependency];
    //cout <<"JXipRuleBuilderImpl::addSelectedFeatureToSelectedDependencyImplementation() name = " + xipDependency->name <<endl;
    XipFeature* xipFeature = xipDependency->features[indexSelectedFeature];
    //cout <<"JXipRuleBuilderImpl::addSelectedFeatureToSelectedDependencyImplementation() attribute = " + xipFeature->attribute <<endl;
    string featureStr = xipFeature->attribute + ":" + xipFeature->value;
    //cout <<"JXipRuleBuilderImpl::addSelectedFeatureToSelectedDependencyImplementation() featureStr = " + featureStr <<endl;
    // selectedDependency is a global and has been initialized by addSelectedXipDependencyImplementation
    (selectedDependency->features).push_back(featureStr);
}

JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addSelectedXipDependencyImplementation(JNIEnv *env, jobject obj, jlong ind) {
    long index = (long) ind;
    //cout <<"JXipRuleBuilderImpl::addSelectedXipDependencyImplementation() index = " <<index <<endl;
    XipUnit* xipUnit = xipResult->sentences[0];
    XipDependency* xipDependency = xipUnit->dependencies[index];
    // selectedDependency is a global and it will be used by addSelectedFeatureToSelectedDependencyImplementation
    selectedDependency = new SelectedDependency(xipDependency, index);
    selectedDependencies.ajoute(selectedDependency);
}

JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_clearGlobalsImplementation(JNIEnv *env, jobject obj) {
...
JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addFocusImplementation(JNIEnv *env, jobject obj, jlong ind) {
    long index = (long) ind;
...
JNIEXPORT void JNICALL Java_com_xerox_jxip_RuleBuilder_addCategoryDependencyImplementation(JNIEnv *env, jobject obj, jstring catDep) {
...
JNIEXPORT jint JNICALL Java_com_xerox_jxip_RuleBuilder_generateRuleCategoryImplementation(JNIEnv *env, jobject obj, jint gHandler, jstring fileName) {
    XipUnit* xipUnit = xipResult->sentences[0];
    RuleGeneration* ruleGeneration = new RuleGeneration(xipUnit);
    try {
        string fileNameStr = jstringToString(env, fileName);
        //cout <<"JXipRuleBuilderImpl::generateRuleCategory() gHandler = " <<gHandler <<" fileName = " <<fileNameStr <<endl;
        XipUnit* xipUnit = xipResult->sentences[0];
