...
static CString tocstring(string& res) {
	wstring base;
	s_utf8_to_unicode(base, (char*)res.c_str());
	return (WCHAR*)base.c_str();
}
...
BOOL XIPDEBUG::OnInitDialog() {
	...
	CString val(tocstring(xipsent->sentences[0]->root->category));
...
void XIPDEBUG::LeafAsFeature(CTreeCtrl& tree,XipLeaf* leaf,HTREEITEM racine) {        
    CString surf=tocstring(leaf->surface);
...
void XIPDEBUG::LeafAsNode(CTreeCtrl& tree,XipLeaf* leaf,HTREEITEM racine) {        
	...
	CString cat(tocstring(leaf->category));
	char ch[100];
    ...
    if (leaf->features.size()>0 && tree==treenode) {
        for (i=0;i<leaf->features.size();i++) {            
			CString att(tocstring(leaf->features[i]->attribute));
            att+=":";
			att += tocstring(leaf->features[i]->value);
    ...
    if (node->Type()==XIPNODE) {                
        if (node->features.size()>0) {
            ...
            for (i=0;i<node->features.size();i++) {
				CString att(tocstring(node->features[i]->attribute));
                att+=":";
				att += tocstring(node->features[i]->value);
        ...
        for (i=0;i<node->daughters.size();i++) {
            ...
            if (node->daughters[i]->Type()==XIPNODE) {
				...
				CString val(tocstring(node->daughters[i]->category));
				val+=ch;
                if (node->daughters[i]->daughters.size()>0 &&
                    node->daughters[i]->daughters[0]->Type()==XIPLEAF) {
                    val+="_";
					val += tocstring(((XipLeaf*)node->daughters[i]->daughters[0])->surface);
                }
...
void XIPDEBUG::ConstructDependency(XipDependency* dep,HTREEITEM racine) {
	CString dependence(tocstring(dep->name));
	int i;
    ...
    for (i=0;i<dep->parameters.size();i++) {
		...
		CString val(tocstring(nodex->category));
		sprintf(ch,":%d",nodex->index);
		val+=ch;
        if (nodex->daughters[0]->Type()==XIPLEAF) {
            val+="_";
			val += tocstring(((XipLeaf*)nodex->daughters[0])->surface);
        }
		dependence+=val;
    }
	...
	if (dep->features.size()>0) {
        for (i=0;i<dep->features.size();i++) {
			CString att(tocstring(dep->features[i]->attribute));
            att+=":";
			att += tocstring(dep->features[i]->value);
			if (att[0]=='#')
