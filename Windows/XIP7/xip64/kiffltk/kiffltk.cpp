...
static void MethodInitialization(string name,TemplateNameMethod func,string infos) {
...
extern "C" {
	...
	Exported bool TemplateName_KifInitialisationModule(KifCode* kifcode,string version) {
		...
		MethodInitialization("sample",&KifTemplateName::MethodSample,"sample(x,y): example of a method implementation");
	...
	Exported bool TemplateName_Terminate(KifCode* kifcode,string version) {
		return true;
	}
}
