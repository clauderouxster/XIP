...
typedef void (*KifExternalModule)(KifCode*);

char* kifErrorStrings[]={"KIF(0): Wrong number of parameters in SPLIT",
...
"KIF(188): Wrong number of parameters in FORMAT",
...
static map<string,bool> kifNumericalFunctions;
...
void StaticInitialisation() {
	if (kifNULL==NULL) {
		...
		kifStringFunctions["isdigit"]=true;
		...
		kifParserFunctions["parsefile"]=true;
		kifParserFunctions["parsexml"]=true;
		...
		kifNumericalFunctions["isa"]=true;
		kifNumericalFunctions["type"]=true;
		kifNumericalFunctions["list"]=true;
		kifNumericalFunctions["chr"]=true;
		
		kifNumericalFunctions["string"]=true;
		kifNumericalFunctions["format"]=true;
		kifNumericalFunctions["int"]=true;
		kifNumericalFunctions["float"]=true;
		kifNumericalFunctions["boolean"]=true;
...
static KifElement* PreParseFile(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	if (callfunc->parameters.size()!=2 && callfunc->parameters.size()!=3)
		return kifcodeptr->Returnerror(kifErrorStrings[180]);
	...
	if (ke->type==kifParser) {
		...
		KifElement* ks=callfunc->parameters[1]->Execute(kifTypeSTRING,dom,kifNULL,false);
		if (callfunc->parameters.size()==3) {
			kof=callfunc->parameters[2]->Execute(kifTypeSTRING,dom,kifNULL,false);
			of=new ofstream(STR(kof->String()));
		}
		else {
			if (kifcodeptr->parseur!=NULL && kifcodeptr->parseur->AppelExterne!=NULL) {
				XipResult* xip=XipCreateCurrentXipResult(kpars->ipar);
				if (XipParseFile(ks->String(),kpars->ipar,xip,1)==0)
					return kifTRUE;
				return kifFALSE;
			}
			else
		...
		if (of==NULL) {
			string message="Impossible to create file: ";
			message+=kof->String();
			return kifcodeptr->Returnerror(message);
		}
		int res=XipParseFileToFile(ks->String(),kpars->ipar, of,XML_NONE,1);
		delete of;
		if (res==0)
			return kifTRUE;
		return kifFALSE;
...
static KifElement* PreParseXMLFile(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	if (callfunc->parameters.size()!=3 && callfunc->parameters.size()!=4)
		return kifcodeptr->Returnerror(kifErrorStrings[180]);
	...
	if (ke->type==kifParser) {
		...
		if (callfunc->parameters.size()==4) {
			...
			if (kifcodeptr->parseur!=NULL && kifcodeptr->parseur->AppelExterne!=NULL) {
				kof=callfunc->parameters[2]->Execute(kifTypeSTRING,dom,kifNULL,false);
				XipResult* xip=XipCreateCurrentXipResult(kpars->ipar);
				if (XipParseXMLFile(kpars->ipar,ks->String(),kof->Integer(),xip)==0)
					return kifTRUE;
				return kifFALSE;
			}
			else
		...
		kof=callfunc->parameters[3]->Execute(kifTypeSTRING,dom,kifNULL,false);
		int res=XipParseFileXMLToFile(ks->String(),kpars->ipar, of,kof->Integer(),XML_NONE,1);
		delete of;
		if (res==0)
			return kifTRUE;
		return kifFALSE;
...
static KifElement* PreLoad(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	...
	if (ke->type==kifKiF) {
		...
		try {            
			kpars->ipar=KifLoadPath(flname,arguments,false);
			if (kpars->ipar==-1) {
				error=kifErrorStrings[184];
				error+=flname;
				return kifcodeptr->Returnerror(error);
			}
...
static KifElement* PreIsDigit(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	if (callfunc->parameters.size()!=1)
		return kifcodeptr->Returnerror(kifErrorStrings[9]);
...
static KifElement* PreSetDate(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	...
	if (timeinfo->type!=kifTime)
		return kifcodeptr->Returnerror(kifErrorStrings[14]);
...
static KifElement* PreYear(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	...
	KifElement* time=callfunc->parameters[0]->Execute(kifNULL,dom,kifNULL,false);
	if (time->type!=kifTime)
		return kifcodeptr->Returnerror(kifErrorStrings[18]);
...
static KifElement* PreDay(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	...
	KifElement* time=callfunc->parameters[0]->Execute(kifNULL,dom,kifNULL,false);
	if (time->type!=kifTime)
		return kifcodeptr->Returnerror(kifErrorStrings[20]);
	struct tm* temps = localtime(&((KifTime*)time)->value); 
...
static KifElement* PreHour(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	...
	KifElement* time=callfunc->parameters[0]->Execute(kifNULL,dom,kifNULL,false);
	if (time->type!=kifTime)
		return kifcodeptr->Returnerror(kifErrorStrings[22]);
	struct tm* temps = localtime(&((KifTime*)time)->value); 
	char ch[100];
...
static KifElement* PreMonth(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	...
	KifElement* time=callfunc->parameters[0]->Execute(kifNULL,dom,kifNULL,false);
	if (time->type!=kifTime)
		return kifcodeptr->Returnerror(kifErrorStrings[24]);
	struct tm* temps = localtime(&((KifTime*)time)->value); 
...
static KifElement* PrePrintLN(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	KifElement* res=kifNULL;
	ostream* os=kifcodeptr->os;
	for (size_t i=0;i<callfunc->parameters.size();i++) {
		if (i)
...
static KifElement* PrePrinterrLN(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	KifElement* res=kifNULL;
	ostream* os=kifcodeptr->os;
	for (size_t i=0;i<callfunc->parameters.size();i++) {
		if (i)
...
static KifElement* PreGet(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	...
	if (kfirst->type==kifString || kfirst->type==kifFloat || kfirst->type==kifInteger) { 
		gets(kifcodeptr->buffer);
		KifElement* ke=new KifString(kifcodeptr,NULL,kifcodeptr->buffer);
...
static KifElement* PreFormat(KifCode* kifcodeptr,KifPredefined* myself,KifElement* pattern,KifDomain* dom,KifElement* val,KifCallFunction* callfunc) {
	if (callfunc->parameters.size()!=2)
		return kifcodeptr->Returnerror(kifErrorStrings[188]);

	KifElement* knum=callfunc->parameters[0]->Execute(kifNULL,dom,kifNULL,false);
	KifElement* kformat=callfunc->parameters[1]->Execute(kifNULL,dom,kifNULL,false);
	if (knum->type==kifInteger)
		sprintf_s(kifcodeptr->buffer,1024,STR(kformat->String()),knum->Integer());
	else 
		sprintf_s(kifcodeptr->buffer,1024,STR(kformat->String()),knum->Float());
	
	return new KifString(kifcodeptr,NULL,kifcodeptr->buffer);
}
...
bool KifCode::KifCreatePredefined(string name,Predefined func) {
...
//===================================================================
	...
	kifcodeptr->KifCreatePredefined("parsefile",&PreParseFile);
	kifcodeptr->KifCreatePredefined("parsexml",&PreParseXMLFile);
...
KifElement* KifGraph::Copy(KifDomain* kp,KifElement* dom) {
	KifElement* ke=new KifGraph(kifcode,kp,graph);
	return ke;
}
...
//===================================================================
KifTime::KifTime(KifCode* klc,KifElement* kp,time_t v) : KifElement(klc,kp,kifTime) {
	if (v==0)
		time(&value);
	else
		value=v;
...
KifElement* KifFST::Compilenet(KifMap* kmap,KifElement* filename,bool with) {
	if (tok!=NULL || kifcode->parseur==NULL)
		return kifFALSE;
	...
	GlobalParseur* p=kifcode->parseur;
	tok=new TokLOOK(p,net,"",0,kifcode->utf8);
	return kifTRUE;
}
	...
	if (tok->parseur->lectures.dernier) {
		...
		if (res[0]!='?') {
			for (int i=0;i<tok->parseur->lectures.dernier;i++)
				kvect->Push(new KifString(kifcode,NULL,tok->parseur->lectures[i]->mot+sz));
...
KifElement* KifFST::Tokdown(KifElement* str,KifElement* feats,KifElement* pattern) {
	...
	if (tok->parseur->lectures.dernier) {		
		...
		if (res[0]!='?') {
			if (pattern->type!=kifVector) {				
				reponse=new KifString(kifcode,NULL,res);
				tok->parseur->nettoieLectures();
				return reponse;
			}
			else {
...
KifElement* KifFST::Load(KifElement* filename,KifElement* flags,KifElement* ty,KifElement* utf8) {
	...
	tok=new TokLOOK(kifcode->parseur,STR(sfilename),STR(sflags),itype,utf);
	return kifTRUE;
}
#endif
...
KifElement* KifTime::plus(KifElement* a,KifElement* b) {
	if (a==this) {
		value+=b->Integer();
		return this;
	}
	long i=a->Integer()+b->Integer();
	KifElement* kv= new KifTime(kifcode,NULL,i);
	return kv;

}
KifElement* KifTime::minus(KifElement* a,KifElement* b) {
	if (a==this) {
...
KifElement* KifIteratorString::Key() {
	...
	if (reverse==true)
		i=value.size()-itx-1;
	KifElement* ke=new KifInteger(kifcode,NULL,i);		
	return ke;
}
...
KifElement* KifIteratorString::IteratorValue() {
	...
	KifString* ke=new KifString(kifcode,NULL,s);		
	return ke;
}
...
void KifMatrix::Save(string& filename,string& thetype) {
	...
	if (thetype=="xml") {
		...
		sprintf_s(kifcode->buffer,1024,"<Matrix name=\"%s\"/>",STR(name));
		//On cree la racine de notre document
		xmlDocPtr doc=xmlSAXParseMemory(NULL,kifcode->buffer,strlen(kifcode->buffer),0);
		...
		for (itins=values.begin();itins!=values.end();itins++) {
			for (itclass=(*itins).begin();itclass!=(*itins).end();itclass++) {
				...
				kifcode->parseur->AjouteAttributValeur(xmlatt,"id",kifcode->buffer);
			}
			i++;
		}
...
KifElement* KifDomain::Declaration(string& s) {
	if (s=="this")
		return this;
	if (s=="list")
		return kifcode->kifBaseFunctions["list"];
...
KifElement* KifCall::Declaration(string& s) {
	if (s=="apply")
		return kifcode->kifBaseFunctions["apply"];
	return NULL;
}
...
KifElement* KifFloat::Declaration(string& n) {
	if (kifMathFunctions[n]!=NULL || TestkifFunctions(n,kifNumericalFunctions)==true || n=="get")
		return kifcode->kifBaseFunctions[n];
	return NULL;
}
...
KifElement* KifDomain::Framedefined(string methodname,KifElement* pattern,KifDomain* dom) {
	...
	KifElement* func=kifcode->Getbasic(methodname,dom);
	if (func!=NULL) {
...
KifElement* KifDomain::Copy(KifDomain* dom,KifElement* decl) {
	...
	for (it=declarations.begin();it!=declarations.end();it++) {
		string n=it->first;
		KifElement* xe=kifcode->Getbasic(n,this);
		if (xe!=NULL) {
...
KifElement* KifString::in(KifElement* a,KifElement* b,KifDomain* kfunc,KifElement* pattern,bool idx) {
	if (kfunc==NULL) {
		...
		if (rs!=string::npos) {
			if (pattern->type==kifString)
				return new KifString(kifcode,NULL,s1);
...
void KifFrame::Run() {
	size_t i;
	KifCode* kcode=kifcode;
	kcode->basegarbage=kcode->garbage.size();
	kcode->trigger+=kcode->basegarbage;
	for (i=0;i<kcode->garbage.dernier;i++)
		kcode->garbage[i]->reference=1024;

	string n="_args";
	KifConst* var=(KifConst*)kcode->kstart->Declaration(n);
	...
	for (i=6;i<instructions.size();i++) {
		...
		if (ke->Variable() || xt==kifInstruction || xt==kifCallFunction || xt==kifInstance || xt==kifXPathExpression) {
			kifcode->Debugging(this,ke,this,NULL);
...
KifElement* KifFile::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'file'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifGraph::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'graph'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifXipNode::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'node'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifXipDependency::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message;
		if (type==kifXipGeneration)
			message="No index for 'generation'";
		else
			message="No index for 'dependency'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifXMLNode::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'xml'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifXMLNodePtr::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'xml'";
		return kifcode->Returnerror(message);
	}
...
KifElement* KifInteger::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'int'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifFloat::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'float'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifBoolean::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'boolean'";
		return kifcode->Returnerror(message);
	}
	return this;
}

KifElement* KifTime::Execute(KifElement* pattern,KifDomain* dom,KifElement* ke,bool rcouple) {
	if (ke!=NULL && ke->type==kifIndex) {
		string message="No index for 'time'";
		return kifcode->Returnerror(message);
	}
	return this;
}
	...
	if (idx==NULL || idx==kifNULL) {
		...
		if (pattern->type==kifMap) {
			...
			for (size_t i=0;i<values->size();i++) {
				sprintf_s(kifcode->buffer,20,"%d",i);
...
KifElement* KifCallFunction::Execute(KifElement* pattern,KifDomain* dom,KifElement* val,bool rcouple) {
	...
	if (body!=NULL) {
		...
		if (body->type==kifCallMethod) {
			Initialisation();		
			KifCallMethod* kpred=(KifCallMethod*)body;
			ke=kpred->Execute(pattern,dom,val,this);
			ke=applyfunction(ke);
	...
	if (err==false) {
		...
		message+=name;
		ke=kifcode->Returnerror(message);
	}

	environment=oldpattern;
	Resetdeclaration(stacksparam);
	Initialisation();
	if (pattern==kifNULL && kifcode->Trigger())
		kifcode->Clean();
	return ke;
}
...
KifElement* KifInstruction::LoopExecute(KifElement* pattern,KifDomain* dom,KifElement* ke) {
	...
	while (result->Boolean()==true) {
		...
		if (kifcode->stacking.size()==0 && kifcode->Trigger())
			kifcode->Clean();
...
KifElement* KifCode::KifParseCall(x_node* xn,KifElement* kf) {
	...
	if (name=="use") {
		...
		if (xn->nodes.size()==2)
			KifLoadModule(xn,xn->nodes[1]->value);
		else {
...
KifElement* KifCode::KifParseXPath(x_node* xn,KifElement* kf) {
	...
	s+=";";
	istringstream is(s);
	clefXML* clexml=parseur->ParseXPathExpression(is,k,buffer,neg,variables_utilisees);            
...
Exported string XipKifExecute(int ipar,string name,vector<string>& parameters,ostringstream* os,bool debugkif) {
...
Exported void KifSetTrigger(int threshold) {
...
KifType KifCode::KifNewType(string n) {
	kifLastType++;
	kifStringType[(KifType)kifLastType]=n;
	return (KifType)kifLastType;
}
...
bool KifCode::KifLoadModule(x_node* xn,string name) {
	...
	long line=bnfkif->x_linenumber(xn->start);
	char lname[1024];
	strcpy(lname,STR(name)+1);
	lname[name.size()-2]=0;
	LoadMe = LoadLibrary(lname);

	// Check to see if the library was loaded successfully 

	if (LoadMe == 0) {
		DWORD err=GetLastError();
		string message=kifErrorStrings[189];
		message+=name;
		throw KifRaiseError(message,current->name,line,xn->end);
	}

	KifExternalModule LibMainEntryPoint;            
	LibMainEntryPoint = (KifExternalModule)GetProcAddress(LoadMe,"KifInitialisationModule");
	
	if (LibMainEntryPoint==NULL) {
		string message=kifErrorStrings[190];
		message+=name;
		throw KifRaiseError(message,current->name,line,xn->end);
	}
	
	(*LibMainEntryPoint)(this);
	return true;
}
