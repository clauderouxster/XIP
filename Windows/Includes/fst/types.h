/* $Id: types.h,v 1.4 2005/03/04 15:45:26 grondeau Exp $ */
...
void FstBaseExport * get_default_context(void) ; /* Needed for the utf8 macros */
 ...
 *******************************************************/
#ifdef _MSC_VER
#pragma warning (once : 4214) /* nonstandard extension used : bit field types other than int */
/* #pragma warning (error : 4700 4701)  Variable used without being initialized */
#endif
...
#ifndef BIT_DEFINED
#define BIT_DEFINED
typedef unsigned short fst_bit;
#endif
#if !defined(FST_BYTE_DEFINED)
#define FST_BYTE_DEFINED
typedef unsigned char fst_byte;
#endif
...
typedef struct ARC {
#ifndef SMALL_FSM
    fst_bit type_bit : 1; /* must be FIRST for ALTCHAINS: Arcs=0, States=1 */
    fst_bit userflag1 : 1;
    fst_bit visit_mark : 2;
    fst_bit big_arc_flag : 1; /* 0 in ARC objects, 1 in BIG_ARC objects */
    fst_bit userflag2 : 2; /* New, for local sequentialization. 1999 Jan. TG */
    fst_bit unused : 9;   /* Use at your own risk. */
#endif
...
typedef struct BIG_ARC {
    ...
    fst_bit visit_mark : 2;
    fst_bit big_arc_flag : 1; /* 0 in ARC objects, 1 in BIG_ARC objects */
    fst_bit unused : 11;   /* Use at your own risk. */
    id_type label; /* index into label table -- use id_to_label() to access print name */
    struct STATE *destination;
    struct BIG_ARC *next;
    void *user_pointer; /* only ALTCHAINS uses this field at the moment */
...
typedef struct STATE {
        fst_bit type_bit : 1; /* must be FIRST for ALTCHAINS. Arcs=0, States=1*/
	fst_bit final : 1;
	fst_bit deterministic : 1;
	fst_bit vector_p : 1;
        fst_bit visit_mark : 8;	/* 8 bits because of byte-fetch */
        fst_bit userflag2 : 2;
        fst_bit is_virtual: 1;
        fst_bit unused : 1;	   /* Use at your own rist */
	union { 
		struct ARC *set;
...
typedef struct PARSE_TABLE {
  int char_encoding;
  int has_multichar_symbols;
  struct CH_NODE **top_node;
} PARSE_TBLtype, *PARSE_TBL;

#define PARSE_TBL_TOP_NODE(X)  (X)->top_node
#define PARSE_TBL_CHAR_ENC(X)  (X)->char_encoding
#define PARSE_TBL_MULTICHAR(X) (X)->has_multichar_symbols
...
typedef struct NETWORK {
  struct {
    ...
    fst_bit completed:1;
    fst_bit minimized:1;
    fst_bit epsilon_free:1;
    fst_bit sorted_states:1;
    fst_bit loop_free:1;
    fst_bit twol_net:1; /* NET created in TWOL_MODE */
    fst_bit visit_marks_dirty:1;
    fst_bit names_matter:1;
    fst_bit shared_arc_lists:1;
    fst_bit has_arc_user_pointer:1;
    fst_bit closed_sigma:1;
    fst_bit start_state_final:1;
    fst_bit tabular:1;
    fst_bit compacted:1;
    fst_bit upper_seq:1;
    fst_bit lower_seq:1;
    fst_bit mark:1;
    fst_bit u_flag_diacr:1;
    fst_bit l_flag_diacr:1;
    fst_bit left_bimachine:1;
    fst_bit right_bimachine:1;
    fst_bit sorted_arcs:1;
    fst_bit reduced_labelset:1;
    fst_bit Kaplan_compressed:1;
    fst_bit is_virtual:1;
    fst_bit is_arc_optimized:1;
    fst_bit unused: 4;
  } flags;
...
typedef struct STANDARD_HEADER {
	...
	unsigned char license_type; /* 1 byte */
	unsigned char reserved1;   /* 1 byte */
	unsigned char reserved2;   /* 1 byte */
	unsigned char reserved3;   /* 1 byte */
	char *ascii_date;
	int date_max;
...
#define HEADER_license_type(X)			(X)->license_type
#define HEADER_reserved1(X)			(X)->reserved1
#define HEADER_reserved2(X)			(X)->reserved2
#define HEADER_reserved3(X)			(X)->reserved3
...
enum char_encoding {CHAR_ENC_UNKNOWN, CHAR_ENC_UTF_8, CHAR_ENC_ISO_8859_1};

#define DEFAULT_CHAR_ENCODING CHAR_ENC_ISO_8859_1

typedef struct C_FSM_CONTEXT {      /* Current global variables .*/
  int mode ;
  char *copyright_string ;          /* COPYRIGHT_OWNER */
  int compose_strategy ;
  int io_character_encoding ;		/* CHAR_ENC_UTF_8 or CHAR_ENC_ISO_8859_1 (default) */
  struct flag_parameters
  {
    fst_bit copy_p ;
    fst_bit determinize_p ;
    fst_bit interactive_p ;            /* fst_IY_INTERACTIVE */
    fst_bit last_errors_p ;            /* fst_LAST_ERRORS */
    fst_bit lex_errors_p ;             /* fst_LAST_ERRORS */
    fst_bit minimize_p ;
    fst_bit name_nets_p ;              /* fst_IY_NAME_NETS */
    fst_bit obey_flags_p ;
    fst_bit prune_p ;
    fst_bit quit_on_fail_p ;           /* fst_IY_QUIT_OF_FAIL */
    fst_bit reclaim_p ;
    fst_bit recursive_define_p ;       /* fst_IY_RECURSIVE_DEFINE */
    fst_bit sq_final_strings_arcs;     /* SQ_FINAL_STRINGS_ARCS */
    fst_bit sq_intern_strings_arcs;    /* SQ_INTERN_STRINGS_ARCS */
    fst_bit sq_string_onelong;         /* SQ_STRING_ONELONG */
    fst_bit unicode_p ;                /* UNICODE_P */
    fst_bit verbose_p ;                /* fst_IY_VERBOSE */
    fst_bit embedded_command_p;        /* for interface->regex flex-ers 99-6-15 TG */
  } flags ;
  ...
  {
    fst_bit ignore_white_space_p ;
    fst_bit zero_to_epsilon_p ;
  ...
  {
    fst_bit altchain_p ;                /* ALTCHAIN_P */
    fst_bit status_bar_p ;              /* DISPLAY_READ_STATUS_BAR */
    long int status_bar_increment ; /* STATUS_BAR_INCREMENT */
    unsigned long arc_count ;       /* ARC_COUNT */
    fst_byte cur_byte ;                 /* CUR_BYTE */
...
extern void FstBaseExport *default_context_object;

/* Macros that define functions from the FST context */

#define IS_UTF8_MODE (((FST_CNTXTptr)get_default_context())->io_character_encoding == CHAR_ENC_UTF_8)

#define FST_fat_to_thin_name		(IS_UTF8_MODE ? utf8_fat_to_thin_name : fat_to_thin_name )
#define FST_intern_literal			(IS_UTF8_MODE ? utf8_intern_literal : intern_literal ) 
#define FST_intern_unicode			(IS_UTF8_MODE ? utf8_intern_unicode : intern_unicode )
#define FST_copy_label_name			(IS_UTF8_MODE ? utf8_copy_label_name : copy_label_name )
  /*#define FST_parse_string			(IS_UTF8_MODE ? utf8_parse_string : parse_string ) */
#define FST_parse_string                       parse_string

#define FST_string_to_fsm			(IS_UTF8_MODE ? utf8_string_to_fsm : string_to_fsm )

#define FST_fat_char_out			(IS_UTF8_MODE ? utf8_fat_char_out : fat_char_out )
#define FST_fat_char_out_print_esc	(IS_UTF8_MODE ? utf8_fat_char_out_print_esc : fat_char_out_print_esc )

#define FST_fat_string_out     (IS_UTF8_MODE ? utf8_fat_string_out : fat_string_out)

#define FST_fat_string_to_buf		(IS_UTF8_MODE ? utf8_fat_string_to_buf : fat_string_to_buf )
#define FST_fprint_fat_string		(IS_UTF8_MODE ? utf8_fprint_fat_string : fprint_fat_string )
#define FST_fprint_fat_string_esc	(IS_UTF8_MODE ? utf8_fprint_fat_string_esc : fprint_fat_string_esc )
#define FST_fprint_fat_string_with_esc (IS_UTF8_MODE ? utf8_fprint_fat_string_with_esc : fprint_fat_string_with_esc )
#define FST_fprint_fat_char_esc		(IS_UTF8_MODE ? utf8_fprint_fat_char_esc : fprint_fat_char_esc )

#define FST_fat_to_thin				(IS_UTF8_MODE ? utf8_fat_to_thin : fat_to_thin )
#define FST_thin_to_temp_fat		(IS_UTF8_MODE ? utf8_thin_to_temp_fat : thin_to_temp_fat )
#define FST_thin_to_fat				(IS_UTF8_MODE ? utf8_thin_to_fat : thin_to_fat )

#define FST_network_to_text_file	(IS_UTF8_MODE ? utf8_network_to_text_file : network_to_text_file )
#define FST_fsm_to_word_file		(IS_UTF8_MODE ? utf8_fsm_to_word_file : fsm_to_word_file )
#define FST_read_line_and_convert	(IS_UTF8_MODE ? utf8_read_line_and_convert : read_line_and_convert )
...
/* -------------------------------------------------------------------- */
/* The VirtualInfo has information about the virtual operation being    */
/* simulated, including methods for computing the arc set and the final */
/* bit.  It is stored as a network property and in each virtual state.  */
/* -------------------------------------------------------------------- */

/* ------------------------------------------------------------------------ */
/* DATA TYPES AND ACCESS FUNCTIONS FOR VIRTUAL NETWORKS                     */
  /* Moved here from John Maxwell's virtual-fsm.h.                          */
/* ------------------------------------------------------------------------ */
...
typedef struct VirtualInfo VirtualInfo;
...
typedef int (*FinalPFN)(STATEptr vstate, VirtualInfo *vinfo);
typedef ARCptr (*ArcSetFN)(STATEptr vstate, VirtualInfo *vinfo);
typedef void (*ExpandOtherFN)(ALPHABETptr new_sigma, NETptr vnetwork,
			      VirtualInfo *vinfo);

struct VirtualInfo {
  VMethods *methods;
  int id;         /* A unique identifier for debugging. */
  ...
  NETptr network;          /* A pointer back to the virtual network */
  int n_dead_states;       /* Possibly useful for pruning */
  /* If op1 and op2 are marked as reclaimable, they should be freed
     when the virtual net that contains them is freed. */
  fst_bit op1_reclaimable;
  fst_bit op2_reclaimable;
};
...
struct VMethods {
  ...
  int operation_id;    /* The enum id for the operation. */
  StateDataType data_type;  /* The data type of the state data */
  FinalPFN final_p_fn; /* Determines whether a virtual state is final. */
  ArcSetFN arc_set_fn; /* Returns the arcs for a virtual state. */
  ExpandOtherFN expand_other_fn;
  int reclaimable;     /* Indicates whether the structure can be freed */
};
...
#define NET_virtual_info(X)   (X)->client_cell
#define STATE_state_data(X)   (X)->client_cell
/* The virtual info is not needed once the arcset is constructed, 
   hence can temporarily reside in that field. */
#define STATE_virtual_info(X) (VirtualInfo *)(X)->arc.set


enum VirtualOperations {Virtual_Concat = 0,
			Virtual_Intersect = 1,
			Virtual_Option = 2,
			Virtual_Minus = 3,
			Virtual_Negation = 4,
			...
			Virtual_One_Plus = 6,
			Virtual_Zero_Plus = 7,
			Virtual_Determinize = 8,
			Virtual_Eps_Remove = 9,
			Virtual_Copy = 10,
			Virtual_Invert = 11,
			Virtual_Lower = 12,
			Virtual_Upper = 13,
			Virtual_Priority_Union = 14,
			Virtual_Tags = 15,
			Virtual_Non_Tags = 16,
			Virtual_Compose = 17,
			Virtual_CrossProduct = 18,
			Virtual_Ignore = 19,
			Virtual_Repeat = 20,
			Virtual_Shuffle = 21,
			Virtual_Other_Than = 22
			};


/* temporary, dummy types for WEIGHTED_NETWORK - To Be Changed in a real application ! */
#define WN char
typedef WN WEIGHTED_NETWORK;
