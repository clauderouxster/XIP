/* $Id: heap.h,v 1.1 2005/06/03 12:11:51 grondeau Exp $ */
/*
...
extern FstDebugHeapCellMode FsmFstExport fst_debug_heap_cell_mode;
...
int FsmFstExport HEAP_cell_size(HEAPptr heap);
#else
...
extern HEAPptr FsmFstExport ALL_HEAPS;

HEAPptr FsmFstExport init_heap(size_t cell_size, size_t block_size, char *name);
long int FsmFstExport heap_items(HEAPptr heap, int total_p);
long int FsmFstExport heap_size(HEAPptr heap, int total_p);
long int FsmFstExport free_list_size(HEAPptr heap);

#ifdef DEBUG
void FsmFstExport *FstDebug_cell_alloc(HEAPptr heap, char *filename, int linenum);
#define cell_alloc(h) FstDebug_cell_alloc(h, __FILE__, __LINE__)
#else
void FsmFstExport *cell_alloc(HEAPptr heap);
#endif
void *debug_cell_alloc(HEAPptr heap, long int *cell_id);

#ifdef DEBUG
void FsmFstExport FstDebug_free_cell(void *cell, HEAPptr heap, char *filename, int linenum);
#define free_cell(cell, heap) FstDebug_free_cell(cell, heap, __FILE__, __LINE__)
#else
void FsmFstExport free_cell(void *cell, HEAPptr heap);
#endif

void free_cell_debug(void *cell, HEAPptr heap);
void FsmFstExport free_heap(HEAPptr heap);
void free_all_heaps(void);
void FsmFstExport reset_heap(HEAPptr heap);
...
long FsmFstExport tally_heaps(FILE *stream);
