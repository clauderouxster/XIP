...
bool compileautomaton(string nom, string sauve, int latinencoding);
void afficheversion();
void KifFinalClean();
...
void Usage()
{
	...
	*flot_erreur << "     -build lex.txt lex.cpt encodingtable: compile a text lexicon into an automaton compatible with trans according to the latin table encoding" << Endl;
...
void affiche_version() {
	...
	lasortie << Endl << "\t" << "Copyright (C) Xerox Corporation 2000-2007" << Endl
		<< "\t" << question << Endl
	...
	lasortie << Endl << "\tImplementation:" << Endl
		...
		<< "\t\tFarhad Nooralahzadeh" << Endl
		<< Endl << "\tConception:" << Endl
		<< "\t\t" << "Salah Aït-Mokhtar" << Endl
		<< "\t\tCaroline Brun" << Endl
		<< "\t\tJean-Pierre Chanod" << Endl;
	lasortie << "\t\tCaroline " << "Hagège" << Endl
		<< "\t\tAaron Kaplan" << Endl
		<< "\t\tClaude Roux" << Endl
		<< "\t\tGilbert Rondeau" << Endl;
	lasortie << "\t\t" << "Frédérique Segond" << Endl;
	lasortie << "\t\t" << "Ágnes Sándor" << Endl << Endl;
...
int main(int argc, char *argv[])
{
	...
	if (b_intrans || b_outtrans) {
		...
		time(&tempsinitial);
		if (!compileautomaton(intrans, outtrans, latintable)) {
	...
	try {
		...
		for (int iautreparseur = 1; iautreparseur < grmfile.dernier; iautreparseur++) {
			...
			strcpy(fgrmlocal.grmname, grmfile[iautreparseur]);
			numberValues = -1;
