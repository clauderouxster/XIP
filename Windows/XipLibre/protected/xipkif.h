...
class KifXIPTRANS : public KifElement {
public:

	string name;
	Xip_Automaton* automaton;

	KifXIPTRANS(KifCode* klc, KifElement* kp) : KifElement(klc, kp, kifXIPTRANS) {
		automaton = NULL;
	}
	...
	KifElement* Copy(KifDomain* kp, KifElement* dom = kifNULL) {
		return new KifXIPTRANS(kifcode, NULL);
	}
