...
public:
	...
	KifElement* CreateSocket() {
		...
		if (sock == -1)
			return kifcode->Returnerror("SOCKET(856): Socket error");
#ifdef WIN32        
		...
		if (checkipaddres(servername) == false) {
			...
			if (hp == NULL)                             /* we don't exist !? */
				return kifcode->Returnerror("SOCKET(851): Cannot find host by name on this machine");
	...
	KifElement* MethodCreateServer(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (hp != NULL)
			return kifcode->Returnerror("SOCKET(831): Server already launched on this socket");
		...
		if (callfunc->Size() == 2) {
			...
			if (s.size() >= MAXHOSTNAME)
				return kifcode->Returnerror("SOCKET(850): Wrong server name");
	...
	KifElement* MethodClose(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (server == true) {
			...
			if (callfunc->Size() == 1) {
				...
				if (socketclient == -1)
					return kifcode->Returnerror("SOCKET(824): Non connected socket");
	...
	KifElement* MethodRead(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (server == true) {
			if (callfunc == NULL || callfunc->Size() == 0)
				return kifcode->Returnerror("SOCKET(820): Missing parameter in read");
		...
		if (currentsock == -1)
			return kifcode->Returnerror("SOCKET(858): No client connected");
	...
	KifElement* MethodRun(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (server == false)
			return kifcode->Returnerror("SOCKET(825): RUN can only be launched from a server");
		...
		while (true) {
			...
			if (act == 'G') {
				...
				if (obj == NULL || obj->type != kifVariable || obj->Private() || obj->Returnvalue()->type != kifDomain) {
					string errmessage = "SOCKET(870): Object '";
					errmessage += res;
					errmessage += "' unknown ";
					errmessage += Msgerror();
			...
			if (act == 'g') {
				...
				if (obj == NULL || obj->type != kifFunction || obj->Private()) {
					string errmessage = "SOCKET(871): Function '";
					errmessage += res;
					errmessage += "' unknown ";
	...
	KifElement* Write(string act, KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (server == true) {
			first = 1;
			if (callfunc->Size() <= 1)
				return kifcode->Returnerror("SOCKET(822): Missing parameter in write");
	...
	KifElement* MethodGet(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (server == true) {
			if (callfunc->Size() == 0)
				return kifcode->Returnerror("SOCKET(821): Missing parameter in receive");
	...
	KifElement* MethodReadRaw(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (server == true) {
			...
			if (callfunc->Size() == 2)
				return kifcode->Returnerror("SOCKET(823): Too many parameter in receive");
			currentsock = sock;
			if (callfunc->Size() == 1) {
	...
	KifElement* MethodWriteRaw(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		if (server == true) {
			first = 1;
			if (callfunc->Size() <= 1)
				return kifcode->Returnerror("SOCKET(827): Missing parameter in send");
	...
	KifElement* MethodGetpeername(KifElement* contextualpattern, KifDomain* domain, KifCallFunction* callfunc, int idthread) {
		...
		return kifcode->Returnerror("SOCKET(852): You cannot use GETPEERNAME on the client side");
	}
...
KifElement* Kifremote::ExecMethod(short n, KifElement* contextualpattern, KifDomain* domain, int idthread, KifCallFunction* callfunc) {
	...
	if (i == functions.size())
		return kifcode->Returnerror("SOCKET(812): Unknown method");
...
extern "C" {
	...
	Exporting bool Init_kifsocket(KifCode* kifcode, string version) {
		...
		KifType res = KifAddNewType("socket", CreatekifkifsocketElement);
		if (res == kifVOID)
			return false;
		kifsocket_type = res;

		res = KifAddNewType("remote", CreatekifremoteElement);
		if (res == kifVOID)
			return false;
		remote_type = res;
	...
	Exporting bool kifsocket_KifInitialisationModule(KifCode* kifcode, string version) {
		...
		MethodInitialization("createserver", &Kifkifsocket::MethodCreateServer, P_THREE | P_TWO, "createserver(string hostname,int port,int nblients: create a server, if hostname is omitted, then the local hostname is used");
		MethodInitialization("connect", &Kifkifsocket::MethodCreateClient, P_TWO, "connect(string hostname,int port): connect to the server");
		MethodInitialization("wait", &Kifkifsocket::MethodWait, P_NONE, "wait(): wait for a client to connect and returns its socket id");
		MethodInitialization("read", &Kifkifsocket::MethodRead, P_ONE | P_NONE, "read(int num): read a string on a socket. On the server side, 'num' is the value returned by 'wait()'. Use 'read()' on the client side");
		MethodInitialization("run", &Kifkifsocket::MethodRun, P_TWO, "run(int num,string stop): Only on the server side: execute remote method invocations for the client 'num'. 'stop' is the string that can be used to stop the server by the client.");
		MethodInitialization("write", &Kifkifsocket::MethodWrite, P_ATLEASTONE, "write(int num,string s): write the string s on the socket. On the server side, num is the value returned by wait()'. Use 'write(string s)' on the client side");
		MethodInitialization("receive", &Kifkifsocket::MethodReadRaw, P_TWO | P_ONE | P_NONE, "receive(int num,int nb): read a string on a socket in a non KiF environment. On the server side, 'num' is the value returned by 'wait()'. Use 'receive()' on the client side");
		MethodInitialization("get", &Kifkifsocket::MethodGet, P_ONE | P_NONE, "get(int num): get one character from a socket in a non KiF environment. On the server side, 'num' is the value returned by 'wait()'. Use 'get()' on the client side");
		MethodInitialization("send", &Kifkifsocket::MethodWriteRaw, P_ATLEASTONE, "send(int num,string s): write the string s on the socket in a non KiF environment. On the server side, num is the value returned by wait()'. Use 'send(string s)' on the client side");
		MethodInitialization("close", &Kifkifsocket::MethodClose, P_NONE | P_ONE, "close(int num): Close a socket. On the server side, if 'num' is provided (it is the value returned by wait()) it closes the client socket, otherwise it closes the current socket.");
		MethodInitialization("blocking", &Kifkifsocket::MethodBlock, P_ONE, "blocking(bool flag): if 'flag' is true, the socket works in 'blocking' mode, otherwise in 'non blocking' mode");
		MethodInitialization("settimeout", &Kifkifsocket::MethodTimeout, P_ONE, "settimeout(int t): Set a time out of 't' seconds on the socket");
		MethodInitialization("gethostname", &Kifkifsocket::MethodGethostname, P_NONE, "gethostname(): return the current host name");
		MethodInitialization("port", &Kifkifsocket::MethodPort, P_NONE, "port(): return the current port number");
		MethodInitialization("getpeername", &Kifkifsocket::MethodGetpeername, P_ONE, "getpeername(int num): return the current peer name on the socket 'num'");
		MethodInitialization("getframe", &Kifkifsocket::MethodGetframe, P_ONE, "getframe(string frame): return a frame object handle declared on the server side");
		MethodInitialization("getfunction", &Kifkifsocket::MethodGetfunction, P_ONE, "getfunction(string frame): return a function handle declared on the server side");
