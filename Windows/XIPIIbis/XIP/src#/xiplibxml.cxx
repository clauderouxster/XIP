...
void GlobalParseurXML::BuildNodeLeaf(Liste* pere,xmlNodePtr noeud,VECTA<Liste*>& TableNoeuds) {
    ...
    strcpy(surf,(char*)token->children->content);
    Trim(surf);
    TraduitChaine(surf);
    //Premiere lecture
    ...
    while (reading!=NULL) {        
        ...
        strcpy((char*)v.surface,surf);
        xmlAttrPtr attreading=reading->properties;        
        //Le premier champ correspond a lemma
        strcpy((char*)v.lemme,(char*)attreading->children->content);
...
void GlobalParseurXML::AnalyseDOCXML(xmlDocPtr doc) {
    ...
    if (strcmp((char*)noeud->name,"XIPRESULT")) {        
        if (doc->URL!=NULL && doc->URL[0])
            sprintf(Chaine_traits,"Unknown XML file type:%s: %s\n",doc->URL,noeud->name);
        else
            sprintf(Chaine_traits,"Unknown XML type: %s\n",noeud->name);
        xmlFreeDoc(doc);
        erreur(Chaine_traits);
    }
