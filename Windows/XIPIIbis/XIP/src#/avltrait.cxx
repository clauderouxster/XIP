...
trait* GlobalParseur::CreeTrait(const char* tampon) {
    if (!strcmp(tampon,CHAINES[0])) erreur(CHAINES[0]);
    trait* tr;
...
#ifdef XIPLIBXML
void Traits::ConstruitXML(GlobalParseur* parseur,xmlNodePtr node,Vecteur* filtretraits) {
    valeur->ConstruitXML(parseur,node,filtretraits);
    if (onto!=NULL && onto->cell(ONTOLOGIE_COMPARAISON)!=NULL)
        onto->cell(ONTOLOGIE_COMPARAISON)->ConstruitXML(parseur,parseur->hierarchies->lahierarchie,node,parseur->TraitsFiltreOntologie);
}
#endif
