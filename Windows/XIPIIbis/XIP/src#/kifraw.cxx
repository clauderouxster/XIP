...
#define conversion2dos(x) dosoutput?s_utf8todos(USTR(x)):x
#endif
...
public:
	...
	KifComparisonString(KifDomain* dom, KifFunction* comp) : li(NULL, NULL, ""), lj(NULL, NULL, ""){
		domain = dom;
		compare = comp;
		li.kifcode = domain->kifcode;
		lj.kifcode = domain->kifcode;
	}
...
public:
	...
	KifComparisonInteger(KifDomain* dom, KifFunction* comp) : li(NULL, NULL, 0), lj(NULL, NULL, 0){
		domain = dom;
		compare = comp;
		li.kifcode = domain->kifcode;
		lj.kifcode = domain->kifcode;
...
public:
	...
	KifComparisonFloat(KifDomain* dom, KifFunction* comp) : li(NULL, NULL, 0), lj(NULL, NULL, 0){
		domain = dom;
		compare = comp;
		li.kifcode = domain->kifcode;
		lj.kifcode = domain->kifcode;
...
KifElement* PreSetDebugFunction(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (kfunc->isCall() || kfunc->isFunction()) {
		...
		if (object->type == kifInstance)
			object = ((KifInstance*)object)->Thevariable(dom, kifGlobals->GetThreadid(), NULL);
		else
			object = object->Execute(kifNULL, dom);
	...
	for (i = 0; i<kifGlobals->garbage.size(); i++) {
		if (kifGlobals->garbage[i] != NULL) {
			if (kifGlobals->garbage[i]->reference>0) {
				if (v.find(kifGlobals->garbage[i]->type) == v.end())
					v[kifGlobals->garbage[i]->type] = 1;
				else
					v[kifGlobals->garbage[i]->type]++;
			}
			else {
				if (tobecleaned.find(kifGlobals->garbage[i]->type) == tobecleaned.end())
					tobecleaned[kifGlobals->garbage[i]->type] = 1;
				else
					tobecleaned[kifGlobals->garbage[i]->type]++;
			}
		}
		else
			tobecleaned[kifVOID]++;
...
KifElement* PreTestUTF8(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (c_is_utf8(USTR(str), str.size()) == true)
		return kifTRUE;
	return kifFALSE;
}
...
KifElement* PreEvaluateMeta(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	static string reg("[\\\\[%d%d%d,x%x%x,u%X%X%X%X,?],&[%#%d(%d)(%d)(%d);,[%c,%C]+;],%%%X%X]");
...
KifElement* PreFind(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (contextualpattern->aVectorContainer()) {
		...
		if (str == "")
			return kifMINUSONE;
		bool utf8 = c_is_utf8(USTR(str), str.size());
		if (utf8)
			i = c_chartobytepositionidx(USTR(str), i, nb, ps);
		int j = str.find(sub, i);
		if (j != -1) {
			...
			case kifVectorInteger:
				while (j != -1) {
					if (utf8) {
						i = c_bytetocharpositionidx(USTR(str), j, nb, ps);
						((KifVectorInteger*)kvect)->values.push_back(i);
					}
					else
						((KifVectorInteger*)kvect)->values.push_back(j);
					j = str.find(sub, j + 1);
				}
				return kvect;
			case kifVectorFloat:
			...
			while (j != -1) {
				if (utf8)
					loc.value == c_bytetocharpositionidx(USTR(str), j, nb, ps);
				else
					loc.value = j;
				kvect->Push(&loc);
				j = str.find(sub, j + 1);
	...
	agnostring str(kbasecall->String());
	i = str.find(sub, i);
	if (i == string::npos)
		return kifMINUSONE;
...
KifElement* PreSplit(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	KifElement* kvect;
	string thestr = callfunc->Evaluatethread(0, dom, idthread)->String();
	string thesplitter;
	if (callfunc->Size() != 1)
...
KifElement* PreMath(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	double v = callfunc->Evaluatethread(0, dom, idthread)->Float();
...
KifElement* PreOrd(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (sz >= 1) {
		if (contextualpattern->type == kifInteger || contextualpattern->type == kifFloat)
			return new KifLong(callfunc->kifcode, NULL, s.code(0));
		...
		if (contextualpattern->aVectorContainer() || sz>1) {
			...
			if (kvect->type == kifVectorInteger) {
				KifContainerLock _lock((KifEvaluate*)kvect);
				s.unicodes(((KifVectorInteger*)kvect)->values);
			}
			else {
				vector<int> vect;
				s.unicodes(vect);
...
KifElement* PreWaitOnFalse(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	KifElement* var = callfunc->parameters[0]->Instruction(0);
	if (var->type != kifInstance)
		callfunc->kifcode->Returnerror("KIF(877): Expecting a variable");
	var = ((KifInstance*)var)->Thevariable(dom, kifGlobals->GetThreadid(), NULL);
...
KifElement* PreWrite(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	for (size_t i = 1; i<callfunc->Size(); i++) {
		res = callfunc->parameters[i]->ExecuteRaw(kifTypeSTRING, dom);
...
KifElement* PreWriteBin(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	KifBaseFile* kfile = (KifBaseFile*)kf;
	for (size_t i = 1; i<callfunc->Size(); i++)
		kfile->WriteBin(callfunc->Evaluatethread(i, dom, idthread)->Byte());
	return kifTRUE;
}
...
KifElement* PreRead(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	KifBaseString* line = kfile->localstring;
	...
	case kifVectorInteger:
		while (kfile->EndOfFile() != kifTRUE) {
			c = kfile->Get();
			((KifVectorInteger*)res)->values.push_back(c);
			counter++;
			if (counter == nb)
				break;
		}
	...
	case kifVectorFloat:
		while (kfile->EndOfFile() != kifTRUE) {
			c = kfile->Get();
			((KifVectorFloat*)res)->values.push_back(c);
			counter++;
			if (counter == nb)
				break;
		}
	...
	case kifVectorString:
		while (kfile->EndOfFile() != kifTRUE) {
			kfile->Readoneline();
			((KifVectorString*)res)->values.push_back(line->value);
			counter++;
			if (counter == nb)
				break;
		}
...
KifElement* PreInsert(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (kf->isString()) {
		...
		string val = callfunc->Evaluatethread(2, dom, idthread)->String();
		if (idx == -1)
...
KifElement* PreSort(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (kf->type == kifVectorString && callfunc->Size() == 2) {
		...
		kf = callfunc->Evaluatethread(1, dom, idthread)->Functioncall(dom);
		if (kf != NULL && kf->isFunction()) {
			KifComparisonString kfcomp(dom, (KifFunction*)kf);
			sort(kvect->values.begin(), kvect->values.end(), kfcomp);
		}
		return kvect;
	}
	...
	if (kf->type == kifVectorInteger && callfunc->Size() == 2) {
		...
		kf = callfunc->Evaluatethread(1, dom, idthread)->Functioncall(dom);
		if (kf != NULL && kf->isFunction()) {
			KifComparisonInteger kfcomp(dom, (KifFunction*)kf);
			sort(kvect->values.begin(), kvect->values.end(), kfcomp);
		}
		return kvect;
	}
	...
	if (kf->type == kifVectorFloat && callfunc->Size() == 2) {
		...
		kf = callfunc->Evaluatethread(1, dom, idthread)->Functioncall(dom);
		if (kf != NULL && kf->isFunction()) {
			KifComparisonFloat kfcomp(dom, (KifFunction*)kf);
			sort(kvect->values.begin(), kvect->values.end(), kfcomp);
		}
		return kvect;
	}
...
#else
	if (knum->type == kifInteger) {
		if (sprintf_s(buffer, 100, STR(sformat), knum->Integer())<0)
			return callfunc->kifcode->Returnerror("KIF(124):Incorrect format specifier or size too long");
	}
	else
	if (sprintf_s(buffer, 100, STR(sformat), knum->Float())<0)
		return callfunc->kifcode->Returnerror("KIF(124):Incorrect format specifier or size too long");
#endif
	return callfunc->kifcode->Providestringraw(buffer);
}
...
KifElement* PreAddword(KifElement* pattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	agnostring str(callfunc->Evaluatethread(1, dom, idthread)->String());
	if (callfunc->Size() == 2)
		((KifAutomaton*)ke)->addword(str);
	else {
		agnostring lemma(callfunc->Evaluatethread(2, dom, idthread)->String());
		((KifAutomaton*)ke)->addworddouble(str, lemma);
	}
	return kifTRUE;
}
...
KifElement* PreGet(KifElement* pattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (kfirst->isString() && dosoutput) {
		string txt = s_dostoutf8(USTR(srep));
