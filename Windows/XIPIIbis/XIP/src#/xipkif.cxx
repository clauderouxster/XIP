...
hmap<short,bool> kifFSTFunctions;
hmap<short,bool> kifNTMFunctions;
hmap<short,bool> kifParserFunctions;
hmap<short,bool> kifXMLFunctions;
hmap<short,bool> kifXipFunctions;
hmap<short,bool> kifGraphFunctions;
hmap<short,bool> kifDependencyFunctions;
hmap<short,bool> kifGenerationFunctions;

static  map<string,string> kifInformationsNode;
static  map<string,string> kifInformationsDependency;
static  map<string,string> kifInformationsGeneration;
static  map<string,string> kifInformationsGraph;
static  map<string,string> kifInformationsNtm;
static  map<string,string> kifInformationsFst;
static  map<string,string> kifInformationsXml;
static  map<string,string> kifInformationsParser;
...
KifElement* ParseurPreParseFiles(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	localstring.kifcode=callfunc->kifcode;
	int i;
	KifVector* kvect=callfunc->kifcode->Providevector();
	for (i=0;i<features.size();i++) {
		localstring.value=features[i];
		kvect->Push(&localstring);
	}
	kmap->Push("features",kvect);
	kvect=callfunc->kifcode->Providevector();
...
KifElement* ParseurPreParseSetOption(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	for (int i=1;i<callfunc->Size();i++) {
		...
		default:
			XIPLONG v=Un<<option;
			XipAddFlagDisplayMode(kpars->id,v);
		}
	}
	return kifTRUE;
}
...
KifElement* ParseurPreParseRemoveOption(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	for (int i=1;i<callfunc->Size();i++) {
		...
		default:
			XIPLONG v=Un<<option;
			XipRemoveFlagDisplayMode(kpars->id,v);
		}
	}
	return kifTRUE;
}
...
KifElement* ParseurPreParse(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	if (callfunc->Size()==3) {
		KifElement* kinst=callfunc->parameters[2]->Instruction(0);
...
KifElement* ParseurPreParseXMLFile(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	if (callfunc->Size()==5) {
		...
		if (leParseur->AppelExterne!=NULL) {
			kof=callfunc->Evaluatethread(2,dom,idthread);
			XipResult* xip=XipCreateCurrentXipResult(kpars->id);
			if (XipParseXMLFile(kpars->id,ks->String(),kof->Integer(),xip)==0)
				return kifTRUE;
			return kifFALSE;
		}
		else
			return callfunc->kifcode->Returnerror(kifErrorStrings[180]);
	}
	if (of==NULL) {
	...
	int res=XipParseFileXMLToFile(ks->String(),kpars->id, of,kof->Integer(),XML_NONE,1);
	delete of;
	if (res==0)
		return kifTRUE;
	return kifFALSE;
...
KifElement* ParseurPreCreateDependency(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	for (itr->Begin();itr->End()==kifFALSE;itr->Next()) {
		string att=c_Tolower(itr->IteratorKeyString());
		string val=c_Tolower(itr->IteratorValueString());
...
KifElement* ParseurPreSetFeature(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	for (itr->Begin();itr->End()==kifFALSE;itr->Next()) {
		att=c_Tolower(itr->IteratorKeyString());
...
KifElement* ParseurPreGetData(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	for (itr->Begin();itr->End()==kifFALSE;itr->Next()) {
		att=c_Tolower(itr->IteratorKeyString());
		vval=c_Tolower(itr->IteratorValueString());
...
KifElement* ParseurPreReadings(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	for (int i=0;i<voc->X.size();i++) {
		smap=new KifMapString(callfunc->kifcode,NULL); 
...
bool KifGraph::Setvalue(KifElement* a,KifElement* index,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (a->type!=kifGraph)
		return true;
	KifGraph* g=(KifGraph*)a;
	graph=g->graph;
...
void KifXipNode::Constructionarbre(Noeud* n,KifVector* v) {
	KifMap* nodestructure=new KifMap(kifcode,NULL);
	//we populate it...
	MapFeatureNode(n->X->courant,n,nodestructure);
	nodestructure->Push("#POS",v->kifcode->Providestringraw(n->X->nom));
	nodestructure->Push("#ID",kifcode->Provideinteger(n->num_noeud));
	v->Push(nodestructure);
	if (n->fils!=NULL) {
		KifVector* subvect=kifcode->Providevector();
		nodestructure->Push("#CHILD",subvect);
...
KifElement* KifXipNode::Vector(KifElement* dom,KifVector* v) {
	...
	nodestructure->Push("#POS",kifcode->Providestringraw(value->X->nom));
	nodestructure->Push("#ID",kifcode->Provideinteger(value->num_noeud));
...
KifElement* KifXipNode::Map(KifElement* dom,KifMap* v) {
	KifMap* nodestructure=v;
	if (nodestructure==NULL)
		nodestructure=kifcode->Providemap();
	else
		nodestructure->Clear();
	//we populate it...
	MapFeatureNode(value->X->courant,value,nodestructure);
...
bool KifXipNode::Setvalue(KifElement* a,KifElement* index,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (a==this)
		return true;
...
bool KifXipDependency::Setvalue(KifElement* a,KifElement* index,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (a==this)
		return true;
	if (a==kifNULL || a==kifNOELEMENT) {
		value=NULL;
...
bool KifXipGeneration::Setvalue(KifElement* a,KifElement* index,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (a==this)
		return true;
	if (a==kifNULL || a==kifNOELEMENT) {
		value=NULL;
...
bool KifXMLNodePtr::Setvalue(KifElement* a,KifElement* index,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (a==kifNULL || a==kifNOELEMENT) {
		if (value!=NULL)
			value->noeud=NULL;
		return true;
...
bool KifVectorNumberPtr::Setvalue(KifElement* ke,KifElement* idx,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	ThreadLock _lock(type);
	if (ke==this)
		return true;
...
bool KifVectorStringPtr::Setvalue(KifElement* ke,KifElement* idx,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	ThreadLock _lock(type);
	if (ke==this)
		return true;
...
bool KifMapPtr::Setvalue(KifElement* ke,KifElement* idx,KifElement* dom,int idthread,KifCallFunction* callfunc) {
	ThreadLock _lock(type);
	if (ke==this)
		return true;
...
KifElement* KifFST::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != kifFST || b->type != kifFST)
		return kifNULL;
...
KifElement* KifFST::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != kifFST || b->type != kifFST)
		return kifNULL;
...
KifElement* KifFST::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) { //In this case, it will be used for composition
	if (a->type != kifFST || b->type != kifFST)
		return kifNULL;
...
bool KifFST::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
Exported KifElement* KifXipNode::Information(KifElement* kstr) {
	...
	if (kifInformationsNode.find(s)==kifInformationsNode.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsNode[s]);
}

Exported KifElement* KifXipDependency::Information(KifElement* kstr) {
	string s=kstr->String();
	if (kifInformationsDependency.find(s)==kifInformationsDependency.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsDependency[s]);
}

Exported KifElement* KifXipGeneration::Information(KifElement* kstr) {
	string s=kstr->String();
	if (kifInformationsGeneration.find(s)==kifInformationsGeneration.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsGeneration[s]);
}

Exported KifElement* KifGraph::Information(KifElement* kstr) {
	string s=kstr->String();
	if (kifInformationsGraph.find(s)==kifInformationsGraph.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsGraph[s]);
}

Exported KifElement* KifFST::Information(KifElement* kstr) {
	string s=kstr->String();
	if (kifInformationsFst.find(s)==kifInformationsFst.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsFst[s]);
}

Exported KifElement* KifNTM::Information(KifElement* kstr) {
	string s=kstr->String();
	if (kifInformationsNtm.find(s)==kifInformationsNtm.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsNtm[s]);
}


Exported KifElement* KifXMLNodePtr::Information(KifElement* kstr) {
	string s=kstr->String();
	if (kifInformationsXml.find(s)==kifInformationsXml.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsXml[s]);
}


Exported KifElement* KifParser::Information(KifElement* kstr) {
	string s=kstr->String();
	if (kifInformationsParser.find(s)==kifInformationsParser.end())
		return kifNULL;
	return kifcode->Providestringraw(kifInformationsParser[s]);
}
//-------------------------------------------------------------------------------
...
KifElement* KifGraph::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* ke) {
...
KifElement* KifXipNode::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* idx) {
...
KifElement* KifXipDependency::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* idx) {
    if (idx!=NULL && !idx->isConst()) {
        if (value==NULL)
            return kifNOELEMENT;
        GlobalParseur* parseur=Leparseur();
...
KifElement* KifXMLNodePtr::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* ke) {
	if (ke!=NULL && ke->type==kifIndex) {
...
KifElement* KifMapPtr::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* idx) {
...
KifElement* KifVectorNumberPtr::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* idx) {
	if (idx==NULL || idx==kifNULL) {
	...
	if (idx->type==kifIndex) {
		...
		key=kind->key->Execute(kifNULL,dom,kifNULL);
		if (kind->interval==true)
			keyright=kind->right->Execute(kifNULL,dom,kifNULL);
	}
	else
		key=idx->Execute(contextualpattern,dom,kifNULL);
...
KifElement* KifVectorStringPtr::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* idx) {
	if (idx==NULL || idx==kifNULL) {
...
Exportation int KifExecuteProgram(string fname,string arguments) {
...
KifElement* ParseurPreParseFeatures(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {    
	...
	for (itr->Begin();itr->End()==kifFALSE;itr->Next())
		feats.push_back(c_Tolower(itr->IteratorValueString()));
	itr->Release();
		...
		if (contextualpattern->type==kifVector) {
			...
			if (voc!=NULL) {
				for (int i=0;i<voc->lemme.dernier;i++) {
					localstring.value=voc->lemme[i];
					vect->Push(&localstring);
				}
			}
			else {
				localstring.value=KifLemma(knode->value);
				vect->Push(&localstring);
			}
			return vect;
		}
...
KifElement* ParseurPreReplace(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	str=c_replacestring(str,reg,rep);
...
static int comptearc=0;



void parcoursdesarcsfst(NETptr net,int max_visits,KifString& kstr,KifCallFunctionLocal* callback,KifElement* kval,KifDomain* dom,bool lower) {
	/***********
	*
		...
		if (STATE_final(state) == TRUE && arc == STATE_arc_set(state)) {
			...
			*/
			if (STATE_visit_mark(state) <= 1) {
				...
				if (callback->Execute(kifNULL,dom)->Boolean()==false)
					break;
		...
		if (arc != NULL) {
			...
			if (!(ARC_label(arc) == EPSILON ||
				ARC_label(arc) == ALTCHAIN_SYMBOL)) {
					...
					lab=id_to_label(id);
					if (lower)
						copy_label_name(id, LOWER, carac, 99, TRUE, FALSE);				
					else
						copy_label_name(id, UPPER, carac, 99, TRUE, FALSE);
...
bool traversefsttrans(char* fst,bool lower,KifCallFunctionLocal* callback,KifElement* kval,KifDomain* dom) {
	...
	kval->Setreference();
	parcoursdesarcsfst(nv->nets[0],10,kstr,callback,kval,dom,lower);
	kval->Resetreference();
	return true;
}
...
KifElement* ParseurPreInvertFst(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (kf->type != kifFST) {
		if (kf->aMapContainer()) {
			KifElement* res;
			if (contextualpattern != NULL && contextualpattern->reference <= 1 && contextualpattern != callfunc && contextualpattern->aMapContainer() && contextualpattern->Evaluate()) {
...
KifElement* ParseurPreTraverseFst(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	KifCallFunctionLocal kfunccall(func->kifcode,func->Name(),func);
	if (!traversefsttrans(STR(sfilename),lower,&kfunccall,kval,dom))
		return kifFALSE;
	return kifTRUE;
}
...
KifElement* ParseurPreTokup(KifElement* contextualpattern,KifDomain* dom,KifCallFunction* callfunc,int idthread) {
	...
	KifFST* kfst=(KifFST*)ke;
	GlobalParseur* parseur=kfst->tok->parseur;
	if (parseur==NULL) {
...
Exported KifElement* KifFloatPtr::divide(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
	double ib=b->Float();
	if (ib==0)
...
Exported KifElement* KifFloatPtr::power(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
	if (autoself) {
...
Exported bool KifFloatPtr::Setvalue(KifElement* ke,KifElement* index,KifElement* dom,int idthread,KifCallFunction* callfunc) {	
	*valueptr=ke->Float();
	return true;
}
...
bool KifStringPtr::Setvalue(KifElement* ke,KifElement* idx,KifElement* dom,int idthread,KifCallFunction* callfunc) {
#ifdef PROFILING
...
KifElement* KifStringPtr::Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* idx) {
	KifBaseString f(NULL,NULL,*valueptr);
	f.kifcode=kifcode;
	return f.Execute(contextualpattern,dom,idx);
}
...
void ParseurKifInit(KifCode* kifcodeptr) {
	...
	kifGlobals->kifTypeString["string"]=kifStringPtr;

#ifdef NTMCONNECTION
	KifRecreatePredefined(kifcodeptr, "traverse", P_FIVE, &ParseurPreTraverseFst);
	...
	KifRecreatePredefined(kifcodeptr,"add",P_THREE,&ParseurPreCompileline);
	...
	KifRecreatePredefined(kifcodeptr,"lookup",P_THREE|P_TWO,&ParseurPreLookup);
	...
	InitMapBooleanString(kifXipFunctions,"next");
	InitMapBooleanString(kifXipFunctions,"sister");
	InitMapBooleanString(kifXipFunctions,"last");
	InitMapBooleanString(kifXipFunctions,"previous");
	InitMapBooleanString(kifXipFunctions,"parent");
	InitMapBooleanString(kifXipFunctions,"data");
	InitMapBooleanString(kifXipFunctions,"features");
	InitMapBooleanString(kifXipFunctions,"rulenumber");
	InitMapBooleanString(kifXipFunctions,"readings");
	InitMapBooleanString(kifXipFunctions,"name");
	InitMapBooleanString(kifXipFunctions,"pos");
	InitMapBooleanString(kifXipFunctions,"vector");
	InitMapBooleanString(kifXipFunctions,"map");
	InitMapBooleanString(kifXipFunctions,"child");
	InitMapBooleanString(kifXipFunctions,"xmlnode");
	InitMapBooleanString(kifXipFunctions,"surface");
	InitMapBooleanString(kifXipFunctions,"nodeinfos");
	InitMapBooleanString(kifXipFunctions,"lemma");
	InitMapBooleanString(kifXipFunctions,"setfeature");
	InitMapBooleanString(kifXipFunctions,"feature");
	InitMapBooleanString(kifXipFunctions,"sentence");
	InitMapBooleanString(kifXipFunctions,"removefeature");
	InitMapBooleanString(kifXipFunctions,"isa",false);
	InitMapBooleanString(kifXipFunctions,"methods",false);
	InitMapBooleanString(kifXipFunctions,"infos",false);
	InitMapBooleanString(kifXipFunctions,"type",false);
	InitMapBooleanString(kifXipFunctions,"number");
	InitMapBooleanString(kifXipFunctions,"offset");
	InitMapBooleanString(kifXipFunctions,"offsetchar");
	InitMapBooleanString(kifXipFunctions,"tokenoffset");
	InitMapBooleanString(kifXipFunctions,"leftoffset");
	InitMapBooleanString(kifXipFunctions,"leftoffsetchar");
	InitMapBooleanString(kifXipFunctions,"lefttokenoffset");
	InitMapBooleanString(kifXipFunctions,"rightoffset");
	InitMapBooleanString(kifXipFunctions,"rightoffsetchar");
	InitMapBooleanString(kifXipFunctions,"righttokenoffset");
	InitMapBooleanString(kifXipFunctions,"string");
	InitMapBooleanString(kifXipFunctions,"int");
	InitMapBooleanString(kifXipFunctions,"float");
	InitMapBooleanString(kifXipFunctions,"boolean");
	...
	kifInformationsNode["vector"]="vector(): return the node tree as a vector";
	kifInformationsNode["map"]="map(): return the node tree as a map";
	...
	kifInformationsNode["rulenumber"]="rulenumber: Return the number of the rule that was used to create that node";
	kifInformationsNode["setfeature"]="setfeature(string att, string val): feature assignment to the node";
	kifInformationsNode["removefeature"]="removefeature(string att): remove the feature from node whose attribute is att";
	kifInformationsNode["sentence"] = "sentence(): return the sentence spanned by a node.";
	...
	kifInformationsNode["features"] = "features(): return a smap of all features.\nfeatures(smap f): set the features from a smap";
	kifInformationsNode["readings"]="readings(): return a smap of all possible readings for a given word";
	kifInformationsNode["next"]="next(): return the next node after current node. Can also return the next node if the structure is flat from the sentence.";
	kifInformationsNode["sister"]="sister(): return the next node after current node";
	...
	kifInformationsNode["nodeinfos"]="nodeinfos(): return the node infos value.\tnodeinfos(string v): set the node infos value.";
	...
	kifInformationsDependency["rulenumber"]="rulenumber: Return the numbers of the rule that were used to create that dependency";
	...
	kifInformationsDependency["features"]="features(): return a smap of all features.\nfeatures(smap f): set the features from a smap";
	kifInformationsDependency["parameters"]="parameters(vector v): return a vector of node variable";
	...
	kifInformationsGeneration["rulenumber"]="rulenumber: Return the numbers of the rule that were used to create that dependency";
	...
	kifInformationsGraph["match"]="match(extractor): extractor should be a graph. We return a vector of all sub-graphs from the current graph that match extractor.";
	...
	kifInformationsFst["loadfst"]="loadfst(string fst, string flags, int strategy,boolean utf8): load an FST file. Strategy=0 for a DEPTH strategy, 1 for a BREADTH strategy.";
	...
	kifInformationsFst["traverse"] = "traverse(string pathname, function f,imap grds,boolean lower): traverse a FST with f(string s,imap grds) called for each new token found in the FST of name pathname, on the FST lower side if lower is true.";
	kifInformationsFst["loadspacedtext"] = "loadspacedtext(string filename,bool utf8): load a text file in which words are stored according to the FST spaced-text format.";
	kifInformationsFst["loadregex"] = "loadregex(string file,bool utf8): Load a file containing a regular expression.";
	kifInformationsFst["compilescript"] = "compilescript(string file,bool utf8): Compile a file containing regular expressions.";
	kifInformationsFst["negate"] = "negate(): Return the negation of the current net.";
	kifInformationsFst["minimize"] = "minimize(): Minimize the current net.";
	kifInformationsFst["optimize"] = "optimize(): Optimize the current net.";
	kifInformationsFst["unoptimize"] = "unoptimize(): Unoptimize the current net.";
	kifInformationsFst["invert"] = "invert(): Invert the current net.";
	kifInformationsFst["vectorize"] = "vectorize(): Vectorize the current net.";
	kifInformationsFst["unvectorize"] = "unvectorize(): Unvectorize the current net.";
	kifInformationsFst["compact"] = "compact(int option): Compact the current net: 1 is SMALLEST, 2 is SMALL, 3 is FAST, 4 is FASTEST.";
	kifInformationsFst["uncompact"] = "uncompact(): Uncompact the current net.";
	kifInformationsNtm["lookupstring"] = "lookupstring(string sentence): Lookup a string with NTM.";
	kifInformationsNtm["lookupfile"]="lookupfile(string filename): Lookup a file with NTM.";
	kifInformationsNtm["setsepconstraint"]="setsepconstraint(bool bychar): Set the constraint separator to BYCHAR is true, or BYTOKEN if false";
	kifInformationsNtm["setoptions"]="setoptions(map options): Set the options";
	kifInformationsNtm["loadntm"]="loadntm(string name): Load NTM file";
	kifInformationsXml["name"]="name(): return the XML node name";
	kifInformationsXml["properties"]="properties(): Properties are stored in map as attribute/value";
	...
	kifInformationsParser["reload"]="reload(): Reload a XIP grammar.";
	kifInformationsParser["grammarfiles"]="map m=grammarfiles(): Return all files for a loaded grammar.";
	kifInformationsParser["categories"]="categories(): Return a vector of all category names.";
	kifInformationsParser["dependencies"]="dependencies(): Return a vector of dependency names.";
	kifInformationsParser["lemmafeature"]="lemmafeature(): Return the lemma feature name in a grammar.";
	kifInformationsParser["surfacefeature"]="surfacefeature(): Return the surface feature name in a grammar.";
	kifInformationsParser["parse"]="parse(string sentence,kif_exchange_data): parse a sentence using the grammar. If no variable is provided to get the analysis, then the result is displayed on the current output.";
	kifInformationsParser["setcatchxml"]="setcatchxml(function): Set the catch function, which will be used in an XML analysis to catch the 'string', which will be parsed beforehand.";
	kifInformationsParser["parsefeatures"]="parsefeatures(svector featinits,svector featres): Return the translation of the features from featinits in featres. This function also returns a POS, if found in featinits";
	kifInformationsParser["getgrammar"]="getgrammar(int handle): Get the current parser object to point to an existing grammar.";
	kifInformationsParser["sentencenumber"]="sentencenumber(): return the current number of the sentence being processed.";
	kifInformationsParser["parsestructure"]="parsestructure(map|vector tree): parse a predefined tree using the grammar. The tree can also be a vector. The structure should respect the output of map or of vector on a 'node'";
	kifInformationsParser["lookup"]="lookup(string sentence,kif_exchange_data): Lookup of a sentence using the grammar FSTs. If no variable is provided to get the analysis, then the result is displayed on the current output.";
	kifInformationsParser["buffer"]="buffer(): Return the internal string buffer.";
	kifInformationsParser["generaterule"]="generaterule(string dependencyname,vector focus,vector nodes,vector nodefeatures,vector dependencies,vector dependencyfeatures,bool adquationmark,int ruletype): generate a rule based on a vector of nodes and a vector of dependencies. If addquestionmark is true, then in case of gaps, the system inserts ?+";
	kifInformationsParser["parsefile"]="parsefile(string input,kif_exchange_data,string output): parse the file input and store the results in output.\r\rparsefile(string input,kif_exchange_data): parse the file input within a XIP library call. A callback function must be provided in that case to catch the XipResult objects.";
	kifInformationsParser["parsexml"] = "parsexml(string input,kif_exchange_data,string output,int depth): parse the XML file input and store the results in file output. Depth is the depth at which the analysis should take place.\r\rparsexml(string input,kif_exchange_data, int depth): parse the XML file input A callback function must be provided in that case to catch the XipResult objects.";
	kifInformationsParser["parsexmlstring"] = "parsexmlstring(string input,kif_exchange_data,int depth): parse the XML string input and store the results in file output. Depth is the depth at which the analysis should take place.";
	kifInformationsParser["generatefromstring"] = "generatefromstring(string input,kif_exchange_data): Generate a sentence out of a dependency string";
	kifInformationsParser["generatefromfile"]="generatefromfile(string pathname,kif_exchange_data): Generate a sentence out of a dependency file";
	kifInformationsParser["texttoxml"]="texttoxml(string file,kif_exchange_data, string tag, string encoding): transform a text file into an XML file, using 'tag' as the new xml root and 'encoding', which can take either 'latin' or 'utf8' as a value";
	kifInformationsParser["addendum"]="addendum(string filename): Enrich a grammar with an external parameter file.";
	kifInformationsParser["setbreakpoint"]="setbreakpoint(int breakpoint): Set a breakpoint in a grammar.";
	kifInformationsParser["name"]="name(): Return the pathname of the grm file";
	kifInformationsParser["setoption"]="setoption(int nb1,int nb2,etc.): Set the options of ids: nb1,nb2 etc. See below for a list of these options.";
	kifInformationsParser["addoption"]="addoption(int nb1,int nb2,etc.): Add the options of ids: nb1,nb2 etc. See below for a list of these options.";
	kifInformationsParser["removeoption"]="removeoption(int nb1,int nb2,etc.): Remove the options of ids: nb1,nb2, nb3 etc.";	
