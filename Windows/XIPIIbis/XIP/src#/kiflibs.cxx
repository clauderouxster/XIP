...
Exported string Whoami_kif() {
	...
	s += KIF_VERSION;
	s += Endl;
	s += "Copyright (C) Xerox Corporation 2010-2014";
	s += Endl;
...
bool KifCode::KifLoadModule(x_node* xn, string n, string& library_name) {
	...
	if (LoadMe == 0) {
		...
		if (getenv("KIFLIBS") != NULL)
			...
			lname = "KIF(999): Please set KIFLIBS";
...
bool KifCode::KifLoadModule(x_node* xn, string n, string& library_name) {
	...
	if (LoadMe == NULL) {
		...
		if (getenv("KIFLIBS") != NULL)
			...
			basename = "KIF(999): Please set KIFLIBS";
...
KifElement* KifCode::KifParseSwitch(x_node* xn, KifElement* kf) {
	...
	if (xn->nodes[1]->token == "depend") {
		...
		if (kfunc->isFunction()) {
			if (kfunc->Size() != 2) {
				string message = "KIF(881): Error, wrong function arguments";
			...
			if (!kfunc->isCall()) {
				string message = "Unknown function: '" + kifGlobals->kifIdString[kfunc->Name()] + "'";
				throw new KifRaiseError(message, kifGlobals->current->name, kifGlobals->current_start, kifGlobals->current_end);
			}
			KifInstance* ki;
			int idfunc = kfunc->Name();
			if (kifGlobals->kconstants->declarations.find(idfunc) != kifGlobals->kconstants->declarations.end()) {
...
KifElement* KifCode::KifParseTryCatch(x_node* xn, KifElement* kf) {
	...
	if (xn->nodes.size() != 1 && xn->nodes[1]->token == "word") {
		...
		if (kifGlobals->kconstants->declarations.find(id) != kifGlobals->kconstants->declarations.end()) {
...
KifElement* KifCode::KifParseForIn(x_node* xn, KifElement* kf) {
	...
	if (kforin->type == kifInstructionFORINRANGE) {
		if (kin->Instruction(0)->type != kifInstance) {
			string message = "Expecting a variable in FOR";
		...
		if (tyvar != kifInteger && tyvar != kifFloat && tyvar != kifLong) {
			string message = "Only numerical variable can be used here";
			throw new KifRaiseError(message, kifGlobals->current->name, kforin->line, xn->end);
		}
...
KifElement* KifCode::KifParseCall(x_node* xn, KifElement* kf) {
	...
	if (name == "setstacklimit") {
		//very specific case, to set the stack size from within
		if (xn->nodes.size() == 2) {
			KifInstruction kxf(NULL, NULL);
			kxf.kifcode = NULL;
			KifBrowse(xn->nodes[1], &kxf);
			res = kxf.instructions[0]->Execute(kifNULL, kstart);
			long sz = res->Integer();
			kifGlobals->stacklimit = sz;
		}
		else {
			string message = "Missing parameter in 'setstacklimit'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}

	if (name == "setjoinedlimit") {
		//very specific case, to set the stack size from within
		if (xn->nodes.size() == 2) {
			KifInstruction kxf(NULL, NULL);
			kxf.kifcode = NULL;
			...
			string message = "Missing parameter in 'setjoinedlimit'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}

	if (name == "setthreadlimit") {
		//very specific case, to set the stack size from within
		if (xn->nodes.size() == 2) {
			KifInstruction kxf(NULL, NULL);
			kxf.kifcode = NULL;
			...
			string message = "Missing parameter in 'setthreadlimit'";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		return kf;
	}
		...
		if (xn->nodes.size() == 2) {
			...
			if (xn->nodes[1]->nodes.size() == 2) {
				...
				if (xn->nodes[1]->nodes[1]->nodes.size() == 2) {
					...
					arg = kxf.instructions[0]->Execute(kifNULL, kstart);
					KifInstruction kxp(NULL, NULL);
					kxp.kifcode = NULL;
					KifBrowse(xn->nodes[1]->nodes[1]->nodes[0], &kxp);
					platform = kxp.instructions[0]->Execute(kifNULL, kstart);
					if (arg->String() == platform->String()) {
						KifBrowse(xn->nodes[1]->nodes[1]->nodes[1], &kxl);
						pathlib = kxl.instructions[0]->Execute(kifNULL, kstart);
					}
					else
						load = false;
				}
					...
					KifBrowse(xn->nodes[1]->nodes[0], &kxf);
					platform = kxf.instructions[0]->Execute(kifNULL, kstart);
				...
				KifBrowse(xn->nodes[1], &kxf);
				pathlib = kxf.instructions[0]->Execute(kifNULL, kstart);
			}
	...
	if (name == "loadin") {
		...
		if (xn->nodes.size() == 2) {
			...
			if (res->Boolean()) {
				...
				kxf.kifcode = NULL;
				KifBrowse(xn->nodes[1]->nodes[0], &kxf);
				res = kxf.instructions[0]->Exec(kifNULL, kstart, kifNULL, 0, NULL);
	...
	if (kfunc != NULL) {
		if (kfunc->type == kifVariable) {
			...
			if (kfunc != NULL && kfunc->type == kifCall)
				...
				string message = "Unknown function call: ";
				message += name;
				throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
			}
		}
...
KifElement* KifCode::KifParseVariable(x_node* xn, KifElement* kf) {
	...
	if (declaration->isValidVariable()) {
		...
		string message = "Invalid variable: '" + name + "'";
		throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
	}
...
KifElement* KifCode::KifParseAXNumber(x_node* xn, KifElement* kf) {
	string& name = xn->value;
	KifElement* kv;
	KIFLONG v = s_XLConvert(STR(name));
	if (v> ((1 << 8) - 1))
...
KifElement* KifCode::KifParseVarPredicate(x_node* xn, KifElement* kf) {
	...
	KifElement* var = CreateVariable(xn, id, kf, kifNOTYPE);
	KifElement* kv;
	if (xn->nodes[0]->value == "predicatevar")
...
KifElement* KifCode::KifParseVarPredicateTerm(x_node* xn, KifElement* kf) {
	short id = KifNewId(xn->nodes[1]->value);
	KifElement* var = CreateVariable(xn, id, kf, kifNOTYPE);
	KifPredicateTerm* kv = new KifPredicateTerm(this, var, id, xn->nodes[1]->value);
	terms[id] = kv;
	kv->Setline(linereference + GetCurrentLine(0, xn->nodes[1]), kifGlobals->debuginfocurrent->currentfile);
	kf->Set(id, var, NULL);
	return var;
...
KifElement* KifCode::KifParseIntentionDoubleVector(x_node* xn, KifElement* kf) {
	...
	kret->Setadd(1);
	//KifInstructionPARAMETER* kparam=new KifInstructionPARAMETER(this,kret);
	KifInstruction* ki = KifCreateInstruction(kret, kifMINUS);
	KifBrowse(xn->nodes[1], ki);
	int idord = KifNewId("ord");
	KifElement* kfunc;
...
KifElement* KifCode::KifParseLabel(x_node* xn, KifElement* kf) {
	...
	if (kifGlobals->kifLabels.find(name) != kifGlobals->kifLabels.end()) {
		string message = "Error: Already defined label: '" + name + "'";
...
KifPredicateContainer* KifCode::Predicatecontainer() {
	...
	if (ktop->declarations.find(KIFPREDICATEENTREE) == ktop->declarations.end()) {
...
KifElement* KifCode::KifParsePredicateDefinition(x_node* xn, KifElement* kf) {
	...
	if (xn->token == "predicatefact" || xn->nodes[1]->token == "abool") {
		...
		if (xn->nodes[1]->token == "predicatedatabase") {
			...
			if (xn->nodes[0]->nodes.back()->token == "predicateparameters")
				ComputePredicateParameters(xn->nodes[0]->nodes.back(), kcf);
			if (xn->nodes[1]->nodes[0]->value == "store")
				kcf->store = _PREDSTORE;
			else
			if (xn->nodes[1]->nodes[0]->value == "remove")
...
KifElement* KifCode::KifParsePredicateAssert(x_node* xn, KifElement* kf) {
	...
	if (id == idassertdb || id == idretractdb) {
		if (xn->nodes.size() != 3)  {
			long line = GetCurrentLine(0, xn);
			string message = "Error: Wrong assertdb or retractdb definition";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
		KifBrowse(xn->nodes[1], kbloc);
	}
		...
		if (xn->nodes.size() != 2)  {
			long line = GetCurrentLine(0, xn);
			string message = "Error: Wrong assert or retract definition";
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
	}
...
KifElement* KifCode::KifParseHaskellWhere(x_node* xn, KifElement* kf) {
	...
	for (int i = 0; i<xn->nodes.size(); i++) {
		if (xn->nodes[i]->token == "word") {
			idname = KifNewId(xn->nodes[i]->value);
			if (kint->body->lambdadomain->declarations.find(idname) != kint->body->lambdadomain->declarations.end()) {
				string message = "Variable: '";
...
KifElement* KifCode::KifParseReturnValue(x_node* xn, KifFunctionLambda* kf, char adding) {
	...
	if (kf->declarations.find(kifGlobals->idreturnvariable) == kf->declarations.end()) {
...
KifElement* KifCode::KifParseHaskellFilter(x_node* xn, KifElement* kbase) {
	...
	if (xn->nodes[0]->value == "dropWhile") {
		...
		var = new KifBoolean(this, var, kifTRUE);
	...
	if (xn->nodes[0]->token == "haskell") {
		...
		if (kprevious != NULL) {
			if (kprevious->type != kifFunctionLambda) {
				string message = "Error: A function with this name already exists: '" + name + "'";
				throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
			}
		...
		if (xn->nodes.size() == 2) {//we do not have a range or a Boolean expression			
			...
			KifBrowse(xn->nodes[1], ki); //compiling a 'guard' section
	...
	for (i = first + inc; i<xn->nodes.size(); i++) {
		...
		if (tok == "range") {
			if (xn->nodes[i]->nodes[0]->token == "let") {
				if (xn->nodes[i]->nodes[0]->nodes[0]->value == "iterator") {
...
KifElement* KifCode::KifParseFunction(x_node* xn, KifElement* kf) {
	...
	if (kprevious != NULL && kprevious->isFunction() == false) {
		string message = "Variable: '";
		message += name;
		message += "' already declared";
		throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
	...
	if (kprevious == NULL || kprevious->Predeclared() == false) {
		if (kprevious != NULL && kprevious->Alreadydeclared() && xn->nodes[last]->token == "declarationending") {
			instances = localinstances;
			return kprevious;
		}
...
KifElement* PreExtensions(KifElement* contextualpattern, KifDomain* dom, KifCallFunction* callfunc, int idthread) {
	...
	if (callfunc->kifcode->kstart->declarations.find(idtype) == callfunc->kifcode->kstart->declarations.end())
		return callfunc->kifcode->Returnerror("KIF(642): Error unknown extension");
	...
	string name("_");
	name += KifIdString(idtype);
	idtype = KifStringId(name);
	...
	if (kframe->declarations.find(callfunc->Name()) == kframe->declarations.end())
		return callfunc->kifcode->Returnerror("KIF(643): Error unknown function extension");
...
KifElement* KifCode::KifParseExtension(x_node* xn, KifElement* kf) {
	...
	short idname = KifNewId(nametype);
	KifElement* ke = NULL;
	...
	//It should be a known type...
	if (kifGlobals->kifTypes.find(nametype) == kifGlobals->kifTypes.end()) {
		message = "Error: Unknown type: '" + nametype + "'";
		throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
	}
	...
	for (int i = 0; i < kframe->instructions.size(); i++) {
		...
		if (ke->isFunction()) {
			...
			if (kifGlobals->KifCreatePredefined(this, name, P_ATLEASTONE, &PreExtensions) == false) {
				kifGlobals->PopStack();
				instances = localinstances;
				message = "Error: Cannot use this name as a function extension: '" + name + "'";
				throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
			}
...
KifElement* KifCode::KifParseFrame(x_node* xn, KifElement* kf) {
	...
	if (kifGlobals->frames.find(idname) != kifGlobals->frames.end()) {
		...
		if (privated) {
			instances = localinstances;
			long line = GetCurrentLine(0, xn);
			string message = "Error: attempt to use private frame:" + name;
			throw new KifRaiseError(message, kifGlobals->current->name, line, xn->end);
		}
	}
	else
...
KifElement* KifCode::Evaluate(KifElement* kf, string& sbase, bool commandline) {
	...
	if (xn == NULL) {
		if (gGlobals->bnfkif->errornumber != -1) {
			...
			char ch[20];
			sprintf_s(ch, 20, " (line:%d)", gGlobals->bnfkif->lineerror);
			msg += ch;
			return Returnerror(msg);
		}
		return Returnerror(kifGlobals->GetThreadid(), kifErrorStrings[244]);
	}
...
void KifCode::Load(x_reading* xr) {
	...
	if (seppos != -1) {
		lepath = xr->name.substr(0, seppos);
		fname = xr->name.substr(seppos + 1, xr->name.size() - seppos - 1);
	}
...
KifElement* KifCode::Find(string& name) {
	...
	hmap<short, KifElement*>::iterator it = kstart->declarations.find(idname);
	if (it == kstart->declarations.end())
		return NULL;
	return it->second;
}
...
KifElement* KifCode::Findraw(string& name) {
	...
	if (iter == kifGlobals->kifStringId.end())
		return NULL;
	hmap<short, KifElement*>::iterator it = kstart->declarations.find(iter->second);
	if (it == kstart->declarations.end())
		return NULL;
	return it->second;
}
...
KifElement* KifCode::Execute(KifFunction* func, vector<KifElement*>& parameters) {
	...
	return kcf.Exec(kifNULL, kstart, kifNULL, 0, NULL);
}
...
void KifCode::FinalGarbage() {
	...
	hmap<short, KifFrame*>::iterator itframe;
...
KifStaticVariable* KifCode::Createstaticvariable(string name, KifElement* kf, x_node* xn) {
	short idname = KifNewId(name);
	if (kifGlobals->kconstants->declarations.find(idname) != kifGlobals->kconstants->declarations.end()) {
...
Exported KifVector* KifCode::Providevector() {
	return new KifVector(this, NULL);
}

Exported KifMap* KifCode::Providemap() {
	return new KifMap(this, NULL);
}

Exported KifList* KifCode::Providelist() {
	return new KifList(this, NULL);
}
...
Exported void KifDomain::Setdeclaration(short n, KifElement* ke, hmap<short, KifElement*>* stacks) {
	...
	if (stacks != NULL) {
		...
		if (declarations.find(n) != declarations.end()) {
			kdec = declarations[n];
			if (kdec == ke)
				return;
			(*stacks)[n] = kdec;
...
Exported void KifDomain::Set(short n, KifElement* ke, hmap<short, KifElement*>* stacks) {
	if (kifGlobals->Test() == false && stacks == NULL) {
		ke->Incrementreference();
		declarations[n] = ke;
		return;
...
Exported void KifDomain::Resetdeclaration(hmap<short, KifElement*>& stacks) {
	if (stacks.size() == 0)
		return;
	hmap<short, KifElement*>::iterator it;
	short n;
...
Exported void KifDomain::SetInstances(KifElement* dom, hmap<short, KifElement*>* stacks) {
	hmap<short, KifElement*>::iterator it;
	dom->Disableadding();
	KifElement* xe;
...
KifElement* KifSelf::Declaration(short n) {
	...
	string message = "a 'self' object can only call functions: ";
...
KifElement* KifBit::Declaration(short n) {
	if (n == kifGlobals->kifStringId["#"] || n == kifGlobals->kifStringId["partof"])
		return kifGlobals->kifBaseFunctions[n];
	if (TestkifFunctions(n, kifGlobals->kifPredefinedFunctions) == true)
		return kifGlobals->kifBaseFunctions[n];
	return NULL;
...
KifElement* KifIndex::Declaration(short n) {
	...
	string message = "Only functions can be called from an index: ";
	message += kifGlobals->kifIdString[n];
	throw new KifRaiseError(message, kifGlobals->current->name, kifGlobals->current_start, kifGlobals->current_end);
	return NULL;
}
...
Exported KifElement* KifString::Information(KifElement* kstr) {
	...
	if (kifGlobals->kifInformationsString.find(s) == kifGlobals->kifInformationsString.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsString[s]);
}

Exported KifElement* KifInteger::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsInt.find(s) == kifGlobals->kifInformationsInt.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsInt[s]);
}
...
Exported KifElement* KifBit::Information(KifElement* kstr) {
	return kifcode->Providestring(kifGlobals->kifInformationsInt["#"]);
}
...
Exported KifElement* KifFloat::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsFloat.find(s) == kifGlobals->kifInformationsFloat.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsFloat[s]);
}
...
Exported KifElement* KifIterator::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsIterator.find(s) == kifGlobals->kifInformationsIterator.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsIterator[s]);
}


Exported KifElement* KifDate::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsDate.find(s) == kifGlobals->kifInformationsDate.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsDate[s]);
}

Exported KifElement* KifTime::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsTime.find(s) == kifGlobals->kifInformationsTime.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsTime[s]);
}

Exported KifElement* KifBaseFile::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsFile.find(s) == kifGlobals->kifInformationsFile.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsFile[s]);
}


Exported KifElement* KifKiF::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsKif.find(s) == kifGlobals->kifInformationsKif.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsKif[s]);
}
...
void KifInitFunctions(KifCode* kifcodeptr) {
	...
	kifGlobals->KifInitInformations();
...
void KifCode::Updatekstart() {
	...
	if (kifGlobals->kconstants == NULL) {
		...
		id = KifNewId("a_switch");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 16);
		kifGlobals->kconstants->Set(id, args, NULL);

		id = KifNewId("a_prefix");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 64);
		kifGlobals->kconstants->Set(id, args, NULL);

		id = KifNewId("a_nocase");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 32);
		kifGlobals->kconstants->Set(id, args, NULL);

		id = KifNewId("a_surface");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 64);
		kifGlobals->kconstants->Set(id, args, NULL);
		...
		id = KifNewId("pred_none");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 0);
		kifGlobals->kconstants->Set(id, args, NULL);

		id = KifNewId("pred_weight");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 1);
		kifGlobals->kconstants->Set(id, args, NULL);

		id = KifNewId("pred_weightstack");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 2);
		kifGlobals->kconstants->Set(id, args, NULL);

		id = KifNewId("pred_normalize");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 4);
		kifGlobals->kconstants->Set(id, args, NULL);

		id = KifNewId("pred_randomize");
		args = new KifConst(this, NULL, id, true);
		kint = new KifBaseInteger(this, args, 8);
		kifGlobals->kconstants->Set(id, args, NULL);
...
KifElement* KifVariableDeclaration::Robfunction(KifElement* kbase) {
	if (function != NULL && kbase != NULL) {
		if (kbase->testfunction(function) == false)
			kifcode->Returnerror("KIF(881): Error, wrong function arguments");
	}
