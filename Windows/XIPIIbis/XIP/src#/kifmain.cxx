...
#define conversion2dos(X) s_utf8todos(X)
#else
...
void displayhelpkif(string wh) {
    ...
    cout<<conversion2dos("-console: open the console editor")<<endl;
    cout<<conversion2dos("-a 'source code' with _args fully piped")<<endl;
    cout<<conversion2dos("-p 'source code' with _args piped one string at a time")<<endl;
    cout<<conversion2dos("-c 'source code' without piped data")<<endl;
    cout<<conversion2dos("Predeclared variables:")<<endl;
    cout<<"\t";
	cout<<conversion2dos("_args: argument vector")<<endl;
	cout << "\t";
	cout << conversion2dos("_paths: _paths[0] is the current directory") << endl;
	cout << "\t";
	cout << conversion2dos("i,j,k: int") << endl;
	cout << "\t";
	cout << conversion2dos("f,g,h: float") << endl;
	cout << "\t";
	cout << conversion2dos("s,t,u: string") << endl;
	cout << "\t";
	cout << conversion2dos("m: map") << endl;
	cout << "\t";
	cout << conversion2dos("v: vector") << endl;
    exit(-1);
}
...
int main(int argc, char *argv[]) {
...
#endif
#ifdef MACXCODE
        cerr<<"Xcode version"<<endl;
#endif 
#ifdef WITHCURL
		cerr<<conversion2dos("Curl library Embedded")<<endl;
#endif
#ifdef FLTKCODE	
		cerr<<conversion2dos("Fast Light ToolKit (FLTK) available")<<endl;
#endif		
        cerr<<endl;
        displayhelpkif(wh);
	}
	...
	for (int i=1;i<argc;i++) {
        ...
        if (args[0]=='-') {
            ...
            if (code=="" && piped!=1) {
                cerr<<conversion2dos("Error: missing code")<<endl;
                exit(-1);
            }
            continue;
        }
	...
	if (code=="") {
        if (name[0]=='-') {
            cerr<<conversion2dos("Missing filename...")<<endl;
            exit(-1);
        }
    ...
    if (Error()==true) {
        cerr<<"Error: "<<STR(Errorstring())<<endl;
        exit(-1);
    }
