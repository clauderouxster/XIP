...
void GlobalParseur::Nettoyage() {
	...
	for (i=0;i<vlarbredestraits.dernier;i++)
		delete vlarbredestraits[i];
	vlarbredestraits.raz();
...
*/
	...
	*/
	...
	for (p=0;p<VariablesProceduresGlobales.etats.dernier;p++) {
		if (VariablesProceduresGlobales.etats[p]!=NULL &&
			VariablesProceduresGlobales.etats[p]->valeur!=NULL) {
				VECTA<TestFormule*>* vtform=
					(VECTA<TestFormule*>*)(VariablesProceduresGlobales.etats[p]->valeur);                
				delete vtform;
			}
	}
...
//----------------------------------------------------------------
	...
	if (Type()==PARSEURTRANS) {
		...
		for (ii=0;ii<gtrans->auvol.size();ii++)
			duplicationfichier(gtrans->auvol[ii]);
		for (ii=0;ii<gtrans->fichiersvoc.size();ii++)
			duplicationfichier(gtrans->fichiersvoc[ii]);
		duplicationfichier(gtrans->nom);
	}
...
void GlobalParseur::TestUtf8Fichier(FILE* f) {
	...
	while (!fichier.eof()) {
		...
		if (c_is_utf8(USTR(ligne),ligne.size())==true) {
			utf=true;
			break;
		}
	}
...
void GlobalParseur::TestUtf8Chaine(uchar* chaine) {
	if (c_is_utf8(chaine,strlen((char*)chaine))==true) {
		ParametresAffichage|=ENTREE_UTF8;
		ParametresAffichageDefini|=ENTREE_UTF8;
		return;
	}
...
char GlobalParseur::ConsVoc(istream& fichier,char lit_couche) {
    ...
    if (trcourant->lemme!=NULL) {
        trcourant->lemme->mot(tampon,300);
        lelem=tampon;
    }
