...
public:
	...
	Exported bool Setvalue(KifElement* ke,KifElement* theidx,KifElement* dom,int idthread,KifCallFunction* callfunc) {
...
public:
	...
	unsigned int sizecolumn;

	//---------------------------------------------------------------------------------------------------------------------
	Kifsparse(KifCode* kifcode,KifElement* base) : KifBasic(kifcode,base,kifsparse_type),nb(0),
		firstinsertion(true),minval(0),maxval(0),sizerow(0),sizecolumn(0){}
	...
	void populate(KifElement* krow,KifElement* kcol,KifElement* value) {
		...
		if (!matrixbyrow.find(r) || !matrixbyrow[r].find(c))
			nb++;
		sizerow=max(sizerow,r);
		sizecolumn=max(sizecolumn,c);
	...
	Exported bool Setvalue(KifElement* ke,KifElement* idx,KifElement* dom,int idthread,KifCallFunction* callfunc) {
		ThreadLock _lock(type);
		if (ke==this)
			return true;
		if (idx==NULL) {
		...
		if (ke->aVectorContainer()) {
			...
			kifcode->Returnerror("MAT(131): Wrong index or Wrong value (recipient should be a map or a sparsevector)");
			return true;
	}
	...
	KifElement* MethodMatrixSize(KifElement* contextualpattern,KifDomain* domain,KifCallFunction* callfunc,int idthread) {
		if (callfunc->Size()==0) {
			KifElement* kvect=Selectvectorinteger(callfunc,contextualpattern);
			kvect->storevalue((long)sizerow);
			kvect->storevalue((long)sizecolumn);
			return kvect;
		}
...
//--------------------------------------------------------------------------------------------------------
	...
	Exported KifElement* multiply(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
		...
		Kifsparse* mb=(Kifsparse*)b;
		if (ma->sizecolumn!=mb->sizerow || ma->sizerow!=mb->sizecolumn)
	...
	KifElement* divide(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
		...
		if (ma->sizecolumn!=inverted.sizerow || ma->sizerow!=inverted.sizecolumn)
			return kifcode->Returnerror("MAT(209): Matrix size mismatch");
...
KifElement* Kifsparseline::andset(KifElement* a,KifElement* b,KifElement* context,bool autoself) {
	...
	if (a!=context) {
		...
		case kifMap:
			...
			break;
		case kifMapFloat:
			res=new KifMapFloat(kifcode,NULL);
			break;
		default:
			if (context->type==kifsparseline_type)
				res=context;
...
Exported KifElement* Kifsparseline::multiply(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type!=kifsparseline_type) {
		Clear();
		return this;
	}
...
//Beware that a might be the current element
//plus: a+b
KifElement* Kifsparseline::plus(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type!=kifsparseline_type) {
		Clear();
		return this;
	}
...
//minus: a-b
KifElement* Kifsparseline::minus(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type!=kifsparseline_type) {
		Clear();
		return this;
	}
...
//Division: a/b
KifElement* Kifsparseline::divide(KifElement* a,KifElement* b,KifElement* context,int idthread,KifCallFunction * callfunc,bool autoself) {
...
extern "C" {
	Exported bool Init_kifsparse(KifCode* kifcode,string version) {
		...
		KifType res=KifAddNewType("sparse",CreatekifsparseElement);
		if (res==kifVOID)
			return false;
		kifsparse_type=res;

		res=KifAddNewType("sparsevector",CreatekifsparselineElement);
		if (res==kifVOID)
			return false;
	...
	Exported bool kifsparse_KifInitialisationModule(KifCode* kifcode,string version) {
		...
		MethodInitializationSparse(kifcode,"matrixsize",&Kifsparse::MethodMatrixSize,P_NONE|P_TWO,"matrixsize(): return the matrix size.\nmatrixsize(int sizerow,int sizecolumn): set the matrix size.");
		...
		MethodInitializationSparse(kifcode,"row",&Kifsparse::MethodRow,P_ONE,"row(x): return the value on row x as a map or as a sparsevector");
		MethodInitializationSparse(kifcode,"column",&Kifsparse::MethodColumn,P_ONE,"column(x): return the value on column x as a map or as a sparsevector");
		...
		MethodInitializationSparse(kifcode,"eigen",&Kifsparse::MethodEigen,P_SIX,"eigen(sparse eigenmatrix,vector valreal,vector valimaginary,int vec,int ortho,int ev_norm):\
