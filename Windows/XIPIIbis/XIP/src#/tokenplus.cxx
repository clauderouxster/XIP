/* $Id: tokenplus.cxx,v 1.60 2011/01/26 10:47:23 roux Exp $ */
...
extern "C" {
	...
	void set_strat_control(void* sc);
}
...
extern "C" {
	...
	int symbolsize(id_type id,GlobalParseur* parseur) {
        static char bufferinterne[1000];		
    ...
    void insert_token_id(id_type id,GlobalParseur* parseur) {
        static char bufferinterne[2000];
		bufferinterne[0]=0;
        word_offset* w=parseur->lectures.fin();
	...
	void insert_all_id_sequence(GlobalParseur* parseur,char prefix) {
		...
		insert_token_char(9,parseur,0);
		char buff[100];
		sprintf(buff,"+score_%d",parseur->editdistancescore);
...
GlobalParseurTokenLookup::~GlobalParseurTokenLookup() {
	...
	set_strat_control(controle_strategies);
...
void GlobalParseurTokenLookup::SentenceAnalysis(char renumerote,char arbre) {
    ...
    if (entree_est_chaine)
        tokenizestring((char*)donneesATraiter,FALSE,TRUE);
    else
        tokenizefile(fichierDonnees,FALSE,TRUE);
...
void GlobalParseurTokenLookup::Ajusteoffset() {
	...
	for (int offset=0;offset<phrase.dernier;offset++) {
		...
		final->borneDroiteCar=dernieroffsetcar+c_size(final->surfacebrute);
		if (df!=0) {
	...
	while (loop) {
		if (wo.longueur>taille_mot || gras!=NULL) {
			if (compteloop==0) {
				...
				conversionUTF8VersGras((uchar*)wo.mot,*gras);
			}
			string s;
			int nbcars=0;
		...
		if (lectures.dernier==0) {
			...
			sprintf(ch,"%s\t%s\t+%s+hmm_%s",wo.mot,wo.mot,TAG_NOUVEAU,TAG_NOUVEAU);
			w->concatene(ch);
		}
...
TokLOOK::~TokLOOK() {
	...
	NETptr net;
	int i;
	for (i=0; i< NV_len (reseaux_lexicaux); i++) {
		if ((net = NV_net (reseaux_lexicaux, i)))
			free_network (net);
	}
	free_nv_only(reseaux_lexicaux);

	NVptr nets;
	for (i=0; i< NV_len (strategies_lexicales); i++) {
		if ((nets = NVV_nv(strategies_lexicales,i)))
			free_nv_only(nets);
	}
	LOWER_SIGMAS_LEXNETS=NULL;
    LEXNETS=NULL;
...
void TokLOOK::litdowner() {
	for (int i=0; i< nets.dernier;i++)
		down_range.ajoute(NET_downrange_map (nets[i]));
}
...
void TokLOOK::lowermap() {
	...
	for (int i=0; i< nets.dernier;i++)
		NET_uprange_map (nets[i])=down_range[i];
}
...
TokLOOK::TokLOOK(GlobalParseur* p,char* script,char* flags,int type,char utf8mode) {
	...
	litupper(reseaux_lexicaux);	
	make_range_map_for_every_net (UPPER, type);
	litdowner();	
