...
KifElement* KifEvaluate::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE || itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->plus(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
		}
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->plus(ke, b, context, idthread,callfunc,false));
		ke->Release();
	}
	it->Release();
	return ref;
...
KifElement* KifEvaluate::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE && itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->minus(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
		}
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->minus(ke, b, context, idthread,callfunc,false));
		ke->Release();
	}
	it->Release();
	return ref;
...
KifElement* KifEvaluate::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE && itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->multiply(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
		}
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->multiply(ke, b, context, idthread,callfunc,false));
		ke->Release();
	}
	it->Release();
	return ref;
...
KifElement* KifEvaluate::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE && itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->divide(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
			if (Errorid(idthread)) {
				it->Release();
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->divide(ke, b, context, idthread,callfunc,false));
		ke->Release();
		if (Errorid(idthread)) {
			it->Release();
			ref->Release();
...
KifElement* KifEvaluate::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE && itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->mod(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
			if (Errorid(idthread)) {
				it->Release();
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->mod(ke, b, context, idthread,callfunc,false));
		ke->Release();
		if (Errorid(idthread)) {
			it->Release();
			ref->Release();
...
KifElement* KifEvaluate::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE && itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->shiftright(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
		}
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->shiftright(ke, b, context, idthread,callfunc,false));
		ke->Release();
	}
	it->Release();
	return ref;
...
KifElement* KifEvaluate::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE && itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->shiftleft(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
		}
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->shiftleft(ke, b, context, idthread,callfunc,false));
		ke->Release();
	}
	it->Release();
	return ref;
...
KifElement* KifEvaluate::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
	...
	if (b->aContainer()) {
		...
		for (it->Begin(), itr->Begin(); it->End() == kifFALSE && itr->End() == kifFALSE; it->Next(), itr->Next()) {
			ke = it->IteratorValue();
			kref = itr->IteratorValue();
			ref->Push(ke->power(ke, kref, context, idthread,callfunc,false));
			ke->Release();
			kref->Release();
		}
	...
	for (it->Begin(); it->End() == kifFALSE; it->Next()) {
		ke = it->IteratorValue();
		ref->Push(ke->power(ke, b, context, idthread,callfunc,false));
		ke->Release();
	}
	it->Release();
	return ref;
...
KifElement* KifEvaluate::Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
...
Exported void KifElement::Push(KifElement*) {
	kifcode->Returnerror(kifErrorStrings[243]);
}
...
Exported KifElement* KifElement::PopFirst(KifElement* ke) {
	return kifcode->Returnerror(kifErrorStrings[243]);
}
...
KifElement* KifIteratorCycleVector::Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
	return iter->Execute(contextualpattern, dom, val);
}

KifElement* KifIteratorCycleElement::Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
	return cycle->value;
}

KifElement* KifIteratorReplicateElement::Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
...
KifElement* KifEvaluateMap::Match(KifBaseMap* basemap, KifDomain* dom, int idthread, KifCallFunction* callfunc) {
	...
	for (int i = 0; i<sz; i++) {
		...
		if (bkey->Name() == idpipe) {
			...
			bval = ((KifInstance*)bval)->Thevariable(dom, idthread, callfunc);
			bval->Setvalue(vals);
			return kifTRUE;
		}
		...
		key = itr->IteratorKey();
		if (bkey->Name() != -1) {
			bkey = ((KifInstance*)bkey)->Thevariable(dom, idthread, callfunc);
			bkey->Setvalue(key);
			key->Release();
		}
		else {
...
KifElement* KifVector::Match(KifBaseVector* basevector, KifDomain* dom, int idthread, KifCallFunction* callfunc) {
	...
	for (int i = 0; i<sz; i++) {
		...
		val = values[iv];
		if (ret->Name() != -1) {
			ret = ((KifInstance*)ret)->Thevariable(dom, idthread, callfunc);
			ret->Setvalue(val);
		}
		else {
			if (ret->type == kifBaseVector) {
...
Exported KifElement* KifBaseVector::Vector(KifElement* dom, KifVector* v) {
	...
	for (int i = 0; i<values.size(); i++) {
		kval = values[i]->Execute(kifNULL, (KifDomain*)dom);
		kvect->Push(kval);
	}
	return kvect;
}
...
Exported KifElement* KifBaseVector::DuplicateHardCodedValue(KifDomain* dom) {
	...
	KifElement* kval;
	for (int i = 0; i<values.size(); i++) {
		kval = values[i]->Execute(kifNULL, dom);
		kvect->Push(kval);
	}
	return kvect;
}
...
KifElement* KifVector::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (!a->aContainer())
		return a->plus(a, b, context, idthread,callfunc,autoself);
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ke->plus(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			itr->Next();
		}
		itr->Release();
...
KifElement* KifVector::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ki = itr->IteratorValue();
			ke = ref->ValueOnIndex(it);
			ke->minus(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			itr->Next();
		}
		itr->Release();
...
KifElement* KifVector::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ki = itr->IteratorValue();
			ke = ref->ValueOnIndex(it);
			ke->multiply(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			itr->Next();
		}
		itr->Release();
...
KifElement* KifVector::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (!a->aContainer())
		return a->divide(a, b, context, idthread,callfunc,autoself);
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ki = itr->IteratorValue();
			ke = ref->ValueOnIndex(it);
			ke->divide(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			if (Errorid(idthread)) {
				itr->Release();
				ref->Release();
...
KifElement* KifVector::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ki = itr->IteratorValue();
			ke = ref->ValueOnIndex(it);
			ke->mod(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			if (Errorid(idthread)) {
				itr->Release();
				ref->Release();
...
KifElement* KifVector::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ki = itr->IteratorValue();
			ke = ref->ValueOnIndex(it);
			ke->shiftright(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			itr->Next();
		}
		itr->Release();
...
KifElement* KifVector::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (!a->aContainer())
		return a->shiftleft(a, b, context, idthread,callfunc,autoself);
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ki = itr->IteratorValue();
			ke = ref->ValueOnIndex(it);
			ke->shiftleft(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			itr->Next();
		}
		itr->Release();
...
KifElement* KifVector::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	...
	if (b->aContainer()) {
		...
		for (it = 0; it<ref->Size(); it++) {
			...
			ki = itr->IteratorValue();
			ke = ref->ValueOnIndex(it);
			ke->power(ke, ki, context, idthread,callfunc,true);
			ki->Release();
			itr->Next();
		}
		itr->Release();
...
KifElement* KifList::xorset(KifElement* a, KifElement* b, KifElement* context, bool autoself) {
	...
	if (b->aContainer()) {
		KifContainerLock _lockb((KifEvaluate*)b);
		KifList* ref = kifcode->Providelist();
		KifIteration* itr = (KifIteration*)b->Newiterator(false);
		for (itr->Begin(); itr->End() == kifFALSE; itr->Next()) {
			kref = itr->IteratorValue();
...
KifElement* KifList::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->plus(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifList::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::minus");
#endif
...
KifElement* KifList::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::multiply");
#endif
...
KifElement* KifList::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->divide(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifList::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::mod");
#endif
...
KifElement* KifList::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::shiftright");
#endif
...
KifElement* KifList::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
...
KifElement* KifList::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::power");
#endif
...
bool KifVector::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
bool KifList::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
#endif
	if (TestkifFunctions(n, kifGlobals->kifListFunctions) == true)
...
Exported KifElement* KifVector::Information(KifElement* kstr) {
	...
	if (kifGlobals->kifInformationsVector.find(s) == kifGlobals->kifInformationsVector.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsVector[s]);
}
...
Exported KifElement* KifList::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsList.find(s) == kifGlobals->kifInformationsList.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsList[s]);
}
...
KifElement* KifList::Copy(KifDomain* kp, KifElement* dom) {
	...
	profilingcall("Vector::Copy");
#endif
	KifElement* ke = kifcode->Providelist();
...
KifElement* KifList::Exec(KifElement* contextualpattern, KifDomain* dom, KifElement* idx, int idthread, KifCallFunction* callfunc) {
	...
	if (idx == NULL || idx->isConst()) {
		...
		if (evaluate == true) {
			KifList* kvect = kifcode->Providelist();
			list<KifElement*>::iterator it;
			for (it = values.begin(); it != values.end(); it++) {
...
bool KifVectorString::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	KifContainerLock _lock(this);
	if (ke == this)
		return true;
...
KifElement* KifVectorString::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
bool KifVectorInteger::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	KifContainerLock _lock(this);
	if (ke == this)
		return true;
...
KifElement* KifVectorInteger::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorInteger::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorInteger::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorInteger::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorInteger::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorInteger::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorInteger::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorInteger::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
bool KifVectorFloat::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	KifContainerLock _lock(this);
	if (ke == this)
		return true;
...
KifElement* KifVectorFloat::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorFloat::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorFloat::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorFloat::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorFloat::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorFloat::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorFloat::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
KifElement* KifVectorFloat::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
...
Exported bool KifTree::Setvalue(KifElement* k, KifElement* index, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (value != kifNULL)
	...
	if (k->aContainer()) {
		...
		if (k->Size() != 1) {
			kifcode->Returnerror("KIF(877): Missing root in the container");
			return true;
		}
		KifIteration* itr = (KifIteration*)k->Newiterator(false);
		itr->Begin();
...
Exported KifElement* KifTree::Information(KifElement* kstr) {
	string s = kstr->String();
	if (kifGlobals->kifInformationsTree.find(s) == kifGlobals->kifInformationsTree.end())
		return kifNULL;
	return kifcode->Providestring(kifGlobals->kifInformationsTree[s]);
}
...
Exported void KifTable::build(int sz) {
	if (sz <= 0)
		return;
...
Exported void KifTable::Insert(int idx, KifElement* ke) {
	if (idx >= size - 1) {
		kifcode->Returnerror(kifErrorStrings[243]);
		return;
	}
...
void KifTable::Merge(KifElement* ke) {
	...
	if (i == -1) {
		kifcode->Returnerror(kifErrorStrings[267]);
		return;
	}
...
Exported void KifTable::Push(KifElement* ke) {
	if (ke == kifRAISEERROR)
		return;
...
KifElement* KifTable::Exec(KifElement* contextualpattern, KifDomain* dom, KifElement* idx, int idthread, KifCallFunction* callfunc) {
...

	{
		...
		kvect->build(size);
		for (int i = ikey; i<iright; i++)
			kvect->values[i - ikey] = values[i];
	}
	return kvect;
}

bool KifTable::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
KifElement* KifTable::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
...
KifElement* KifTable::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::minus");
#endif
...
KifElement* KifTable::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::multiply");
#endif
...
KifElement* KifTable::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
...
KifElement* KifTable::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (!a->aContainer())
		return a->mod(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifTable::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::shiftright");
#endif
...
KifElement* KifTable::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::shiftleft");
#endif
	if (!a->aContainer())
...
KifElement* KifTable::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
#ifdef PROFILING
	profilingcall("Vector::power");
#endif
	if (!a->aContainer())
...
KifElement* KifTable::Declaration(short n) {
	if (TestkifFunctions(n, kifGlobals->kifVectorFunctions) == true || n == KifStringId("resize"))
		return kifGlobals->kifBaseFunctions[n];
	return NULL;
}
...
KifElement* KifIteratorList::Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
#ifdef PROFILING
	...
	return ke->Execute(contextualpattern, dom, val);
}

KifElement* KifIteratorVector::Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
	if (reverse == false) {
		if (itx == values->size())
			return kifNULL;
	}
	...
	return (*values)[itx]->Execute(contextualpattern, dom, val);
}

KifElement* KifIteratorTable::Execute(KifElement* contextualpattern, KifDomain* dom, KifElement* val) {
	if (reverse == false) {
		if (itx == size)
			return kifNULL;
	}
...
bool KifSetString::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
bool KifSetInteger::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
bool KifSetFloat::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
bool KifSet::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom,int idthread,KifCallFunction* callfunc) {
	if (ke == this)
		return true;
...
KifElement* KifSetString::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->plus(a, b, context, idthread,callfunc,autoself);
...
//-----------------------------------------------------------------------------------------------------
KifElement* KifSetInteger::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->plus(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetInteger::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->minus(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetInteger::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->multiply(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetInteger::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->divide(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetInteger::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->mod(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetInteger::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->shiftright(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetInteger::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->shiftleft(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetInteger::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->power(a, b, context, idthread,callfunc,autoself);
...
//-----------------------------------------------------------------------------------------------------
KifElement* KifSetFloat::plus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->plus(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetFloat::minus(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->minus(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetFloat::multiply(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->multiply(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetFloat::divide(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->divide(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetFloat::mod(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->mod(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetFloat::shiftright(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->shiftright(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetFloat::shiftleft(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->shiftleft(a, b, context, idthread,callfunc,autoself);
...
KifElement* KifSetFloat::power(KifElement* a, KifElement* b, KifElement* context, int idthread,KifCallFunction * callfunc,bool autoself) {
	if (a->type != type)
		return a->power(a, b, context, idthread,callfunc,autoself);
