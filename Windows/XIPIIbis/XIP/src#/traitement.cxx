...
void GlobalParseur::NettoiePhrase() {
	...
	if (fenetre != -1) {
		...
		TasDeListeVoc.dernier = phrase[0]->position;
		for (i = 0; i<phrase.dernier; i++) {
			phrase[i]->raz();
			if (TasDeListeVoc.dernier>phrase[i]->position)
				TasDeListeVoc.dernier = phrase[i]->position;
		}
...
void GlobalParseur::SentenceAnalysis(char renumerote, char arbre) {
	...
	while (!finLecture()) {
		...
		Parse(OUI);
		num_phrase++;
		//Ce compteur de phrase est global a toutes les analyses.
		Detruit();
	}
...
word_offset* GlobalParseur::fournirWordOffset() {
	word_offset* w;
	if (lectures.dernier >= lectures.taille)
