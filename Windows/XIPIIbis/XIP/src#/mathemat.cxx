...
GlobalParseur* initNouveauParseur(int parsnum) {	
 ...
 void Ajoutenoeud(PyObject* liste,XipNode* node,map<int,PyObject*>& noeuds,int i) {
	 ...
	 PyObject* pos=PyString_FromString(node->category.c_str());
	 PyDict_SetItemString(noeud, "#pos", pos);
	 Py_DECREF(pos);
	 
	 pos=PyInt_FromLong(node->idnode);
	 ...
	 if (node->Type()==XIPLEAF) {
		 ...
		 pos=PyString_FromString(feuille->lemma.c_str());
		 PyDict_SetItemString(noeud, "#lemma", pos);
		 Py_DECREF(pos);
		 pos=PyString_FromString(feuille->surface.c_str());
 ...
 void Ajoutedependancepython(PyObject* liste,map<int,PyObject*>& noeuds,XipDependency* d,int i) {
	 ...
	 Py_DECREF(dep);
	 PyObject* pos=PyString_FromString(d->name.c_str());
	 ...
	 for (j=0;j<d->features.size();j++) {
		 ...
		 pos=PyString_FromString(d->features[j]->value.c_str());
 ...
 void Ajouteunitpython(PyObject* unit,XipUnit* xunit,int i) {
	 PyObject* liste=PyList_New(0);
 ...
 void Creeresultatpython(PyObject* resultat,XipResult* result) {	 
	 for (int i=0;i<result->sentences.size();i++)
		 Ajouteunitpython(resultat,result->sentences[i],i);
 }
...
extern "C" {
    ...
    static PyObject* xips_loadgrm(PyObject* self, PyObject* args) {
        ...
        try {
           ipar=XipGrmFile(grmfile,1,0);
            ...
            return PyString_FromString(msg->message.c_str());
        }

        return PyInt_FromLong(ipar);
    }
    ...
    static PyObject* xips_reloadgrm(PyObject* self, PyObject* args) {
		...
		if (!PyArg_ParseTuple(args,"i",&parsnum)) {
			PyErr_SetString(PyExc_AttributeError, "Wrong parameters");
            return PyString_FromString("Please provide a grammar handle");
		}
    ...
    static PyObject* xips_parsestring(PyObject* self, PyObject* args) {
		...
		Creeresultatpython(resultat,xip);
		delete xip;
	...
	static PyObject* xips_parsedependencystring(PyObject* self, PyObject* args) {
		...
		if (res==0)
			...
			return PyString_FromString(affichagemessageerreur[res]);
		}
    }
    ...
    static PyObject* xips_version(PyObject* self, PyObject* args) {
        char version[1000];      
         ...
         sprintf(version,"XIP: Xerox Incremental Parser %s (2000-2007) ",VERSION_XIP_BUILD);
    ...
    static PyObject* xips_parsefile(PyObject* self, PyObject* args) {
        ...
        retourInitialParseur(precedent);
        return PyString_FromString("end");
    }
    ...
    static PyObject* xips_xiplexicalnodes(PyObject* self, PyObject* args) {
        ...
        for (i=0; i< parseur_courant->NoeudsLexicaux.dernier;i++) {
            PyObject* pcourant=PyInt_FromLong(parseur_courant->NoeudsLexicaux[i]->num_noeud);
            PyList_Insert(liste,i,pcourant);
			Py_DECREF(pcourant);
        }
    ...
    static PyObject* xips_xipfictivenodes(PyObject* self, PyObject* args) {
        ...
        for (i=0; i< parseur_courant->NoeudsDependance.dernier;i++) {
            PyObject* pcourant=PyInt_FromLong(parseur_courant->NoeudsDependance[i]->N.num_noeud);
            PyList_Insert(liste,i,pcourant);
			Py_DECREF(pcourant);
        }
    ...
    static PyObject* xips_xipinternalstring(PyObject* self, PyObject* args) {
        ...
        return PyString_FromString((char*)parseur_courant->donneesATraiter);
    }
    ...
    static PyObject* xips_dependency(PyObject* self, PyObject* args) {
		...
		if (parseur->TestOptions(AFFICHAGE_NUMERO_DEPENDANCE)) {
			...
			for (i=0;i<rf->numeros.dernier;i++) {
				sprintf(num,"%d",parseur->VlabelVecteurIndex[rf->numeros[i]]->numero);
				if (i)
					traits+=",";
				traits+=num;
			}
    ...
    static PyObject* xips_get_sourcecharset(PyObject* self, PyObject* args) {
		...
		if (ty==LATIN_ENCODING)
			return PyString_FromString("latin-1");
    ...
    static PyObject* xips_metpoids(PyObject* self, PyObject* args) {
        ...
        if (parseur==NULL)
            return PyInt_FromLong(0);
		return PyInt_FromLong(parseur->proba_metpoids(id,value));
	}
	...
	static PyObject* xips_metseuil(PyObject* self, PyObject* args) {
		...
		return PyInt_FromLong(parseur->proba_metseuil(id,value));
	}
	...
	static PyObject* xips_metvaleur(PyObject* self, PyObject* args) {
		...
		return PyInt_FromLong(parseur->proba_metvaleur(id,value));
	}
	...
	static PyObject* xips_loadkif(PyObject* self, PyObject* args) {
		...
		if (KifExecuteProgram(filename,pargs)==-1)
			return PyInt_FromLong(-1);
	...
	static PyObject* xips_kif(PyObject* self, PyObject* args) {
		...
		if (BaseParseur==NULL || BaseParseur->kifloader==NULL) {
			PyErr_SetString(PyExc_AttributeError, "No KiF program in memory");
			return PyInt_FromLong(-3);
		}
		...
		bool theerror=false;
		KifElement* resultat=XipKifExecuteObject(functionname,params,theerror);
		if (theerror==true) {
	...
	static PyObject* xips_typereglestring(PyObject* self, PyObject* args) {
		...
		return PyString_FromString(tys);
	}
	...
	static PyObject* xips_nbregles(PyObject* self, PyObject* args) {
		...
		return PyInt_FromLong(parseur->VBases.dernier);
	}
    ...
    static PyMethodDef NoeudMethode[] = {
		...
		{"loadkif",xips_loadkif,METH_VARARGS,"loadkif(filename,arguments) load a KiF program"},
		{"kif",xips_kif,METH_VARARGS,"kif(function_name,arguments) Execute a KiF function, the argument should be a vector of strings"},
        {NULL,NULL,0,NULL}
    };
...
inline string EnMinuscule(string& iterateur) {
    return c_Tolower(iterateur);
}

inline string EnMajuscule(string& iterateur) {
    return c_Toupper(iterateur);
}
...
void VariableVectorIndex::raz() {
    VECTA<int>* vic;
    for (int i=0;i<index.etats.dernier;i++) {
        if (index.etats[i]!=NULL && index.etats[i]->valeur!=NULL) {
            vic=(VECTA<int>*)index.etats[i]->valeur;
            delete vic;
            index.etats[i]->valeur=NULL;
        }
    }
...
string VariableIndexeeDependance::valdep(ResultatFonction* rf) {
	for (int i=0;i<indexes.dernier;i++)
		if (rf==indexes[i]) {
			char ch[100];
			if (type==REEL)
				sprintf(ch,"%g",valeurs[i]);
			else
				sprintf(ch,"%d",valeurs[i]);
...
void VariableIndexeeDependance::CreationXMLDependance(GlobalParseur* parseur,
    ...
    for (int id=0;id<indexes.dernier;id++)
        if (rf==indexes[id]) {
            ...
            parseur->AjouteAttributValeur(feature,"attribute",ch);
            if (type==REEL)
                sprintf(ch,"%g",valeurs[id]);
            else
                sprintf(ch,"%d",valeurs[id]);
            parseur->AjouteAttributValeur(feature,"value",ch);
            return;
        }   
}
...
void VariableIndexeeChaine::CreationXMLNoeud(GlobalParseur* parseur,xmlNodePtr parent,Noeud* n) {
    ...
    for (int i=0;i<indexes.dernier;i++)
        if (n==indexes[i]) {
            ...
            sprintf(ch,"#%s",nom);
            parseur->AjouteAttributValeur(feature,"attribute",ch);
			sprintf(ch,(char*)valeurs[i]->c_str());
            parseur->AjouteAttributValeur(feature,"value",ch);
            return;
        }   
}
    ...
    while (k!=':') {
        ...
        if (ty==AUCUN) {
            ...
            if (e!=NULL && e->valeur!=NULL)
                v=(Variable*)e->valeur;
            if (err==OUI) {
                sprintf(lect,"%s: %s",CHAINES[303],tampon);
                erreur(lect);
            }
        }
        ...
        if (nomproc==NULL)
            ...
            sprintf(tampon,"%s::%s",lect,nomproc->Fx->nom);
        char vecteur=NON;
        ...
        while (etat!=100) {
            ...
            case 21: //on lit l'operateur
                ...
                case '}': //fin de la lecture
                    ...
                    break;
                default:
					sprintf(suite,"%s:%c",CHAINES[77],k);
                    erreur(suite);
                }
                break;
    ...
    while (boucle) {
        ...
        case 0:
            ...
            case 2:                
                ...
                K=LectureMin(fichier);
                dollar=NON;
                while (K>32 && !strchr(tabstopchar,K)) {
                    if (K=='#') {                        
                        nompos=pos;
                        nbdd++;
                    }
            ...
            case 3:
                ...
                if (e==NULL || e->valeur==NULL) {
					if (kifmode==1) {
						...
						if (testerreur==OUI) {
							sprintf(err,"Unknown variable: %s",tampon);
							erreur(err);
						}
						else
							return NULL;
           ...
           case 17: //lecture du test du for
               ...
               if (operation==-1) {
                   sprintf(tampon,"Unknown operator: %s",ope);
                   erreur(tampon);
               }
               ope[1]=0; 
           ...
           case 18: //appel d'une procedure
               ...
               if (tf!=NULL) {
                   if (tf->Type()!=TESTFONCTIONPROCEDURE &&
                       ...
                       sprintf(tampon,"%s%s",CHAINES[307],err);
                       erreur(tampon);
                   }
               }
               soustf=new TestFormule;
...
double TestFormule::TraitementFormuleString(GlobalParseur* parseur,
    ...
    case VCHAINEDEPENDANCE:
        ...
        if (rf==NULL) {
            ...
            sprintf(parseur->ajoute_info_erreur," dependency:%d",depch->indice-1);
            return NON;
        }        
    ...
    case VLEMMECHAINEINDEX:
        ...
        if (n==NULL) {
            ...
            sprintf(parseur->ajoute_info_erreur," node:%d",vch->indice);
            return NON;
        }
    ...
    case VCHAINE:
        ...
        if (vfs->indice_chaine!=NULL) {
            ...
            if (id>=vstr->chaine.size()) {
                erreur=OUI;
                parseur->erreur_survenue=15;
                sprintf(parseur->ajoute_info_erreur," %s",vstr->nom);
                return NON;
            }
    ...
    case VCHAINENOEUD:
        ...
        if (n==NULL) {
            erreur=OUI;
            parseur->erreur_survenue=8;
            sprintf(parseur->ajoute_info_erreur," node:%d",vchn->indice);
            return OUI;
        }
    ...
    case VDICTIONNAIRE:
		...
		if (vvect->indice_chaine!=NULL) {
			...
			if (id>=buffbis.size()) {
				erreur=OUI;
				parseur->erreur_survenue=15;
				sprintf(parseur->ajoute_info_erreur," %s",vdict->nom);
				return NON;
			}
			car=buffbis[id];
			buff+=car;
        ...
        if (vvect->var->chaine[(int)res]!=NULL) {
            if (vvect->indice_chaine!=NULL) {
                ...
                if (id>=vvect->var->chaine[(int)res]->size()) {
                    erreur=OUI;
                    parseur->erreur_survenue=15;
                    sprintf(parseur->ajoute_info_erreur," %s",vvect->var->nom);
                    return NON;
                }
    ...
    case 25: //right of a string
        ...
        if (operateur==24)//c'est la partie gauche
            buff+=c_left(buffbis,iv);
        else
            buff+=c_right(buffbis,iv);
        break;            
    ...
    case 89: //regexpback
        ...
        if (a!=NULL) {                                    
            ...
            case 37:
                ...
                if (e==NULL)
                    break;
                reschaine=c_left(buffbis,a->maxpos);
                res=0;
                break;
            case 89:
    ...
    case 94: //@monthstring
        time(&date_et_heure);
        temps = localtime(&date_et_heure); 
        strcpy(ch,parseur->CHAINES[338+temps->tm_mon]);
        buff+=ch;
        break;
    case 95://@daystring
        time(&date_et_heure);
        temps = localtime(&date_et_heure); 
        strcpy(ch,parseur->CHAINES[350+temps->tm_wday]);
        buff+=ch;
        break;
    case 96://@hour
        time(&date_et_heure);
...
double TestFormule::TraitementFormule(GlobalParseur* parseur,VECTA<Noeud*>& noeuds,
    ...
    case VNOEUD:
        ...
        if (n==NULL) {
            erreur=OUI;
            parseur->erreur_survenue=8;
            sprintf(parseur->ajoute_info_erreur," node:%d",v->indice);
            return NON;
        }
    ...
    case VDEPENDANCE:
        ...
        if (rf==NULL) {
            erreur=OUI;
            parseur->erreur_survenue=9;
            sprintf(parseur->ajoute_info_erreur," dependency:%d",dep->indice-1);
            return NON;
        }        
        ind=dep->var->AjouteDependance(rf);
        i=dep->var->position;
            ...
            if (ivect->indice_vecteur==NULL)
                ...
                if (id<ivect->var->valeurs.dernier)
                    ...
                    if (operateur!=0) {
                        ...
                        sprintf(parseur->ajoute_info_erreur," %s",ivect->var->nom);
                        return NON;
                    }
                }
            }
...
double TestFonctionDependance::TraitementFormule(GlobalParseur* parseur,
	...
	if (depproc->arg.dernier!=arg.dernier) {
		...
		sprintf(parseur->ajoute_info_erreur," %s",Fx->nom);
		return -1;
	}
        ...
        if (nomproc->parametres[iar]->reinit==NON) {
            ...
            if (e!=NULL) {
                ...
                if (vtform==NULL || formules[iar]==NULL) {
                    ...
                    sprintf(parseur->ajoute_info_erreur," %s",nomproc->Fx->nom);
                }
                else {
...
double TestFonctionKif::TraitementFormule(GlobalParseur* parseur,
    ...
    for (iar=0;iar<arg.dernier;iar++) {
        ...
        if (arg[iar]>=0) {//c'est un noeud
            ...
            if (n==NULL) {
                err=OUI;
                sprintf(parseur->ajoute_info_erreur," %s: Missing node variable",Fx->nom);
                return NON;
            }
	...
	if (Error() || err) {
		...
		sprintf(parseur->ajoute_info_erreur," %s",STR(sres));
...
double TestFonctionPython::TraitementFormule(GlobalParseur* parseur,
	...
	pcourant=PyInt_FromLong(parseur->iParseurCourant);

	char chrun[100];
	sprintf(chrun,"XipNode.parseur=%d",parseur->iParseurCourant);
	PyRun_String(chrun,Py_file_input,pDict_principal,proc_python->pDico);
	sprintf(chrun,"XipDependency.parseur=%d",parseur->iParseurCourant);
	PyRun_String(chrun,Py_file_input,pDict_principal,proc_python->pDico);
    ...
    for (iar=0;iar<arg.dernier;iar++) {
        ...
        if (arg[iar]>=0) {//c'est un noeud
            ...
            if (n==NULL) {
                err=OUI;
                sprintf(parseur->ajoute_info_erreur," %s: Missing node variable",nomproc->Fx->nom);
                return NON;
            }
            //On donne le numero de noeud comme parametre
            pcourant=PyInt_FromLong(n->num_noeud);
        }
        else {//c'est une dependance
            int idep=arg[iar]*-1;
            if (dep!=NULL && (*dep)[idep]!=NULL)
                pcourant=PyInt_FromLong((*dep)[idep]->index);
            else
                pcourant=PyInt_FromLong(-1);
        }
    ...
    for (iar=0;nomproc!=NULL && iar<nomproc->parametres.dernier;iar++) {
        ...
        if (nomproc->parametres[iar]->reinit==NON) {
            ...
            case VPYTHONOBJET:
                if (((VariablePython*)variab)->echange!=NULL) {
                    ...
                    py_args.ajoute(PyInt_FromLong(0));
				type_utilise=OUI;
                break;
            case VVECTEURCHAINE:
                ...
                for (i=0;i<vecteurchaine->chaine.dernier;i++) {
                    pcourant=PyString_FromString(vecteurchaine->chaine[i]->c_str());
                    PyList_Insert(liste,i,pcourant);
					Py_DECREF(pcourant);
                }
                py_args.ajoute(liste);                
            ...
            case VDICTIONNAIRE:
                ...
                for (iter=vdict->dico.begin();iter!=vdict->dico.end();iter++) {
					pelement=PyString_FromString((*iter).second.c_str());
        ...
        if (formules[iar]!=NULL) {
            if (formules[iar]->TypeString()) {
                ...
                if (nomproc->parametres[iar]==NULL)
                    return NON;
                pcourant=PyString_FromString((char*)buff.c_str());                
            }
            else {
                res=formules[iar]->TraitementFormule(parseur,noeuds,err,dep);
                if (nomproc->parametres[iar]->type==ENTIER)
                    pcourant=PyInt_FromLong((long)res);
                else
            ...
            if (nomproc->parametres[iar]->sorte==VCHAINE)
                pcourant=PyString_FromString("");
            else
                pcourant=PyInt_FromLong(0);
        }
        ...
        if (nomproc->parametres[iar]->reinit==NON) {
            ...
            pentree=PyString_FromString(STR(dom));
            ...
            if (parseur->OS==NULL) {
                ...
                *os<<"Error in file:"<<proc_python->nomfichier<<Endl;
                ...
                if (pvalue!=NULL && PyString_Check(pvalue)) {
                    ...
                    *os<<Endl<<"In python procedure \""<<proc_python->nom<<"\": "<<PyString_AsString(perr)<<Endl;
					Py_DECREF(perr);
				}
        ...
        if (nomproc->parametres[iar]->reinit==NON) {
            ...
            if (pcourant!=NULL) {
				...
				case VVECTEURCHAINE:
                    ...
                    if (PyList_Check(pcourant)==1) {
                        ...
                        for (i=0;i<tail;i++) {
                            ...
                            //On le range dans notre vecteur
                            if (pelement!=NULL && PyString_Check(pelement))
                                reschaine=PyString_AsString(pelement);
                            else
                                reschaine="";
                ...
                case VDICTIONNAIRE:
                    ...
                    while (PyDict_Next(pcourant, &pos, &key, &value)) {
                        if (key!=NULL && value!=NULL && PyString_Check(key) && PyString_Check(value)) {
                            ...
                            reschaine=PyString_AsString(value);
                ...
                case VCHAINE:
                    pelement=PyDict_GetItem(proc_python->pDico,pcourant);                   
                    if (PyString_Check(pelement))
                        ((VariableString*)variab)->chaine=PyString_AsString(pelement);
                    Py_DECREF(pcourant);
                    PyDict_DelItem(proc_python->pDico, pcourant);
                    break;
                }                 
	...
	if (presultat!=NULL) {
		if (PyFloat_Check(presultat))
			...
			if (PyInt_Check(presultat))
				res=PyInt_AsLong(presultat);
			else {
