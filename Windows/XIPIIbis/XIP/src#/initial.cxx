...
char VERSION_XIP_BUILD[] = "13.02 \"build 07\"";
...
void afficheversion() {
	cout<<"XIP:"<<VERSION_XIP_BUILD<<endl;
}
...
GlobalParseur* ChoixParseur(Fichiergrm& fgrm,char xml,char* chemin) {
    ...
    if (xml==OUI) {
        ...
        try {
			...
			strcpy(gxml->nom_fichier_grm, fgrm.grmname);
            return gxml;
        }
    ...
    if (fgrm.trans[0]!=0) {
        ...
        try {
			...
			strcpy(gtrans->nom_fichier_grm, fgrm.grmname);
            return gtrans;
        }
        catch(XIPError* lerreur) {
            if (gtrans!=NULL)
    ...
    if (fgrm.tokenize[0]!=0 && fgrm.lookup[0]!=0) {
        ...
        try {
			...
			strcpy(glob->nom_fichier_grm, fgrm.grmname);
			return glob;
        }
        catch(XIPError* lerreur) {
            if (glob!=NULL)
    ...
    if (gp==NULL && fgrm.ntm[0]!=0) {
        ...
        try {
			...
			strcpy(gntm->nom_fichier_grm, fgrm.grmname);
			return gntm;
        }
        catch(XIPError* lerreur) {
            if (gntm!=NULL)
...
#else
	if ( (ntm!=NULL && ntm[0]!=0) || tok[0]!=0 || lookup[0]!=0)
		lance_erreur("This XIP version is not FST enabled");
#endif
    ...
    if (fgrm.hmm[0]!=0) {
        ...
        try {
			...
			strcpy(ghmm->nom_fichier_grm, fgrm.grmname);
            return ghmm;
        }
        catch(XIPError* lerreur) {
            if (ghmm!=NULL)
    ...
    try {
		...
		strcpy(gp->nom_fichier_grm, fgrm.grmname);
        return gp;
    }
    catch(XIPError* lerreur) {
        if (gp!=NULL)
...
int GlobalParseur::InitialisationParseur(typeLangue langue,
    ...
    *flot_erreur<<endl;
    ...
    gVecteurParseurs.detruit(iParseurCourant);
...
Exportation void XipGrammarPathnames(int ipar,
	...
	if (leParseur->Type()==PARSEURTRANS) {
		...
		for (ii=0;ii<gtrans->auvol.size();ii++)
			divers.push_back(gtrans->auvol[ii]);
		for (ii=0;ii<gtrans->fichiersvoc.size();ii++)
			divers.push_back(gtrans->fichiersvoc[ii]);
		divers.push_back(gtrans->nom);
	}
...
Exportation void XipGrammarInformation(int ipar,
   ...
   if (leParseur->ChunkerLicense==OUI)
    ...
    if (leParseur->UsageLimitee==1) {
        string str="limited:";
        char ch[100];
        sprintf(ch,"%d",leParseur->max_controle_regle);
        str+=ch;
...
Exportation void XipFreeGrammar(unsigned int iParseur) {
    gVecteurParseurs.detruit(iParseur);     
}
...
Exportation int XipLoadGrammarBase(char* grmfile,
    ...
    char crypte=NON;
	Fichiergrm fgrm;
	strcpy(fgrm.grmname,grmfile);
	strcpy(fgrm.ntm,ntmfile);
	strcpy(fgrm.hmm,hmmfile);
	strcpy(fgrm.genere,generefile);
	strcpy(fgrm.tokenize,tokenfile);
	strcpy(fgrm.lookup,lookupfile);
	strcpy(fgrm.lookup_flags,lookup_flags);
...
//	leParseur->kifcode=kcode;
    ...
    if (reloadparameter==0 || chemingrm==OUI) {
    ...
    if (chemingrm==OUI) {
		...
		strcpy(leParseur->Nom_Fichier,fgrm.grammar);
		Recuperenom(fgrm.grammar,leParseur->repertoire);
		strcpy(leParseur->nom_fichier,fgrm.grammar);
    }
    else
        strcpy(leParseur->nom_fichier,leParseur->Nom_Fichier+strlen(leParseur->repertoire));

	if (leParseur->Type()==PARSEURTRANS) {
		GlobalParseurTrans* gtrans=(GlobalParseurTrans*)leParseur;
		gtrans->rechargementsarcs();
	}
...
Exportation int XipSetIndentFilePathname(int ipar,string pathname) {
	...
	strcpy(leParseur->Nom_Sortie,pathname.c_str());
	return 0;
}
...
Exportation int XipSetWorkingFilesPath(int ipar,string path) {
    ...
    if (last_slash==NULL)
        sprintf(nom,"%c%s",SEP,leParseur->Nom_Sortie);
    else
		strcpy(nom,last_slash);    

	strcpy(leParseur->Nom_Sortie,path.c_str());
	strcat(leParseur->Nom_Sortie,nom);
    last_slash= strrchr(leParseur->Nom_Trace,SEP);
    
    if (last_slash==NULL)
        sprintf(nom,"%c%s",SEP,leParseur->Nom_Trace);
    else
		strcpy(nom,last_slash);    

	strcpy(leParseur->Nom_Trace,path.c_str());
	strcat(leParseur->Nom_Trace,nom);
...
char LoadGrmFiles(Fichiergrm& fgrm,int& ITAILLEVECT,char premier) {
    ...
    if (grm.fail()) {
        sprintf(bufread,"Cannot open: %s",fgrm.grmname);
    ...
    while (!grm.eof()) {
        ...
        if (!strcmp(bufread,"flags:")) {
			...
			if (fgrm.lookup_flags[0]) {
				strcpy(bufread,"A FLAG string has already been provided");
				lance_erreur(strdup(bufread));
			}
            ...
            if (bufread[0]!=0) {
                ...
                strcpy(fgrm.lookup_flags,bufread);            
            }
            continue;
        }
		...
		if (!strcmp(bufread,"blockingrulespace:")) {			
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Blocking Rule Space string too long");
                    lance_erreur(strdup(bufread));
                }
		...
		if (!strcmp(bufread,"kifarg:")) {
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"KIFARG too long");
                    lance_erreur(strdup(bufread));
                }
        ...
        if (!strcmp(bufread,"grm:")) {
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for GRM too long");
                    lance_erreur(strdup(bufread));
                }
				strcpy(grmkeep,fgrm.grmname);
				strcpy(fgrm.grmname,bufread);
                crypte=LoadGrmFiles(fgrm,numberValues,NON);
				strcpy(fgrm.grmname,grmkeep);
            }
            continue;
        }
        ...
        if (!strcmp(bufread,"lookup:")) {
			if (fgrm.lookup[0]) {
				strcpy(bufread,"A LOOKUP file has already been provided");
				lance_erreur(strdup(bufread));
			}
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for LOOKUP too long");
                    lance_erreur(strdup(bufread));
                }
        ...
        if (!strcmp(bufread,"tokenizer:")) {
			if (fgrm.tokenize[0]) {
				strcpy(bufread,"A TOKENIZER file has already been provided");
				lance_erreur(strdup(bufread));
			}
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for TOKENIZER too long");
                    lance_erreur(strdup(bufread));
                }
                strcpy(fgrm.tokenize,bufread);            
            }
            continue;
        }
		...
		if (!strcmp(bufread,"generation:")) {
			if (fgrm.genere[0]) {
				strcpy(bufread,"A GENERATION file has already been provided");
				lance_erreur(strdup(bufread));
			}
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for GENERATION too long");
                    lance_erreur(strdup(bufread));
                }
                strcpy(fgrm.genere,bufread);            
            }
            continue;
		}
        ...
        if (!strcmp(bufread,"ntm:")) {
			if (fgrm.ntm[0]) {
				strcpy(bufread,"A NTM file has already been provided");
				lance_erreur(strdup(bufread));
			}
        ...
        if (!strcmp(bufread,"trans:")) {
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for TRANS too long");
                    lance_erreur(strdup(bufread));
                }       
                strcpy(fgrm.trans,bufread);                            
            }
            continue;
        }
        ...
        if (!strcmp(bufread,"hmm:")) {
			if (fgrm.hmm[0]) {
				strcpy(bufread,"A HMM file has already been provided");
				lance_erreur(strdup(bufread));
			}
        ...
        if (!strcmp(bufread,"crfmodel:")) {
			if (fgrm.model[0]) {
				strcpy(bufread,"A CRF model file has already been provided");
				lance_erreur(strdup(bufread));
			}
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for MODEL too long");
                    lance_erreur(strdup(bufread));
                }       
                strcpy(fgrm.model,bufread);                            
            }
            continue;
        }
        ...
        if (!strcmp(bufread,"crfmkcls:")) {
			if (fgrm.mkcls[0]) {
				strcpy(bufread,"A MKCLS file has already been provided");
				lance_erreur(strdup(bufread));
			}
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for MKCLS too long");
                    lance_erreur(strdup(bufread));
                }       
                strcpy(fgrm.mkcls,bufread);                            
            }
            continue;
        }
        ...
        if (!strcmp(bufread,"crfbrown:")) {
			if (fgrm.brown[0]) {
				strcpy(bufread,"A BROWN file has already been provided");
				lance_erreur(strdup(bufread));
			}
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for BROWN too long");
                    lance_erreur(strdup(bufread));
                }       
                strcpy(fgrm.brown,bufread);                            
            }
            continue;
        }
        ...
        if (!strcmp(bufread,"grammar:")) {
			if (fgrm.grammar[0]) {
				strcpy(bufread,"A GRAMMAR file has already been provided");
				lance_erreur(strdup(bufread));
			}
            GrmTrim(grm,(uchar*)bufread,999,OUI);
            if (bufread[0]!=0) {
        ...
        if (!strcmp(bufread,"gram:")) {
            ...
            if (bufread[0]!=0) {
                if (strlen(bufread)>999) {
                    strcpy(bufread,"Pathname for GRAM too long");
                    lance_erreur(strdup(bufread));
                }
                strcpy(fgrm.grammar,bufread);
            }
		...
		if (!strcmp(bufread,"parameter:") ||
			...
			if (bufread[0]!=0) {
				if (strlen(bufread)>999) {
					strcpy(bufread,"Pathname for PARAMETER too long");
					lance_erreur(strdup(bufread));
				}
...
{
    sprintf(question,"XIP: Xerox Incremental Parser %s (2000-2013) ",VERSION_XIP_BUILD);
}
...
Exportation char XipGetLabels(int ipar, char* surface,char* lemme) {
    ...
    strcpy(surface,leParseur->SURFACE);
    strcpy(lemme,leParseur->LEMME);
    return 0;
}
