...
public:
	...
	Exported KifElement* IteratorKey() {
		if (reverse == false) {
			if (it == values->end())
				return kifNOELEMENT;
			return new KifUString(kifcode, NULL, it->first);
		}
		if (itr == values->Rend())
			return kifNOELEMENT;
		return new KifUString(kifcode, NULL, it->first);
...
public:
	...
	KifElement* IteratorKey() {
		...
		return new KifUString(kifcode, NULL, itr->first);
	}
	...
	KifElement* IteratorValue() {
		if (reverse == false) {
			...
			return new KifUString(kifcode, NULL, it->second);
		}
		if (itr == values->Rend())
			return kifNULL;

		return new KifUString(kifcode, NULL, itr->second);
	}
