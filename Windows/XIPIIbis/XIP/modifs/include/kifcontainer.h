...
public:
	...
	Exported bool Insertvalue(KifDomain* dom, KifElement* v, hmap<short, KifElement*>&);
...
public:
	...
	void storevalue(wchar_t v) {
		KifContainerLock _lock(this);
		values.push_back(c_unicodetoutf8(v));
	}
	...
	wstring getustring(int i) {
		...
		if (i < 0 || i >= values.size())
			return 0;
		wstring wx;
		s_utf8tounicode(wx, STR(values[i]));
		return wx;
	}
...
public:
	...
	wstring IteratorValueUString() {
		...
		if (i < 0)
			return L"";
		wstring w;
		s_utf8tounicode(w, STR((*values)[i]));
		return w;
	}
...
public:
	...
	KifElement* ValueOnIndex(size_t ikey) {
		...
		if (ikey < 0 || ikey >= values.size())
			return kifNOELEMENT;
		return new KifUString(kifcode, NULL, values[ikey]);
	}
	...
	KifElement* Last() {
		...
		if (values.size() == 0)
			return kifNULL;
		return new KifUString(kifcode, NULL, values.back());
	}
...
public:
	...
	KifElement* IteratorValue() {
		...
		if (i < 0)
			return kifNOELEMENT;
		return new KifUString(kifcode, NULL, (*values)[i]);
	}
...
public:
	...
	string getstring(int i) {
		...
		if (i < 0 || i >= values.size())
			return "";
		return c_unicodetoutf8(values[i]);
	}
...
public:
	...
	//Predicate management
	bool Insertvalue(KifDomain* dom, KifElement* v, hmap<short, KifElement*>& kept);
	bool unified(KifDomain* dom, KifElement* a);
	bool unifiedDeclaration(KifDomain* dom);
	Exported KifElement* ExtractPredicateVariables(KifElement* contextualpattern, KifDomain* dom, KifElement* val, KifElement* e, bool root);
	Exported KifElement* EvaluePredicateVariables(KifElement* context, KifDomain* dom);
...
public:
	...
	KifElement* IteratorKey() {
		if (reverse)
			return new KifUString(kifcode, NULL, *rit);
		return new KifUString(kifcode, NULL, *it);
	}
