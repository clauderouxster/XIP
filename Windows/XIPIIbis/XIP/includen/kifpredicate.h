...
public:
	...
	bool Insertvalue(KifDomain* dom,KifElement* v,hmap<short,KifElement*>&);
...
public:
	...
	Exported void Methods(KifVectorString* fcts);
	Exported KifElement* Information(KifElement*);
	Exported KifElement* Store(KifElement* db,KifDomain*,int);
	Exported KifElement* Remove(KifElement* db,KifDomain*,int);
};
...
public:
	...
	hmap<short, bool> sorted;
	hmap<short,KifPredicateVariableInstance*>* dico;
	...
	Exported KifElement* Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* val=kifNULL);
	KifElement* PredicateEvalue(VECTA<KifElement*>& stack,KifPredicate* currenthead,bool cut,int depth);
...
public:
	...
	Exported KifElement* Execute(KifElement* contextualpattern,KifDomain* dom,KifElement* val=kifNULL);	
	Exported KifElement* Evalue(KifInstructionEvaluate* contextualpattern,KifPredicate*,VECTA<KifElement*>& stack,int pos,int depth,bool cut);
...
public:
	...
	KifPredicateLocalInstruction(KifCode* klc,hmap<short,KifPredicateVariableInstance*>* context,KifElement* e,bool n,bool d) : KifElement(NULL,NULL,kifPredicateLocalInstruction) {
        dico=context;
		negation=n;
		disjunction=d;
		instruction=e;
