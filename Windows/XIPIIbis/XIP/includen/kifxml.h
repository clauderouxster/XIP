...
public:
	...
	bool Setvalue(KifElement* kval,KifElement* kindex,KifElement* domain,int idthread=0,KifCallFunction* callfunc=NULL);
	...
	KifElement* MethodNext(KifElement* contextualpattern,KifDomain* domain,KifCallFunction* callfunc,int idthread) {
        ...
        if (knode->type!=kifXML)
            return kifcode->Returnerror("XML(098): Expecting an XML node");
...
public:
	...
	KifElement* MethodOnClosing(KifElement* contextualpattern,KifDomain* domain,KifCallFunction* callfunc,int idthread) {
		...
		objectend=callfunc->parameters[1]->Instruction(0);

		if (objectend->type==kifInstance)
			objectend=((KifInstance*)objectend)->Thevariable(domain,kifGlobals->GetThreadid(),NULL);
		else
			objectend=objectend->Execute(kifNULL,domain);
		return kifTRUE;
	}
