...
public:
	...
	virtual Exported bool Setvalue(KifElement* ke, KifElement* idx, KifElement* dom = NULL,int idthread=0,KifCallFunction* callfunc=NULL);
...
public:
	...
	long getinteger(int i) {
		...
		if (i<0 || i >= values.size())
			return 0;
		return s_XLConvert(STR(values[i]));
	}
...
public:
	...
	KifTable(KifCode* klc, KifElement* kp) : KifEvaluate(klc, kp, kifTable) {
		values = NULL;
		size = 0;
	}
