...
char GlobalParseur::AnalyseForme(VECTA<char*>& enreg,
	...
	if (prolog == OUI) {
		...
		strcpy_s(vlex.surfacebrute,2048, enreg[0]);
		strcpy_s(vlex.surface,2048, enreg[1]);
		strcpy_s(vlex.lemme,2048, enreg[2]);
	}
...
ListeVoc* GlobalParseur::ConstruitUneForme(VECTA<char*>& enreg, char* entree, int numMot, char ajoute) {
	...
	if (catX.dernier >= 1) {
		if (AFFICHE_ERREUR == OUI) {
			char buffer[1000];
			stringstream bff;
			bff << CHAINES[64] << vlex.X->nom << "(" << catog << ")\n";
			strcpy_s(buffer, 1000, (char*)bff.str().c_str());
...
void GlobalParseur::LexiconEntryParsing(char* chaine, char** duplicat) {
	...
	*pt = 0;
	if (tablelatin) {
		chainetraduction = conversion_latin_table_to_utf8(tablelatin, (uchar*)(ptsurface + 1));
		ListeTagsAjouter(STR(chainetraduction));
	}
	else {
		if (encodage_source == encodage_grammaire)
...
void GlobalParseur::AffichageResultat() {
	...
	if (XML == OUI) {
		if (gAfficherResultats == OUI) {
			...
			if (!TestOptions(AFFICHAGE_PASSAGE_XML_FORMAT)) {
				if (langue_de_la_grammaire != NULL)
					sprintf_s(buffer,100, "<LUNIT language=\"%s\"/>", langue_de_la_grammaire);
				else
					strcpy_s(buffer,100, "<LUNIT language=\"Unknown\"/>");
			}
			else {

				sprintf_s(buffer,100, "<Sentence id=\"E%d\"/>", this->num_phrase);
			}
			/** End MCR added code for passage XML output **/
	...
	if (TestOptions(AFFICHAGE_TREEBANK)) {
		char ch[10];
		Noeud* n;
		int nm,j;
		ExtractionFonction* rf;
		ResultatFonction* fonc;
		if (NoeudsLexicaux.dernier) {
			for (i = 0; i<NoeudsLexicaux.dernier; i++) {
				...
				n = NoeudsLexicaux[i];
				if (n != NULL) {
					sprintf_s(ch,10, "%d", i + 1);
					string ligne = ch;
					ligne += "\t";
					ligne += n->fils->surface; //surface
					ligne += "\t";
					ligne += n->fils->lemme; //lemma
					ligne += "\t_\t"; //NO CPOSTAG
					ligne += n->X->nom; //POSTAG
					Chaine_traits[0] = 0;
					n->source->traitsactif->valeur->ConcateneAttributs(this, Chaine_traits, FILTRE, "|");
					ligne += "\t";
					ligne += Chaine_traits; //FEATURES
					ligne += "\t";					
					nm = n->pere->fils->dernier->num_mot;

					sprintf_s(ch,10, "%d\t", nm + 1); //HEAD
					ligne += ch;
					nm = n->pere->fils->dernier->num_noeud;
					rf = PREMIERFONCTIONS[nm]; //Dependency to the HEAD
					bool premier = OUI;

					if (rf != NULL) {
						for (j = 0; j < rf->dernier; j++) {
							fonc = rf->vecteur[j];
							if (fonc != NULL && fonc->arg.dernier == 2 && fonc->Fx != NULL && fonc->Fx->cacher == NON && fonc->arg[1] == n) {
								if (premier) {
									ligne += "\t";
									premier = NON;
									ligne += fonc->Fx->nom;
					...
					nm = n->num_noeud;					
					rf = PREMIERFONCTIONS[nm];
					premier = OUI;
					//Une dependance unaire (par exemple une entite nommee)
					if (rf != NULL) {
						sprintf_s(ch,10, "%d", nm + 1);
						for (j = 0; j < rf->dernier; j++) {
							fonc = rf->vecteur[j];
							if (fonc != NULL && fonc->arg.dernier == 1 && fonc->Fx != NULL && fonc->Fx->cacher == NON) {
								if (premier) {
									ligne += "\t";
									premier = NON;
								}
					...
					//Deuxieme argument d'une dependance
					rf = SECONDFONCTIONS[nm];
					if (rf != NULL) {
						for (j = 0; j < rf->dernier; j++) {
							fonc = rf->vecteur[j];
							if (fonc != NULL && fonc->Fx != NULL && fonc->Fx->cacher == NON) {
								if (premier) {
									ligne += "\t";
									premier = NON;
								}
								nm = fonc->arg[0]->num_mot;
								sprintf_s(ch,10, "%d", nm + 1);
								ligne += "\t";
								ligne += ch;
								ligne += "\t";
	...
	if (CalculFonction && gAfficherResultats == OUI) {
		...
		//On affiche aussi la generation si elle a ete calculee
		if (TestOptions(AFFICHAGE_GENERATION)) {
			laststrgen = "";
			parcoursgeneration(laststrgen);
			*os << "G:" << ConversionChaine(this, (char*)laststrgen.c_str()) << endl;
		}
	}
...
int GlobalParseur::TestLangue(string& laphrase) {
	...
	taille_phrase = phrase.dernier;
	if (guesser == NULL) {
		sprintf_s(ch,10, "%d", iParseurCourant);
...
void GlobalParseur::Execute(char sauve_resultat, char construit) {
	...
	if (DESAMBIGUISATION == OUI && xipresult == NULL) {
		ListeVoc* v;
		char* pt;
		int iphrase = 0;
		int iph;

		if (TestOptions(AFFICHAGE_JSON) && vkif_echange->kif->type == kifVector) {
			for (iph = 0; iph < phrase.dernier; iph++) {
				v = phrase[iph];
				KifVector* vstring = new KifVector(vkif_echange->kif->kifcode, NULL);				
				vkif_echange->kif->Push(vstring);
				KifBaseString str(NULL, NULL);
				KifBaseInteger borne(NULL, NULL);
				str.kifcode = vkif_echange->kif->kifcode;
				str.value = v->surface;
				vstring->Push(&str);
				if (v->nbX == 1) {
					str.value = v->lemme[0];
					...
					if (TestAfficheFonction(AFFICHE_TRAITS)) {
						if (v->tr[0] != NULL) {
							KifTreeMapStringString* m = new KifTreeMapStringString(vkif_echange->kif->kifcode, NULL);
							(v->tr[0])->TraitsEnMap(this, m->values);
							vstring->Push(m);
						}
					}
				}
				else {
					KifVector* readings = new KifVector(vkif_echange->kif->kifcode, NULL);
					vstring->Push(readings);
					for (i = 0; i < v->nbX; i++) {
						//Then the lemma
						str.value = v->lemme[i];
						readings->Push(&str);
						...
						if (TestAfficheFonction(AFFICHE_TRAITS)) {
							if (v->tr[i] != NULL) {
								KifTreeMapStringString* m = new KifTreeMapStringString(vkif_echange->kif->kifcode, NULL);
								(v->tr[i])->TraitsEnMap(this, m->values);
								readings->Push(m);
							}
						}
					}
				}
				borne.kifcode = vkif_echange->kif->kifcode;
				borne.value = borneGauche;
				vstring->Push(&borne);
				borne.value = borneDroite;
				vstring->Push(&borne);
				borne.value = borneGaucheCar;
				vstring->Push(&borne);
				borne.value = borneDroiteCar;
				vstring->Push(&borne);
			}
			return;
		}
...
void GlobalParseur::AffichageGenerationModeTexte(ostream* os) {
	NCoord* nco;
	float maxX = 5, maxY = 5;


	genereGraphe* fr = &graphegeneration;
...
void GlobalParseur::ExecuteExtraction() {
	...
	if (INTERVICTOIRE != NULL) {
		...
		if (AffichageArbre) {
			if (TestOptions(AFFICHAGE_GENERATION))
				AffichageGenerationModeTexte(os);
			else
	...
	if (xipresult != NULL)    {
		...
		if (TestOptions(AFFICHAGE_GENERATION)) {
			laststrgen = "";
			char car = OUI;
			parcourssousnoeud(&graphegeneration, laststrgen, car);
		}
	}
	else {
		AffichageResultat();
	...
	if (DESAMBIGUISATION == OUI) {
		if (XML != NON) {
			...
			if (xipresult != NULL) {
				...
				noeud = &Phrase[0]->N;
				INTERVICTOIRE->fils = noeud;
				INTERVICTOIRE->dernier = noeud;
				for (i = 1; i < Phrase.dernier; i++) {
					noeud->frere = &Phrase[i]->N;
					noeud = noeud->frere;
				}
