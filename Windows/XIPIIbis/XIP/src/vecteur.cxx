...
#ifdef XIPLIBXML
void Vecteur::ConstruitXML(GlobalParseur* parseur,xmlNodePtr node,Vecteur* filtre) {
        ...
        if (val!=0) {
            for (register int j=gBitVect-1;j>=0;j--) {
                ...
                if (val < 0 && fil <0) {
                    ...
                    parseur->AjouteAttributValeur(feature,"attribute",parseur->TableDesTraits[pos]);
                    parseur->AjouteAttributValeur(feature,"value",parseur->TableDesTraits[pos+1]);
                }
                val<<=1;
                fil<<=1;
            }
