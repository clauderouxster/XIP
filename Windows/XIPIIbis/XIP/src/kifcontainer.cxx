...
bool KifTuple::Setvalue(KifElement* ke, KifElement* idx, KifElement* dom) {
	...
	if (idx != NULL && !idx->isConst())
		return kifcode->Returnerror("KIF(415): You cannot modify a tuple element");
	...
	size = ke->Size();
	if (!size)
		return kifcode->Returnerror("KIF(416): You cannot create an empty tuple");
	...
	if (!ke->aContainer())
		return kifcode->Returnerror("KIF(417): Expecting a container");
...
KifElement* KifTable::Declaration(short n) {
	if (TestkifFunctions(n, kifGlobals->kifVectorFunctions) == true || n == idwaiton || n == idtrigger || n == KifStringId("addtowaiton") || n == KifStringId("resize") || n == KifStringId("count"))
		return kifGlobals->kifBaseFunctions[n];
...
void KifTable::Methods(KifVectorString* fcts) {
	...
	for (it = kifGlobals->kifVectorFunctions.begin(); it != kifGlobals->kifVectorFunctions.end(); it++)
		fcts->values.push_back(kifGlobals->kifIdString[it->first]);
	fcts->values.push_back(kifGlobals->kifIdString[KifStringId("resize")]);
	fcts->values.push_back(kifGlobals->kifIdString[KifStringId("count")]);
}
