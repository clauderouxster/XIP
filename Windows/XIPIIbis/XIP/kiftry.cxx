...
typedef KifElement* (KifTemplateName::*TemplateNameMethod)(KifElement*,KifDomain*,KifCallFunction*);
...
public:
	...
	static map<string,TemplateNameMethod> linkedmethods;
...
map<string,KifCallMethod> KifTemplateName::kifexportedmethods;
map<string,TemplateNameMethod>  KifTemplateName::linkedmethods;
...
KifElement* KifIteratorTemplateName::Next() {
...
static void MethodInitialization(string name,TemplateNameMethod func) {
	//A KifCallMethod object is used by KiF to call the ExecuteMethod on our object
	KifCallMethod kcm(name);
	KifTemplateName::kifexportedmethods[name]=kcm;
	KifTemplateName::linkedmethods[name]=func;
}
...
extern "C" {
	Exported bool KifInitialisationModule(KifCode* kifcode) {
		...
		TemplateName_type=kifcode->KifAddNewType("TemplateName",CreatekifTemplateNameElement);
		if (TemplateName_type==kifVOID)
			return false;
		...
		MethodInitialization("sample",&KifTemplateName::MethodSample);
