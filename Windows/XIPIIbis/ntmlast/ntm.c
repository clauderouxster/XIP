/* $Id: ntm.c,v 1.14 2004/05/14 09:46:59 ait Exp $ */ 
...
inline int isSpace(NtmContext* ct,int c) {return ct->spaces[(unsigned) c];}
inline int isSeparator(NtmContext* ct,int c) {
    return ct->separators[(unsigned) c];}
...
inline void ntmUndefineSpaces(NtmContext* ct) {
    memset(ct->spaces, 0, NTM_SPACES_SIZE*2);
    ct->spaces[ID_EOT] = 1;
...
inline void ntmUndefineSeparators(NtmContext* ct) {
    memset(ct->separators, 0, NTM_SEPARATORS_SIZE*2);
    ct->separators[ID_EOT] = 1;
