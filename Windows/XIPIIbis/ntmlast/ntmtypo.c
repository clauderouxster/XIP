/* $Id: ntmtypo.c,v 1.7 2003/05/20 14:52:44 ait Exp $ */ 
...
collectFinalSol( NtmContext* ct, 
        ...
        if (j >= typodata->longestInputLength) {
            if ( (typodata->tempoSol[i].side == NONE) ||
                 ( (typodata->tempoSol[i].side == LEFT) &&
                   (checkLeftContext( &(ct->levels), initpos - 1,
                                     configs[ind].cpos,
                                     typodata->tempoSol[i].cstate,
                                     net) > 0 ) ) ||
                 (checkRightContext( &(ct->levels), configs[ind].cpos,
                                      typodata->tempoSol[i].cstate,
                                      net) > 0 ) ) {
                if (j > typodata->longestInputLength) {
    ...
    if (found_sol == 0) {
        ...
        symbolid = readInputCharId(&(ct->levels), initpos);
 ...
 */
inline int checkRightContext( Levels *lev, long rightpos,
                              STATEptr fromstate,
                              NETptr net) {
...
inline int checkLeftContext( Levels* lev, long leftpos,
                             long rightpos,
                             STATEptr fromstate,
                             NETptr net) {
        ...
        if ( STATE_vector_p(cstate) ) {
            ...
            if ( binary_sigma_member( NET_sigma(net), ID_RC) ) {
                ...
                for(i=0; i < range_len; i++) {
                    if ( cstate = AV_dest(arc_vector, range_items[i])  ) {
                        /* match with RC ! */
                        return checkRightContext(lev, rightpos,
                                             /* ARC_destination(arc),*/
                                             cstate,
                                            net) ;
                    }
...
inline NormElement *readNormElement2(NtmContext *ct, int initpos, NETptr net) {
    ...
    while ( (td->cconfigsNb > 0) ) {
        ...
        for(i=0, k=td->cconfigs[i]; i < td->cconfigsNb ; i++, k=td->cconfigs[i]) {
            /* read the next input for this config */
            in_id = readInputCharId(&(ct->levels), configs[k].cpos);
            if (in_id == ID_EOT) {
