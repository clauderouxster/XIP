/* $Id: ntmmorpho.c,v 1.10 2004/05/14 09:47:00 ait Exp $ */ 
...
inline void processTempoSol(NtmContext *ct, int configind, Side side) {
    ...
    if (tempoSolNb == MORPHO_TEMPOSOL_MAXSIZE) {
		...
		if ( (ct->checkSeparatorConstraint == FALSE) ||
             (side != NONE) ||
             ( ( isSeparator(ct,
                             readInputCharId(&(ct->levels),cpos)) ||
                 isSeparator(ct,
...
NtmToken * collectDefaultSol(NtmContext *ct) {
    ...
    if (normform != NULL) {
        for(i=0; i < length; i++)
            normform[i] = readInputCharId(&(ct->levels),i+ct->inputPos);
            ...
            if (tempoSol[i].weight >= weight) {
                if ( ( tempoSol[i].side == NONE) ||
                     ...
                     ( (tempoSol[i].side == RIGHT) &&
                       (checkRightContext( &(ct->levels), initpos+j,
                                         tempoSol[i].cstate,
                                         net) > 0 ) ) ) {
                    lg = j;
...
ntmReadToken(
    ...
    if (endOfInput(&(ct->levels), ct->inputPos)) return getLastToken();
