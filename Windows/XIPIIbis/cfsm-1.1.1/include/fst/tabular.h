/* $Id: tabular.h,v 1.6 2000/01/04 18:26:36 privault Exp $ */
/******************************************************
 **
 **  TABULAR.H
 **
...
typedef struct NET_TRANSITION_TABLE {
  struct {
    bit simple_fsm:1;          /* flag: the net is not a transducer */
    bit single_char_upper:1;   /* flag: all upper labels are single chars */
    bit single_char_lower:1;   /* flag: all lower labels are single chars */
    bit unused:5;              /* 5 bit for any purpose */
  } flags;
...
typedef struct TT_LABEL_INFO {
  struct {
    bit glob_id:1;              /* flag: the ID is a global one */
    bit simple_id:1;            /* flag: the ID is of arrity one */
    bit single_char_upper:1;    /* flag: upper label is a single char */
    bit single_char_lower:1;    /* flag: lower label is a single char */
  } flags;
...
NTTptr FsmFstExport NTT_evaluate_table (NTTptr ntt);
...
NETptr FsmFstExport TTB_ttnet_to_net (NETptr ttnet, int reclaim_p);
