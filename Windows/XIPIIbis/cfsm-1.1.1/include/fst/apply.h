/* $Id: apply.h,v 1.46 2003/02/12 23:08:52 lk Exp $ */
...
id_type FsmFstExport *INPUT_MATCH;  /* match table of the input side */
id_type FsmFstExport *OUTPUT_MATCH; /* match table of the output side */
...
void FsmFstExport select_match_tables_for_apply (int input_side);
...
typedef struct APPLY_CONTEXT {
    ...
    void (*next_symbol_fn)(id_type *, void *);
...
void free_io_sequence(IO_SEQptr io_sequence);
void initialize_io_sequence(IO_SEQptr *io_sequence);
void reset_io_sequence(IO_SEQptr io_sequence);
...
APPLYptr FsmFstExport init_apply_context(APPLYptr cntxt);
void FsmFstExport free_apply_context(APPLYptr cntxt);
...
void FsmFstExport init_apply_to_string(char *input, APPLYptr apply_context);
void FsmFstExport init_apply_to_stream(FILE *stream, APPLYptr apply_context);
void FsmFstExport init_apply_net(APPLYptr cntxt);

/* Creating and initializing an APPLY_CONTEXT_VECTOR structure: */
void FsmFstExport initialize_hyper_unit(APPLYptr apply_context, void (*final_output_fn)(void*));

int apply_standard_network(APPLYptr apply_context);
int apply_reduced_network(APPLYptr apply_context);
int apply_bimachine(APPLYptr apply_context);
int FsmFstExport apply_network(APPLYptr apply_context);
int apply_patterns(APPLYptr apply_context);
...
void FsmFstExport write_to_buffer(void *apply_context);
void merge_to_buffer(void *apply_context);
void FsmFstExport store_in_table(void *apply_context);
void write_to_out_stream(void *cntxt);

/* Storing the final output of compose-apply in a tree structure: */
void FsmFstExport store_in_a_tree(void *cntxt);
void store_in_out_seq(void *apply_context);
void fetch_output(void *cntxt);
void symbol_from_in_vector(id_type *in_sym, void *apply_context);
...
/* APPVECptr create_apply_context_vector(NVptr nv, void *final_output_fn, int side_p); */
APPVECptr create_apply_context_vector(NVptr nv, void (*final_output_fn)(void*), int side_p);
void FsmFstExport free_apply_context_vector (APPVECptr appvec);
void FsmFstExport reset_apply_context_vector (APPVECptr appvec);

int FsmFstExport apply_netvector (APPVECptr appvec);

/* Set up of the global variables INPUT_MATCH and OUTPUT_MATCH at the
   initialization of the recursive compose-apply, depending on the
   value of the input_side. */
int recursive_compose_apply_networks(char *string, APPLYptr apply_context);
