/* $Id: types.h,v 1.48 2003/02/12 23:00:07 lk Exp $ */
...
void FsmFstExport * get_default_context(void) ; /* Needed for the utf8 macros */
...
typedef struct ARC {
    ...
    bit unused : 9;   /* Use at your own risk. */
#endif
...
typedef struct BIG_ARC {
    ...
    bit visit_mark : 2;
    bit big_arc_flag : 1; /* 0 in ARC objects, 1 in BIG_ARC objects */
    bit unused : 11;   /* Use at your own risk. */
    id_type label; /* index into label table -- use id_to_label() to access print name */
    struct STATE *destination;
    struct BIG_ARC *next;
    void *user_pointer; /* only ALTCHAINS uses this field at the moment */
...
typedef struct STATE {
	...
	bit vector_p : 1;
        ...
        bit userflag2 : 2;
        bit is_virtual: 1;
        bit unused : 1;	   /* Use at your own rist */
	union { 
		struct ARC *set;
...
enum char_encoding {CHAR_ENC_UTF_8, CHAR_ENC_ISO_8859_1};
...
#define FST_symbol_from_string		(IS_UTF8_MODE ? utf8_symbol_from_string : symbol_from_string )
...
#define FST_word_file_to_fsm		(IS_UTF8_MODE ? utf8_word_file_to_fsm : word_file_to_fsm )
